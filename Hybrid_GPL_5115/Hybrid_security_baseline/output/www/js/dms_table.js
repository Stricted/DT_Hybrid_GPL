...
$(".button_dms_delete_list").live("click", function(){
    if($(".add_item_control:hidden").size() > 0){
        deleteIndex = $(".button_dms_delete_list").index(this);
        if(!isButtonEnable("button_delete"+choice[deleteIndex])){
            return;
        }    
        parent.call_dialog(common_delete, firewall_hint_delete_list_item, common_ok, "pop_OK", common_cancel, "pop_Cancel",delete_pop_ok);
...
function delete_pop_ok() {
    deleteFilter(deleteIndex, $(".user_dms_add_line"));
    parent.clearDialog();
    if(choice.length == 1){
        var choice_old =[];
        addFilter($('#share_directory_list tr'), choice_old);
        button_enable('button_add'+choice[0], '1');
        return;
    }
    
    if(choice.length>1){
        if(choice.length-1==deleteIndex){
            $("#button_add"+choice[choice.length -2]).show();
        }
    }
    choice.splice(deleteIndex,1);
    if(choice.length == ADD_NUM-1){
        button_enable('button_add'+choice[ADD_NUM-2], '1');
    }
}

$(".button_dms_browse_list").live("click", function(){
    var index = $(".button_dms_browse_list").index(this);
    if (!isButtonEnable('button_browse'+choice[index])){
        return;
    }
    check_browse_num = index;
    parent.$('.dialog_close_btn').die('click');
...
function browse_pop_ok() {
    ...
    if (typeof(folder_path) != 'undefined') {
        ...
        parent.$('.dialog, .info_dialog').fadeOut(function() {
            if (0 == parent.$('.info_dialog').size()) {
                parent.$('#div_wrapper').hide();
            }
        });
        $('#share_directory_input'+choice[check_browse_num]).attr("value",folder_path);
        parent.$('#wait_table').remove();
        if (0 == parent.$('.info_dialog').size()) {
            parent.$('#div_wrapper').hide();
        }
    } else {
        parent.$('.dialog, .info_dialog').fadeOut(function() {
            if (0 == $('.info_dialog').size()) {
                parent.$('#div_wrapper').hide();
            }
        });
    }
...
function browse_pop_cancel() {
    ...
    $('#share_directory_input'+choice[check_browse_num]).attr("value",'/');
    g_main_displayingPromptStack.pop();
}

$(".button_dms_add_list").live("click", function(){
    var addLine = "";
    var addIndex = $(".button_dms_add_list").index(this);
    if(!isButtonEnable("button_add"+choice[addIndex])){
        return;
    }
    addLine += "<tr height='35' class=\'user_dms_add_line\'>"+
                   "<td width='5%'>"+
                       "<input name='share_directory' id='share_directory" + (choice[choice.length-1]+1) + "' type='checkbox' checked />"+
                   "<td width='56%'>"+
                       "<input id='share_directory_input" + (choice[choice.length-1]+1) + "' name='share_directory_input"+ (choice[choice.length-1]+1) +"' type='text' value='/'  class=\'dms_input\' readonly size='20'/>"+
                   "</td>"+
                   "<td width='15%'>"+
                       "<span class=\'button_dms_browse_list\' id='button_browse" + (choice[choice.length-1]+1) + "'><span class='button_left'><span class='button_right'><span class='button_center'>" + common_browse + "</span></span></span></span>"+
                   "</td>"+
                   "<td width='14%'>"+
                       "<span class=\'button_dms_delete_list\' id='button_delete" + (choice[choice.length-1]+1) + "'><span class='button_left'><span class='button_right'><span class='button_center'>" + common_delete + "</span></span></span></span>"+
                   "</td>"+
                   "<td width='10%'>"+
                       "<span class=\'button_dms_add_list\' id='button_add" + (choice[choice.length-1]+1) + "'><span class='button_left'><span class='button_right'><span class='button_center'>" + common_add + "</span></span></span></span>"+
                   "</td>"+
               "</tr>";
    choice.push(choice[choice.length-1]+1);
    $('#share_directory_list').append(addLine);
    $("#button_add"+choice[addIndex]).hide();
    if(2 == choice.length){
        button_enable('button_delete'+choice[0], '1');
    }
...
$(".dms_checkbox_list").live("click", function(){
    button_enable('apply', '1');
});
