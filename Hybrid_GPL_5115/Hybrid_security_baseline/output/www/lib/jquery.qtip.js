...
(function ($) {
	...
	function bgiframe() {
		...
		// html = '<iframe class="qtip-bgiframe" frameborder="0" tabindex="-1" src="javascript:false" ' + 'style="display:block; position:absolute; z-index:-1; filter:alpha(opacity=\'0\'); border: 1px solid red; ' + 'height:' + dimensions.height + 'px; width:' + dimensions.width + 'px" />'; modify by likun lkf44009 for z-index = -1
		html = '<iframe class="qtip-bgiframe" frameborder="0" tabindex="-1" src="javascript:false" ' + 'style="display:block; position:absolute; z-index:15000; filter:alpha(opacity=\'0\'); border: 1px solid red; ' + 'height:' + dimensions.height + 'px; width:' + dimensions.width + 'px" />';
	...
	function QTip(target, options, id) {
		...
		$.extend(self, self.options.api, {
			show: function (event) {
				...
				}else if(self.options.show.effect.type===undefined){
					self.elements.tooltip.show(null, afterShow);
				}
				else {
					alert(self.options.show.effect.type)
					...
					$('div.qtip[id^="qtip"]').not(self.elements.tooltip).each(function () {
						if($(this).qtip('api').status.rendered === true) {
							...
							if(typeof elemIndex === 'number' && elemIndex > -1) {
								// $(this).css({ zIndex: parseInt($(this).css('z-index'), 10) - 1 }); modify by likun lkf44009 for z-index = -1
   ...
   $.fn.qtip.defaults = {
      // Content
      content: {
         prerender: true, // preender the qtip object
         text: false,
         url: false,
         data: null,
         title: {
   ...
   $.fn.qtip.styles = {
	  ...
	  yellow: {
		 ...
		 width: {min: 250, max: 250},
		 background: '#ffffe7',
		 tip: true,
		 classes: { tooltip: 'qtip-yellow' }
	  }
   };
})(jQuery);