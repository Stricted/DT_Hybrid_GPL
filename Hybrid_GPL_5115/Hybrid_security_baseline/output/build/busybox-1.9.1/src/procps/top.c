...
static NOINLINE void display_process_list(int count, int scr_width)
{
	...
	/*start this is show physical RAM for system status*/
#ifdef SUPPORT_ATP_PROCESS_STATUS
		"  PID  PPID USER     STAT   VSS %MEM %CPU COMMAND");
#else
		"  PID  PPID USER     STAT   VSZ %MEM %CPU COMMAND");
#endif
	/*end this is show physical RAM for system status*/
#else
...
/*end add process status*/
		...
		while ((p = procps_scan(p, scan_mask)) != NULL) {
			...
			if (scan_mask == TOP_MASK) {
				...
				top[n].pid = p->pid;
				top[n].ppid = p->ppid;
				/*start this is show physical RAM*/
#ifdef SUPPORT_ATP_PROCESS_STATUS
				top[n].vsz = p->rss;
#else
				top[n].vsz = p->vsz;
#endif
				/*end z228368 this is show physical RAM*/
#if ENABLE_FEATURE_TOP_CPU_USAGE_PERCENTAGE
