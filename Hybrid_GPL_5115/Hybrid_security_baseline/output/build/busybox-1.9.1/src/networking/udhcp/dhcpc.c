...
/* ?·??2aê? add by wangpu begin */
static  unsigned int g_discovercount = 0;
char    g_acCmd[255];
char    g_interface[16];
/* ?·??2aê? add by wangpu end */
		...
		if (retval == 0) {
			...
			case INIT_SELECTING:
				if (packet_num < discover_retries) {
					...
					packet_num++;
                    /* 环回测试 add by wangpu begin */
                    g_discovercount ++;
                    printf("Circle test: send & recv packet num:[%d]\n",g_discovercount);
					memset(g_acCmd,0,sizeof(g_acCmd));
					memset(g_interface,0,sizeof(g_interface));
					sprintf(g_interface,"%s",client_config.interface);
			        sprintf(g_acCmd,"echo %d > /var/circle/%s",g_discovercount-packet_num+1,g_interface);
		            system(g_acCmd);
					/* 环回测试 add by wangpu end */
					continue;
				}
				...
				if (*message == DHCPACK) {
					...
					if (client_config.quit_after_lease) {
						...
						/* 环回测试 add by wangpu begin */
                            ...
                            //printf("Circle test: send & recv packet num:[%d]\n",g_discovercount);
							//sprintf(g_interface,"%s",client_config.interface);
			                //sprintf(g_acCmd,"echo %d > /var/client/%s",g_offercount,g_interface);
		                    //system(g_acCmd);
						    state = INIT_SELECTING;
