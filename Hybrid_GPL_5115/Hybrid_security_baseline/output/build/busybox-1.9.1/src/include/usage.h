...
#define date_full_usage \
	...
	USE_FEATURE_DATE_ISOFMT( \
       ...
       "	-l logstr	Write logstr with timestamps to /var/atp_boot_track.log\n" \
	) \
...
/*Start of Maintain 2008-3-1 17:11 for 修改Busybox中的传输工具，满足cwmp传输需求 by z65940*/
#if ENABLE_FTPGET
...
#endif
/*End of Maintain 2008-3-1 17:11 for 修改Busybox中的传输工具，满足cwmp传输需求 by z65940*/
...
#define httpdiag_trivial_usage \
       "[OPTION]... HOST"
#define httpdiag_full_usage \
       "retrieve a file using HTTP or FTP in the tr143 DownloadDiagnostics/UpLoadDiagnostics\n\n" \
       "Options:\n" \
       "	-g 		Download\n" \
       "	-s            Upload\n" \
...
#define traceroute6_trivial_usage \
       "traceroute6 [-dnrvV] [-m max_ttl] [-p port#] [-q nqueries]\n" \
...
#define vconfig_full_usage \
       ...
       "	nfmark_to_priority	[vlan-name] [vlan_qos]\n"
#define smuxctl_trivial_usage \
       "COMMAND [OPTIONS]..."
#define smuxctl_full_usage \
       "Create and remove virtual overlay devices." \
       "\n\nOptions:\n" \
       "	add		[Lower Layer interface-name] [Overlay interface-name]\n" \
       "	rem		[Overlay Interface]" 
...
#define ifenslave_trivial_usage \
       "-[f|d|c] <master-if> <slave-if> [<slave-if>]"
...
#define transtool_option    \
    ...
    USE_GETOPT_LONG( \
       ...
       "\t-l, --local            Local file path\n" \
       "\t-r, --remote           Remote file path\n" \
       "\t-P, --port             Port to be used, optional\n" \
       "\t-B, --bind             Bind local ip, optional\n" \
       "\t-A, --addr             Remote resolved ip, optional\n" \
       "\t-b, --begin            Transfer start position\n" \
       "\t-e, --len              Transfer length\n" \
       "\t-m, --max              Max transfer size\n" \
       "\t-c, --compress         Compress downloaded file" \
	) \
	SKIP_GETOPT_LONG( \
	   "\t-g    Download\n" \
       "\t-s    Upload\n" \
...
#define ftpput_trivial_usage ftpget_trivial_usage
#define ftpput_full_usage ftpget_full_usage
#endif
...
#if ENABLE_ATP_MCAST
#define mcast_trivial_usage \
	    "[OPTION]... HOST"
#define mcast_full_usage \
	    "mcast download a file through multicast channel\n\n" \
	    transtool_option
#endif

#define atpping_trivial_usage \
	    "[OPTION]... HOST"
#define atpping_full_usage \
	    "ping a address via HTTP\n\n" \
	    transtool_option
