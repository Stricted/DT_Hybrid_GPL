...
struct ebt_u_replace
{
	...
	char command;
	/*
	 * here we stick the hook to do our thing on (can be -1 if unspecified) 对应的链
	 */
	int selected_hook;
...
struct ebt_u_entry
{
	...
	unsigned int invflags;      /* 是否取反标记 */
	uint16_t ethproto;
	char in[IFNAMSIZ];          /* 进来的接口 */
	char logical_in[IFNAMSIZ];  //
	char out[IFNAMSIZ];         /* 出去的接口 */
	char logical_out[IFNAMSIZ];
	unsigned char sourcemac[ETH_ALEN];  /* 源MAC */
	unsigned char sourcemsk[ETH_ALEN];  /* 源MAC掩码 */
	unsigned char destmac[ETH_ALEN];    /* 目的MAC */
	unsigned char destmsk[ETH_ALEN];    /* 目的MAC掩码 */
	struct ebt_u_match_list *m_list;    /* 匹配列表 */
	struct ebt_u_watcher_list *w_list;
	struct ebt_entry_target *t;         /* 操作，注意一条规则肯定只会有一个动作 */
	struct ebt_u_entry *next;           /* 下一条规则 */
};
...
{
	char name[EBT_FUNCTION_MAXNAMELEN];
	unsigned int size;                      /* 表示本target自己的数据扣除了ebt_entry_target */
	void (*help)(void);
