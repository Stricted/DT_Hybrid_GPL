...
/* Start of by d00107688 2010-07-31 红区使用DHCP Relay的时候会导致广播报文无法发送到LAN侧 */
#ifdef CONFIG_SUPPORT_ATP
    if (sk 
        && (AF_INET == sk->sk_family)
        && (htonl(INADDR_BROADCAST) == fl->fl4_dst))
    {
        return 0;
    }
#endif
/* End of by d00107688 2010-07-31 红区使用DHCP Relay的时候会导致广播报文无法发送到LAN侧 */
