...
static enum hrtimer_restart xfrm_timer_handler(struct hrtimer * me)
{
	...
	spin_lock(&x->lock);
	/* start of by d00107688 当SA idle之后尽早删除内核的SA，这样做可能存在风险，考虑
...
#endif
/* End of by d00107688 当SA idle之后尽早删除内核的SA */
...
{
/* start of DTS2010071201494  by d00107688 2010-07-15 Homehub B018问题，Blackdns配置为IP方式，配置idletime时长为60s，实际测试超时时长在45-48秒，不准确  */
#if 0
    ...
    //if (!x->curlft.use_time)
        x->curlft.use_time = get_seconds();
#endif
/* end of DTS2010071201494  by d00107688 2010-07-15 Homehub B018问题，Blackdns配置为IP方式，配置idletime时长为60s，实际测试超时时长在45-48秒，不准确  */
...
#if 0
// TODO: 待优化
static void xfrm_state_idle_clear(struct xfrm_state *x)
{
	int i;
    int iFlag = 0;
	unsigned long now = get_seconds();
    ...
    for (i = 0; i < 10; i++)
    {
        ...
        if (pstOther && (pstOther->lft.idle_time_seconds)
                && (20 <= (now - pstOther->curlft.use_time)) 
...
static void xfrm_idle_timer_handler(unsigned long data)
{
	...
	/* start of by d00107688 2010-08-25 如果该SA已经idle timeout了，那么下次idle timeout时删除 */
    if (x->ucFlag & IDLE_TIMEOUT_FLAG)
    {
        xfrm_state_delete(x);
        return ;
    }
	/* end of by d00107688 2010-08-25 如果该SA已经 idle timeout了，那么下次timeout就删除 */
...
/* start of by d00107688 当SA idle之后尽早删除内核的SA */
    ulResetTime = idle >> 1;
/* End of by d00107688 当SA idle之后尽早删除内核的SA */
...
/* end of DTS2010071201494  by d00107688 2010-07-15 Homehub B018问题，Blackdns配置为IP方式，配置idletime时长为60s，实际测试超时时长在45-48秒，不准确  */
	...
	/* start of by d00107688 如果该SA idle timeout了，那么下次idle timeout就删除 */
    x->ucFlag |= IDLE_TIMEOUT_FLAG;
    pstOther->ucFlag |= IDLE_TIMEOUT_FLAG;
	/* end of by d00107688 如果该SA idle timeout了，那么下次idle timeout就删除之 */
