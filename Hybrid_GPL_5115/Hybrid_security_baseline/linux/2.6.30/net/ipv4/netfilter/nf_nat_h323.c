...
/* start add by d00107688 for bug fix */
void nf_nat_follow(struct nf_conn *ct,
			  struct nf_conntrack_expect *exp)
{
    ...
    pr_debug("now, nat for: src[%u.%u.%u.%u:%d] ----> dst[%u.%u.%u.%u: %d]",
    ...
    if (is_local_addr(exp->tuple.src.u3))
    {
        pr_debug("no need do nat");
        return ;
    }
...
/* end of by d00107688 for bug fix */ 
/****************************************************************************/
#if 0
...
static int nat_rtp_rtcp(struct sk_buff *skb, struct nf_conn *ct,
			...
			__be16 port, __be16 rtp_port,
			struct nf_conntrack_expect *rtp_exp,
			struct nf_conntrack_expect *rtcp_exp, unsigned int mediatype)
#endif
{
    ...
    #else
	rtp_exp->expectfn = nf_nat_follow;
    #endif
	rtp_exp->dir = !dir;
    ...
    #else
	rtcp_exp->expectfn = nf_nat_follow;
    #endif
	rtcp_exp->dir = !dir;
	...
	if (i >= H323_RTP_CHANNEL_MAX) {
        pr_debug("nf_nat_h323: out of expectations\n");
	...
	if (set_h245_addr(skb, data, dataoff, taddr,
			  ...
			  htons((port & htons(1)) ? nated_port + 1 :
						    nated_port)) == 0) {
    #else
	if (MEDIA_CONTROL_CHANNEL == mediatype)
	{
		nated_port += 1;
	}
	if (set_h245_addr(skb, data, dataoff, taddr,
...
#else
	if (!is_local_addr(new->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u3))
	{
		/* For DST manip, map port here to where it's expected. */
		range.flags = (IP_NAT_RANGE_MAP_IPS | IP_NAT_RANGE_PROTO_SPECIFIED);
		...
		pr_debug("now, nat for: src[%u.%u.%u.%u:%d] ----> dst[%u.%u.%u.%u: %d] to src[%u.%u.%u.%u:%d]",
			NIPQUAD(new->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u3.ip), 
			(new->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u.udp.port),
			NIPQUAD(new->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u3.ip), 
			(new->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.udp.port),
			NIPQUAD(range.min_ip),
