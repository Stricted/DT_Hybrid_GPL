...
*/
static int  mac_init(struct mac_head*, int max_macs, char srcaddr, 
    char usemac, char usemasq, char usefw, void* proxyremap);
...
struct mac_addr {
  ...
  /*start modifying for vdf qos by p44980 2008.01.16*/
  int mask;                   // mask 
 /*end modifying for vdf qos by p44980 2008.01.16*/
};
...
/* HG586V100R001 c40163 2007-8-14 BT modify end */ 
  ...
  /* HG586V100R001 c40163 2007-8-14 BT add begin */
  memset((h->macs), 0, (sizeof(struct mac_addr) * max_macs));
  if (usefw) {
  	int iInd;
  	for (iInd = 0; iInd < max_macs; iInd++) {
  		h->macs[iInd].class = 0xffffffff;
  	}
  }
  /* HG586V100R001 c40163 2007-8-14 BT add end */ 
	return 0;
...
static int mac_classify(struct mac_head *head, struct sk_buff *skb)
{
  ...
  if (head->usefw) {
	...
	/* Start of modified by f00120964 for qos wrr function 2012-1-12 */
    /*start add by l00184769:to keep the kernel and cms of qos consistent*/
    //unsigned int uMark = (skb->mark & 0xff);
    /*end add by l00184769:to keep the kernel and cms of qos consistent*/
	unsigned int uMark = (skb->mark & 0xfff);
    uMark = uMark >> 8;
    /* End of modified by f00120964 for qos wrr function 2012-1-12 */
#else
	unsigned int uMark = 0xffffffff;
#endif
	for (iInd = 0; iInd < head->mac_max; iInd++) {
		if (head->macs[iInd].class == (uMark & head->macs[iInd].mask)) {
			//printk("wrr used fw, hit! iInd=0x%x, Mark=0x%x,  mask=0x%x\n", iInd, uMark, head->macs[iInd].mask);
			return iInd;
		}
	}
...
static int wrr_init(struct Qdisc *sch, struct nlattr *opt)
{
  ...
  */
  maciniterr=mac_init(&q->filter,qopt->bands_max,qopt->srcaddr,
      qopt->usemac,qopt->usemasq,qopt->usefw,q->proxydict);
  /* HG586V100R001 c40163 2007-8-14 BT modify end */ 
...
static struct sk_buff *wrr_dequeue(struct Qdisc *sch)
{
    ...
    /*start of w00104696 ADD for DTS2010071700974: WRR ShapingRate is invalid 20101027*/
	/* No packet - so machine should be removed from heap */
    if (!b->que->q.qlen)
	    heap_remove(&q->h, band);
    /*end of w00104696 ADD for DTS2010071700974: WRR ShapingRate is invalid 20101027*/
...
static int wrr_tune_std(struct Qdisc *sch, struct nlattr *opt)
{
	...
	if (qopt->change_class) {
  	/* HG586V100R001 c40163 2007-8-21 BT modify begin */
  	int idx;
	if (q->filter.usefw) {
		idx = qopt->addr[0] - 1;
		if ((idx < 0) || (idx >= q->filter.mac_max)) {
			idx = 0;
		}
		q->filter.macs[idx].class = qopt->addr[1];
		q->filter.macs[idx].mask = qopt->addr[2];
		//printk("idx=0x%x, class=0x%x, mark=0x%x\n", idx, q->filter.macs[idx].class, q->filter.macs[idx].mask);
		q->filter.mac_cur = q->filter.mac_max;
	}
	else {
	    idx=lookup_mac(&q->filter,qopt->addr);
	}
  	/* HG586V100R001 c40163 2007-8-21 BT modify end */
		weight_setvalue
