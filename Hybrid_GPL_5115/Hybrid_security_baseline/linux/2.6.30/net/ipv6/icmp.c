...
static int icmpv6_rcv(struct sk_buff *skb)
{
    ...
    { 
        ...
        stCheckSum.usLength = ntohs(ipv6hdr->payload_len);
        memcpy(&stCheckSum.sin6_saddr,saddr,sizeof(struct in6_addr));
        memcpy(&stCheckSum.sin6_daddr,daddr,sizeof(struct in6_addr));
        memcpy(&stCheckSum.uszTemBuff,(unsigned char *)hdr,stCheckSum.usLength);
