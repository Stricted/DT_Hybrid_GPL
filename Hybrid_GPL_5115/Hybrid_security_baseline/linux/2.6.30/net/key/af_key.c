   ...
   not true.
...
#endif
       // COLOR_DEBUG("user: %llu hard_byte_limit: %llu\r\n", 
	...
	if (lifetime != NULL) {
        ...
        x->lft.soft_byte_limit 
            == XFRM_INF? XFRM_INF:(lifetime->sadb_lifetime_bytes * 1024);
#endif
       // COLOR_DEBUG("user: %llu soft_byte: %llu\r\n", 
    ...
    if (lifetime != NULL) {
        //x->lft.idle_time_seconds = _KEY2X(lifetime->sadb_lifetime_allocations);
        //x->lft.idle_time_seconds = _KEY2X(lifetime->sadb_lifetime_bytes);
        //x->lft.idle_time_seconds = lifetime->sadb_lifetime_addtime;
...
static int pfkey_acquire(struct sock *sk, struct sk_buff *skb, struct sadb_msg *hdr, void **ext_hdrs)
{
	...
	if (x->km.state == XFRM_STATE_ACQ) {
	 /* start of by d00107688 for DTS2010071202630  Blackdns 将IP地址加入黑名单后周期不准确  */     
#ifdef CONFIG_SUPPORT_ATP
...
#endif
/* End of by d00107688 for DTS2010071202630  Blackdns 将IP地址加入黑名单后周期不准确  */       
	}
