...
#define DYNTOS_PROC_TRANSTBL_FILENAME "nf_dtos_w2ldscp_transtbl"
			...
			int (*okfn)(struct sk_buff *))
{
	 ...
	 if(ct->dyntos.status == DYNTOS_INHERITED) {
			...
			ipv4_change_dsfield(ip_hdr(skb), (__u8)(~DYNTOS_DSCP_MASK),
         ...
         /*TODO: should we skip broadcast packets ?? */
			...
			pktDscp = ipv4_get_dsfield(ip_hdr(skb)) >> DYNTOS_DSCP_SHIFT;
...
{
	 dyntos_proc_file = create_proc_entry(DYNTOS_PROC_TRANSTBL_FILENAME, 0644, proc_net_netfilter);
...
{
	 remove_proc_entry(DYNTOS_PROC_TRANSTBL_FILENAME, proc_net_netfilter);
