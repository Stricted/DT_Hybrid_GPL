...
extern __be16	g_sH323H245Port;
#endif

#if defined(CONFIG_SYSCTL) && defined(CONFIG_NF_CONNTRACK_H323) && defined(CONFIG_NF_CONNTRACK_PROC_COMPAT)
extern int alg_h323_enable;
#endif
/* Hooks for NAT */
...
/* Start of DTS2010101804489 by d00107688 2010-10-23 H323通话一段时间没有图像显示 */
	nf_ct_expect_init(rtp_exp, NF_CT_EXPECT_CLASS_DEFAULT, nf_ct_l3num(ct),
			  NULL, //&ct->tuplehash[!dir].tuple.src.u3,
			  &ct->tuplehash[!dir].tuple.dst.u3,
			  IPPROTO_UDP, NULL, &rtp_port);
/* End of DTS2010101804489 by d00107688 2010-10-23 H323通话一段时间没有图像显示 */

    pr_debug("create rtp exp src:[%u.%u.%u.%u], dst[%u.%u.%u.%u] addr: [%u.%u.%u.%u] dstport: %d",
        NIPQUAD(ct->tuplehash[!dir].tuple.src.u3.ip), 
        NIPQUAD(ct->tuplehash[!dir].tuple.dst.u3.ip),
        NIPQUAD(addr.ip),
        port);
...
/* End of DTS2010101804489 by d00107688 2010-10-23 H323通话一段时间没有图像显示 */
    ...
    pr_debug("create rtcp exp src:[%u.%u.%u.%u], dst[%u.%u.%u.%u] addr: [%u.%u.%u.%u] dstport: %d",
        NIPQUAD(ct->tuplehash[!dir].tuple.src.u3.ip), 
        NIPQUAD(ct->tuplehash[!dir].tuple.dst.u3.ip),
        NIPQUAD(addr.ip), 
        rtcp_port);
				...
				H2250LogicalChannelParameters *channel)
{
	...
	if (channel->
		...
		pr_debug("process rtcp media control channel");
		/* RTCP */
#if 0
		ret = expect_rtp_rtcp(skb, ct, ctinfo, data, dataoff,
		       ...
		       TransportAddress *taddr)
{
    ...
    #endif
	{
        pr_debug("Get address [%u.%u.%u.%u] tuplehash src[%u.%u.%u.%u]",
    ...
    pr_debug("create exp src:[%u.%u.%u.%u], dst[%u.%u.%u.%u] dstport: %d, dir: %0x proto: %0x",
			 ...
			 Setup_UUIE *setup)
{
	...
	if (setup->options & eSetup_UUIE_fastStart) {
        pr_debug("Now, begin to process fast start message");
...
static int tcpmss_set_packet(struct sk_buff *skb,
		     ...
		     unsigned int minlen)
{
	...
	if (tcplen != tcph->doff*4) {
			pr_debug("xt_TCPMSS: bad length (%u bytes)\n",
			       skb->len);
		return -1;
	}
...
static int tcpmss_modify_packet(struct sk_buff *skb)
{
	struct iphdr *iph = ip_hdr(skb);
	__be16 newlen;
	int ret;
    
	ret = tcpmss_set_packet(skb, (iph->ihl * 4), 1460,
