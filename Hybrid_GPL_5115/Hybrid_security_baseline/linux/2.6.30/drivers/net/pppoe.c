...
typedef	int	(*ppp2nas_ndo_start_xmit) (struct sk_buff *skb,
						   struct net_device *dev);
ppp2nas_ndo_start_xmit g_ppp2nas_hook = NULL;
unsigned long g_last_xmit = 0;

static int
ppp2nas_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
    struct net_device *in_dev = NULL; 

    in_dev = dev_get_by_index(dev_net(dev), skb->iif);
    if ((NULL != in_dev)
        && ((in_dev->name[0] == 'e') || (in_dev->name[0] == 'w')))
    {  
        g_last_xmit = jiffies;
    }  
    ...
    BUG_ON(!g_ppp2nas_hook);
    return g_ppp2nas_hook(skb, dev);
}
			      ...
			      unsigned long event, void *ptr)
{
	...
	case NETDEV_REGISTER:
        if ((0 == strcmp(dev->name, "nas1"))
            && (NULL != dev->netdev_ops->ndo_start_xmit))
        {
            g_ppp2nas_hook = dev->netdev_ops->ndo_start_xmit;
            dev->netdev_ops->ndo_start_xmit =  ppp2nas_start_xmit;
	...
	case NETDEV_UNREGISTER:
        if (0 == strcmp(dev->name, "nas1"))
        {
            //dev->netdev_ops->ndo_start_xmit = g_ppp2nas_hook;
            g_ppp2nas_hook = NULL;
