...
DTS2013051005443	2013/05/28  z00185914     【拨号】单板使用DDNS服务时，有线拨号保持按需拨号不断开；无线拨号，开启SNTP/DDNS，不促发按需拨号
DTS2013070601078    2013/07/06  z00159386      70V voip特性合入B890主干
================================================================*/
...
#define IP4_HDR     (0x45)
#define IP6_HDR     (0x60)
#define SIOCSUSBNET_NEXT_HWADDR  (0x89F1)
/* DTS2013011003247   x81004082 2013/1/10 end> */
//////////////////////////////////////////////////////////////////////////////////////////////
...
/* <DTS2013051005443 z00185914 2013/05/28 BEGIN */
static int g_lan_data = 0;
#define DNS_LAN_MARK 0x200000
/* DTS2013051005443  z00185914 2013/05/28 END> */
...
/* zhangtai */
       ...
       set_rmnet_intr_ep(&(status->desc));
       set_rmnet_device(dev->udev);
/* <DTS2013011408161 move urb request to ndistty z00185914 20130125 BEGIN */
	   set_hwcdc_device(dev);
/* DTS2013011408161 move urb request to ndistty z00185914 20130125 END> */
#endif
...
/* DTS2013011408161 move urb request to ndistty z00185914 20130125 DELETE 7 lines */
        }
    }
    return 0;
}
...
/* < DTS2013011706280 improve NDIS dial up slowly z00185914 20130119 BEGIN */
	...
	int i = 0;
#endif
	unsigned char	source_addr[ETH_ALEN] = {0xf0,0x7d,0x68,0x15,0xa2,0xa2};/* <DTS2013092206920 xiazhongyue 20131016 modify  > */
/* DTS2013011706280 improve NDIS dial up slowly z00185914 20130119 END> */
    ...
    /* <DTS2013092206920 xiazhongyue 20130922 begin */
    struct ethhdr eth;
    ...
    if (IP6_HDR == (0xF0 & skb->data[sizeof(struct ethhdr)]))
    {
        ...
        {        
            ...
            memcpy(eth.h_dest, dev->peer_addr, ETH_ALEN);
            memcpy(eth.h_source, source_addr, ETH_ALEN);/* <DTS2013092206920 xiazhongyue 20131016 modify  > */
            memcpy(skb->data, (unsigned char *)&eth, sizeof(struct ethhdr));
        }
    } 
	/* DTS2013092206920 xiazhongyue 20130922 end  > */
...
static int hw_open (struct net_device *net)
{
    ...
    /* <DTS2013020502561   x81004082 2013/2/5 begin*/
    /* 删除无效代码和有bug的代码，共删除18行 */
    /* DTS2013020502561   x81004082 2013/2/5 end> */
...
/* <DTS2013092206920 xiazhongyue 20130922 begin */
int hw_usbnet_ioctl(struct net_device *net, struct ifreq *rq, int cmd)
{
    struct hw_cdc_net		*dev = netdev_priv(net);
    ...
    switch (cmd)
    {
    case SIOCSUSBNET_NEXT_HWADDR:
        memset(dev->peer_addr,0,ETH_ALEN);
...
/* DTS2013092206920 xiazhongyue 20130922 end  > */
/* <DTS2013051005443 z00185914 2013/05/28 BEGIN */
int is_lan_data(struct sk_buff *skb, struct hw_cdc_net *dev)
{
    ...
    if (NULL != in_dev)
    {
        if (((in_dev->name[0] == 'e') || (in_dev->name[0] == 'r')))
        {  
             is_lan_data = 1;
        }  
	...
	else
	{
        if ((NULL != skb->lanindev)
            && ((skb->lanindev->name[0] == 'e') || (skb->lanindev->name[0] == 'r')))
        {
            is_lan_data = 1;
        }            
...
/* DTS2013011706280 improve NDIS dial up slowly z00185914 20130119 END> */
    ...
    if (dev->is_ncm ) {
		...
		if (skb)
		{
			g_lan_data = is_lan_data(skb,dev);
		}
		/* DTS2013051005443  z00185914 2013/05/28 END> */
...
static const struct net_device_ops hw_netdev_ops = {
    ...
    .ndo_do_ioctl = hw_usbnet_ioctl,/* <DTS2013092206920 xiazhongyue 20130922 modify  > */
};
#endif
...
#define SD5115H_VOIP_QOS
#if defined(SD5115H_VOIP_QOS)
#define VOIP_MARK  0x0000000AUL
static inline int check_voip_skb(struct sk_buff *skb)
{
    if (VOIP_MARK == (VOIP_MARK&skb->mark))
    {
        return 1;
    }
...
static inline struct sk_buff *ncm_tx_voip_fixup(struct ncm_ctx *ctx, struct sk_buff *skb)
{
    ...
    err = ntb_init(ctx, &voip_ntb, ctx->tx_max_ntb);
    if (unlikely(err < 0)) 
    {
        return NULL;
    }

    voip_ntb.skb = ncm_get_skb(ctx);
    if (unlikely(voip_ntb.skb == NULL)) 
    {
        return NULL;
    }

    err = ntb_add_dgram(ctx, &voip_ntb, skb->len, skb->data, GFP_ATOMIC);
    ...
    skb2 = ntb_finalize(ctx, &voip_ntb);  
    return skb2;
}

#endif /* SD5115H_VOIP_QOS */
...
/* <DTS2013070601078  z00159386  20130706  begin */
#if defined(SD5115H_VOIP_QOS)
    if (check_voip_skb(skb))
    {
        /* 检测到VOIP ，直接按照格式发送单个数据包*/
        skb2 = ncm_tx_voip_fixup(ctx, skb); /* TODO */
        if (NULL != skb2) 
        {
            dev_kfree_skb_any(skb);
            dev->stats.tx_packets++;
...
//#endif
    ...
    g_usbnet->do_ioctl = hw_usbnet_ioctl;/* <DTS2013092206920 xiazhongyue 20130922 modify  > */
...
static const struct usb_device_id    hw_products [] = {
    ...
    {
       USB_DEVICE_MATCH_HUAWEI(5,0x12d1),	//zjq
...
static int __init hw_cdc_init(void)
{
    ...
    g_usbnet = alloc_netdev(sizeof(struct hw_cdc_net), "rmnet%d", ether_setup);
    if (!g_usbnet) {
