...
static unsigned int pcie0_sel = CONFIG_PCIE0_SEL;
...
static unsigned int pcie0_mem_space_size = CONFIG_PCIE0_DEVICES_MEM_SIZE;
...
static int __init pcie0_mem_parser(char* str)
{
	...
	pcie0_iatu_table[2].lbar = 0x40000000 + pcie0_mem_space_size;
	return 1;
}
...
static int __init pcie_init(void)
{
	if (pcie0_sel > pcie0_x1_sel
			...
			|| CONFIG_PCIE0_DEVICES_CONFIG_SIZE > 0x10000000
			|| CONFIG_PCIE0_DEVICES_CONFIG_SIZE < 0) {
		printk(KERN_ERR "Invalid parameter pcie0_sel(%d), "\
				...
				CONFIG_PCIE0_DEVICES_CONFIG_SIZE);
		return -EINVAL;
	}
	...
	printk("pcie0 sel: 0x%x, pcie0 memsize:0x%x. "\
			"pcie0_iatu_table[1].lbar=0x%x\n",
			pcie0_sel, pcie0_mem_space_size,
			pcie0_iatu_table[1].lbar);
	...
	if (pcie0_sel == pcie0_x1_sel) {
		...
		if (pcie0_work_mode == pcie_wm_rc) {
			...
			pcie_info[pcie_controllers_nr].conf_base_addr =
				(unsigned int)ioremap_nocache(DBI_BASE_ADDR_0,
						0x1000);
			...
			pcie_info[pcie_controllers_nr].base_addr =
				(unsigned int)ioremap_nocache(PCIE0_MEMIO_BASE,
					CONFIG_PCIE0_DEVICES_CONFIG_SIZE);
