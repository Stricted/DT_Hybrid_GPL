...
#ifdef CONFIG_CRASH_DUMPFILE	
	snprintf(g_acWriteCrashBuf,2048, "%sModules linked in:\n",g_acWriteCrashBuf);
#endif
...
#ifdef CONFIG_CRASH_DUMPFILE		
		snprintf(g_acWriteCrashBuf,2048, "%s %s%s",g_acWriteCrashBuf, mod->name, module_flags(mod, buf));
#endif
#if defined (CONFIG_MIPS_BRCM)
...
#ifdef CONFIG_CRASH_DUMPFILE
		snprintf(g_acWriteCrashBuf,2048, "%s init_addr(%p - %p), core_addr(%p - %p)\n",g_acWriteCrashBuf, 
			mod->module_init,
			mod->module_init+mod->init_text_size,
			mod->module_core, 
			mod->module_core+mod->core_text_size);
...
#ifdef CONFIG_CRASH_DUMPFILE		
		snprintf(g_acWriteCrashBuf,2048, "%s [last unloaded: %s]",g_acWriteCrashBuf, last_unloaded_module);
#endif
	}
	printk("\n");
#ifdef CONFIG_CRASH_DUMPFILE	
