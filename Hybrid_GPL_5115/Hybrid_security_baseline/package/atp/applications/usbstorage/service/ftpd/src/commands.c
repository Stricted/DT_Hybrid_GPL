...
int bftpd_check_login()
{
    ...
    {
        ulAllowWan = ATP_LoginCtlCheckFtpAcc(user,ATP_FTPTYPE_FTPSEXPLICIT,isAccFromlan);
        ulisFtpsExCtl = VOS_FALSE;
    }
    else 
    {
        ulAllowWan = ATP_LoginCtlCheckFtpAcc(user,ATP_FTPTYPE_FTPNORMAL,isAccFromlan);
    }
    if(VOS_OK != ulAllowWan)
    {
        control_printf(SL_FAILURE , "530 The user is not allowed to access from %s via %s.",isAccFromlan?"lan":"wan",ulisFtpsExCtl?"ftps":"ftp");
        return -1;
    }
    if(!strcasecmp(user,"anonymous")) //add by y00181549 for ftp anonymous禁止登陆
    {
        control_printf(SL_FAILURE , "530 The user is forbidden.");
        return -1;
    }
#else
    if(strcasecmp(user,"anonymous"))
    {
        ulAllowWan = ATP_LoginCtlAllowWanAcc(user, ATP_LOGINTYPE_FTP);
    }
    ...
    if( iSocketFd > 0 )
    {
        ...
        if( AF_INET == saLocal.sa_family )
        {
            ...
            if( ATP_FTPSERVERCMS_ACL_PORT_OPERATE_OPEN == enPortOperate )
            {
                ...
                {
                    /* < DTS2012082702277: replaced by zhaiqi.z00174311 2012-08-30 begin */
                    ATP_BFTPD_CHILD_SendRecv_Deal_ACL_Msg( ATP_FTPSERVERCMS_ACL_PORT_OPERATE_CLOSE, eLastOpenIPType, ntohs((unsigned short)iLastOpenDataPort) );
                    /* DTS2012082702277: replaced by zhaiqi.z00174311 2012-08-30 end> */
                }
            ...
            /* < DTS2012082702277: replaced by zhaiqi.z00174311 2012-08-30 begin */
            ATP_BFTPD_CHILD_SendRecv_Deal_ACL_Msg( enPortOperate, ATP_FTPSERVERCMS_IPTYPE_IPV4, ntohs(sa4Local.sin_port) );
            /* DTS2012082702277: replaced by zhaiqi.z00174311 2012-08-30 end> */
        ...
        else if( AF_INET6 == saLocal.sa_family )
        {
            ...
            /* < DTS2012082702277: replaced by zhaiqi.z00174311 2012-08-30 begin */
            ATP_BFTPD_CHILD_SendRecv_Deal_ACL_Msg( enPortOperate, ATP_FTPSERVERCMS_IPTYPE_IPV6, ntohs(sa6Local.sin6_port) );
            /* DTS2012082702277: replaced by zhaiqi.z00174311 2012-08-30 end> */
