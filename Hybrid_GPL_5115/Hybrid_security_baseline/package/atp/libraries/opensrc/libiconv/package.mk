...
define ICONV_INSTALL_HEADER_CMDS
	$(Q)if [ ! -d $(ICONV_AFTER_EXTR_SRC) ]; then \
	mkdir -p $(ICONV_AFTER_EXTR_SRC); \
	cd $(ICONV_AFTER_EXTR_SRC); \
	$(ICONV_TAR_EXTRACT_OPT) $(ICONV_TAR_SRC); \
	fi
	install $(ICONV_AFTER_EXTR_SRC)/src/include/iconv.h $(STAGING_DIR)/usr/include; \
	install $(ICONV_AFTER_EXTR_SRC)/src/include/iconv.h $(HOST_DIR)/usr/include;
endef
...
define ICONV_BUILD_CMDS
	...
	install $(SERVICE_ICONV_SRC_DIR)/include/iconv.h $(STAGING_DIR)/usr/include; \
	install $(SERVICE_ICONV_SRC_DIR)/include/iconv.h $(HOST_DIR)/usr/include;
endef
