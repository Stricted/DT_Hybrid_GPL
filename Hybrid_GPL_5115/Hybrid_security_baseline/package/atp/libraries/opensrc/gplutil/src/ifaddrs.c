		   ...
		   void *p, size_t len, uint32_t scope, uint32_t scopeid)
{
  ...
  switch (family)
    ...
    case AF_PACKET:
      ...
      ((struct sockaddr_ll *) sa)->sll_halen = (uint8_t)len;
      break;
    default:
			...
			struct sockaddr *sa, uint32_t prefixlen)
{
  ...
  switch (family)
    ...
    case AF_INET:
      ...
      p = (unsigned char *) &((struct sockaddr_in *) sa)->sin_addr;
      max_prefixlen = 32;
      break;
    case AF_INET6:
      memset (&((struct sockaddr_in6 *) sa)->sin6_addr, 0,
	      sizeof (((struct sockaddr_in6 *) sa)->sin6_addr));
      p = (unsigned char *) &((struct sockaddr_in6 *) sa)->sin6_addr;
...
nl_sendreq (int sd, int request, int flags, int *seq)
{
  ...
  req_hdr->nlmsg_type = (unsigned short)request;
  req_hdr->nlmsg_flags = (unsigned short)(flags | NLM_F_REQUEST);
  req_hdr->nlmsg_pid = 0;
  req_hdr->nlmsg_seq = (uint32_t)t;
...
nl_getmsg (int sd, int request, int seq, struct nlmsghdr **nlhp, int *done)
{
  ...
  for (;;)
	...
	{
	  if (nh->nlmsg_pid != (unsigned int)pid || nh->nlmsg_seq != (unsigned int)seq)
	    continue;
...
/* ---------------------------------- */
  ...
  for (build = 0; build <= 1; build++)
      ...
      if (build)
	{
	  ...
	  ifl = NULL;
         //lint -save -e124
	  data += NLMSG_ALIGN (sizeof (struct ifaddrs)) * (unsigned int)icnt;
	  xdata = data + dlen;
	  ifname = xdata + xlen;
	  iflist = ifdata;
	  ifflist =
      ...
      for (nlm = nlmsg_list; nlm; nlm = nlm->nlm_next)
	{
	  ...
	  for (nlh = nlh0;
	       NLMSG_OK (nlh, nlmlen); nlh = NLMSG_NEXT (nlh, nlmlen))
	    {
	      ...
	      if (nlh->nlmsg_pid != (unsigned int)pid || nlh->nlmsg_seq != (unsigned long)nlm->seq)
		continue;
		...
		case RTM_NEWLINK:
		  ...
		  nlm_scope = 0;
		  nlm_index = (unsigned int)ifim->ifi_index;
		  nlm_prefixlen = 0;
		  if (build && ifa && ifflist)
		    ifflist[nlm_index] = ifa->ifa_flags = (unsigned short)ifim->ifi_flags;
		  break;
		case RTM_NEWADDR:
		  ...
		  nlm_index = (unsigned int)ifam->ifa_index;
...
/* DTS2013032006115 p00134535 2013-3-20 change for getaddrinfo begin */
	      rtasize =
		NLMSG_PAYLOAD (nlh, 0) - NLMSG_ALIGN (nlm_struct_size);
/* DTS2013032006115 p00134535 2013-3-20 change for getaddrinfo end */
	      for (rta =
		   ...
		   RTA_OK (rta, rtasize); rta = RTA_NEXT (rta, rtasize))
		{
		  ...
		  switch (nlh->nlmsg_type)
			...
			case IFLA_BROADCAST:
			  ...
			  sa_len = ifa_sa_len (AF_PACKET, (int)rtapayload);
			...
			case IFA_ADDRESS:
			  ...
			  ifamap.address_len = (int)rtapayload;
			  break;
			case IFA_LOCAL:
			  ifamap.local = rtadata;
			  ifamap.local_len = (int)rtapayload;
			  break;
			case IFA_BROADCAST:
			  ifamap.broadcast = rtadata;
			  ifamap.broadcast_len = (int)rtapayload;
			  break;
#ifdef HAVE_IFADDRS_IFA_ANYCAST
			case IFA_ANYCAST:
		  ...
		  if (ifamap.address_len != ifamap.local_len ||
		      (ifamap.address != NULL &&
		       memcmp (ifamap.address, ifamap.local,
			       (unsigned long)ifamap.address_len)))
		    {
		      ...
		      else
			{
                        if(ifa && data)
                        {
                            ...
                            ifa_make_sockaddr (nlm_family, ifa->ifa_addr,
                            	     ifamap.address,
                            	     (size_t)ifamap.address_len, nlm_scope,
                            	     nlm_index);
                             //lint -save -e124
                            data +=
                            NLMSG_ALIGN (ifa_sa_len
		      ...
		      else
			{
                        if(ifa && data)
                        {
                            ...
                            ifa_make_sockaddr (nlm_family, ifa->ifa_broadaddr,
                                 ifamap.broadcast,
                                 (size_t)ifamap.broadcast_len, nlm_scope,
                                 nlm_index);
                             //lint -save -e124
                            data +=
                            NLMSG_ALIGN (ifa_sa_len
