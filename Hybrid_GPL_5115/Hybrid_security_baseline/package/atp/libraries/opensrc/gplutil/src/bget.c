...
//lint --e{40}  --e{826} 
void *bget(pvBgetHandle, requested_size)
  void* pvBgetHandle;
  bufsize requested_size;
{
    ATP_UTIL_BGET_GLOBAL_ST* pstBH = (ATP_UTIL_BGET_GLOBAL_ST*)pvBgetHandle;
    ...
    size += (bufsize)sizeof(struct bhead);     /* Add overhead in allocated buffer
					 to size required. */
...
#ifdef BestFit
	while (b != &pstBH->freelist) {
	    if (b->bh.bsize >= size) {
		if ((best == &pstBH->freelist) || (b->bh.bsize < best->bh.bsize)) {
		    best = b;
		}
	    }
...
#ifdef BufStats
		    pstBH->totalloc += size;
		    pstBH->numget++;		  /* Increment number of bget() calls */
#endif
	...
	if (size > exp_incr - sizeof(struct bhead)) {
		...
		pstBH->totalloc += size;
		pstBH->numget++;	      /* Increment number of bget() calls */
		pstBH->numdget++;	      /* Direct bget() call count */
#endif
		buf =  (void *) (bdh + 1);
		return buf;
	    }
...
#endif /* BECtl */
    printf("bget report: memory expired, pid:%d\n",
        getpid());
    return NULL;
}
...
{
    //lint -save -e515 -e516
    char *buf = (char *) bget(pvBgetHandle, size);
    //lint -restore
	...
	if (rsize == 0) {
	    ...
	    rsize = bd->tsize - (bufsize)sizeof(struct bdhead);
	} else {
	    rsize -= (bufsize)sizeof(struct bhead);
	}
	assert(rsize >= size);
...
void *bgetr(pvBgetHandle, buf, size)
  void* pvBgetHandle;
  void *buf;
  bufsize size;
{
    ...
    //lint -save -e515 -e516
    if ((nbuf = bget(pvBgetHandle, size)) == NULL) { /* Acquire new buffer */
	return NULL;
    }
    //lint -restore
    if (buf == NULL) {
...
#endif
	osize -= (bufsize)sizeof(struct bhead);
    assert(osize > 0);
...
void brel(void* pvBgetHandle, void *buf)
{
...
#ifdef BufStats
    pstBH->numrel++;			      /* Increment number of brel() calls */
#endif
    assert(buf != NULL);
...
#ifdef BufStats
	pstBH->totalloc -= bdh->tsize;
	assert(pstBH->totalloc >= 0);
	...
	assert(pstBH->freelist.ql.blink->ql.flink == &pstBH->freelist);
	assert(pstBH->freelist.ql.flink->ql.blink == &pstBH->freelist);
	b->ql.flink = &pstBH->freelist;
	b->ql.blink = pstBH->freelist.ql.blink;
	pstBH->freelist.ql.blink = b;
...
void bpool(pvBgetHandle, buf, len)
  void* pvBgetHandle;
  void *buf;
  bufsize len;
{
    ...
    if (pstBH)
    {
        memset(pstBH, 0, sizeof(ATP_UTIL_BGET_GLOBAL_ST));
        pstBH->freelist.ql.blink = &pstBH->freelist;
        pstBH->freelist.ql.flink = &pstBH->freelist;
    }
    else
    {
        printf("bpool failed.\n");
    ...
    //lint -save -e685
    assert(len - (bufsize)sizeof(struct bhead) <= -((bufsize) ESent + 1));
    //lint -restore
    ...
    len -= (bufsize)sizeof(struct bhead);
...
void bstats(pvBgetHandle, curalloc, totfree, maxfree, nget, nrel)
  void* pvBgetHandle;
