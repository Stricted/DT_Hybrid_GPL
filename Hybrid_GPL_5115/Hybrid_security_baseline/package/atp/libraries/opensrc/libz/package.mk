...
define ZLIB_BUILD_CMDS
	...
	$(TARGET_ATP_ENVS) O=$$($(2)_BDIR)/ PROGRAM_NAME=$$($(2)_REAL_PROGRAM_NAME) IS_DYNAMIC="$$($(2)_IS_DYNAMIC)" ATPFLAGS="-DMODULE_VERSION=\\\"$$($(2)_VERSION_STR)\\\" -D$(2)_MODULE_VERSION=\\\"$$($(2)_VERSION_STR)\\\" $$($(2)_FLAGS)" $(MAKE) -C $(SERVICE_ZLIB_SRC_DIR) $$($(2)_TARGET); \
	fi
endef

define ZLIB_CLEAN_CMDS
	$(Q)if [ -f $(SERVICE_ZLIB_SRC_DIR)/Makefile ]; then \
	$(CLEAN_ATP_ENVS) O=$$($(2)_BDIR)/ PROGRAM_NAME=$$($(2)_REAL_PROGRAM_NAME) $(MAKE) -C $(SERVICE_ZLIB_SRC_DIR) clean; \
	fi
endef
