...
struct usb_diskinfo{
    ...
    char modname[SHORT_STRLEN];/*product model type name, such as DataTravel*/
    ...
    char  writeprot[SHORT_STRLEN];  /*write protect flag*/
} *usbdiskinfos;
...
int attacheddisks = 0;
/*start modify by y00181549 for get_freediskslot加锁保护，防止重入后访问相同资源，at 20121007*/
/*get a free member*/

static DEFINE_MUTEX(semdisks);
...
static struct usb_diskinfo * get_freediskslot()
{
    ...
    {
        	attacheddisks ++;
              /*返回时即给使用标志赋值，以避免在插入多个u盘时导致重入*/
              ((struct usb_diskinfo *)diskinfos+i)->isvalid = 1;
              mutex_unlock(&semdisks);
...
/*end modify by y00181549 for get_freediskslot加锁保护，防止重入后访问相同资源，at 20121007*/
...
//    int n = 0;
    ...
    len += sprintf (page + len, "%s","Diskname Vendor       ID       Type   ConnectionType Capacity Port WriteProtect ModName\r\n");
    for(i = 0;i < MAX_DISKS;i++)
    {
        if(1 == ((struct usb_diskinfo *)diskinfos+i)->isvalid)
        {
            ...
            len += sprintf (page + len, "%-4s ", disk->usbport);
            len += sprintf (page + len, "%-12s ", disk->writeprot);
            len += sprintf (page + len, "%-16s ", disk->modname);
...
/*added by w00190448@20120714 解决读取u盘容量问题。之前一直是读取的0.end*/      
  ...
  /*Added by lvxin00135113@20091217 实现DT需求，实现TR140规范*/
	...
	/*added by w00190448@20120714 解决读取u盘容量问题。之前一直是读取的0.start*/ 
...
#else
        msleep(200); //对于sdkp重入的问题，延时是比较好的解决办法....by y00191549 at 20121007
        disk->capacity = (((unsigned long long)(sdkp->capacity)*512/1024))/1024;//以M为单位
#endif
        /*added by w00190448 @20120714 解决读取u盘容量问题。之前一直是读取的0.end*/
        ...
        /*add by y181549 for identification of different usb port for equipment at 20120914*/
        /*add by y181549 for identification of mode of usb disk at 20121130*/
        snprintf(disk->modname,SHORT_STRLEN,"%.16s",sdp->model);
        printk("mod name is %s\r\n",disk->modname);
	 /*add by y181549 for identification of mode of usb disk at 20121130*/
        /*START ADD by y00181549 for UDK0000310731 at 20131104*/
        sd_read_write_protect_flag(sdkp, buffer);
        snprintf(disk->writeprot,SHORT_STRLEN -1 ,"%s",sdkp->write_prot ? "on" : "off");
        printk("write protect is %s\r\n",disk->writeprot);
        /*END ADD by y00181549 for UDK0000310731 at 20131104*/
    }
...
#endif
/*Add end, l67944, 20100625*/
#if defined(CONFIG_BCM96362) || defined(CONFIG_BCM963268) || defined(CONFIG_HISI_CHIP)
    g_USB_PROBE = 1;
    printk("\n\n\n\nsd_probe: USB insert!!\n\n\n\n");
#endif
