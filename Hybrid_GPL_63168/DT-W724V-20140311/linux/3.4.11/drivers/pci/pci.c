...
int pcie_set_readrq(struct pci_dev *dev, int rq)
{
	...
	if (rq < 128 || rq > 4096 || !is_power_of_2(rq))
		goto out;
#if defined(CONFIG_BCM_KF_MIPS_BCM9685XX) && defined(CONFIG_BCM968500)
			//	Max read request size should not exceed 256 on Lilac
			if(rq > 256)
			{
					dev_printk(KERN_WARNING, &dev->dev, "Tried to program max read request size of %d. Set to Lilac maximum - 256\n", rq);
					rq = 256;
			}
#endif
	...
	if ((ctl & PCI_EXP_DEVCTL_READRQ) != v) {
		ctl &= ~PCI_EXP_DEVCTL_READRQ;
						...
						// Max payload size should bev equal to max read request size
