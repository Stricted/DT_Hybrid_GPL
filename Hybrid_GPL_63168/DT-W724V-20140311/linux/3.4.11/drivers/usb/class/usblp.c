...
static 	struct usb_printerinfo * get_freediskslot(void);

extern struct proc_dir_entry proc_root;
struct proc_dir_entry *proc_printermedium = NULL;
struct proc_dir_entry *printer_pid;
static struct task_struct *user_app = NULL; 
static char pid_str[128];
...
static int proc_user_printer_wr (struct file *file,
                             ...
                             void *data)
{
   strncpy(szUsbPrinerDevInfo, buffer,sizeof(szUsbPrinerDevInfo)-1);
   return count;
}
...
static void proc_user_ipp_create(void)
{
    ...
    proc_ipp->write_proc = (write_proc_t  *)proc_user_ipp_wr;
    proc_ipp->read_proc = (read_proc_t *)proc_user_ipp_rd;


    proc_printerdev = create_proc_entry("proc_user_printer", S_IFREG | S_IRUSR , &proc_root ); 
    proc_printerdev->write_proc = (write_proc_t  *)proc_user_printer_wr;
    proc_printerdev->read_proc = (read_proc_t *)proc_user_printer_rd;
	return;
}
static void proc_user_ipp_remove(void)
{
...
/* start of lKF33113 20110720 解决HP部分打印机无法直接通过网关USB安装驱动问题，发消息给USBMOUNT进程 */
#ifdef CONFIG_ATP_USB_ADAPTOR
#ifdef CONFIG_ATP_USB_PRINT_INSTALL_DRIVER
//static struct task_struct *user_app = NULL; 
extern char pid_str[128];
#endif
#endif
/* end of lKF33113 20110720 解决HP部分打印机无法直接通过网关USB安装驱动问题，发消息给USBMOUNT进程 */
/*
 * URB callback.
 */
...
static int proc_printer_pid_wr (struct file *file,
                             const char *buffer,
                             unsigned long count,
                             void *data)
{
...
static int proc_printer_pid_rd (char *page, char **start,
                              off_t off, int count,
                              int *eof, void *data)
{
    int len = 0;
...
/*Add by Huawei*/
	...
	usblp->intf = intf;
       ...
       //发信号给umount，umount对进程文件处理好后发消息给cms
       for(i = 0 ; i<2 ; i++)
       {
...
next1: 
	...
	printk(KERN_INFO "usblp%d: USB %sdirectional printer dev %d "
...
#ifdef CONFIG_ATP_USB_ADAPTOR		
        {
		char *manustring = NULL; 
		char *productstring = NULL;
		/* print product, vendor strings */
		if (NULL != dev->manufacturer)
			manustring = dev->manufacturer;
		else
			manustring = "Unknown";
		/*start of modify for DTS201211300068unknown的情况，则从设备的deviceid中解析by y00181549*/
        	if (NULL != dev->product)
        		productstring = dev->product;
        	else
        		productstring = "Unknown";
		if ((!strcmp(manustring,"Unknown")) && (!strcmp(productstring,"Unknown")) && (NULL != usblp->device_id_string))
		{
			char * pstr = NULL;
			char acdeviceidstring[1024] = {0};
			unsigned int ulstrlen = 0;
			unsigned int i = 0;
			unsigned int ullastindex = 0; 
			snprintf(acdeviceidstring,sizeof(acdeviceidstring),"%s",&usblp->device_id_string[2]);
			ulstrlen = strlen(acdeviceidstring);
			for (i = 0; i < ulstrlen;i++)
			{
				if (';' == acdeviceidstring[i])
				{
					acdeviceidstring[i] = '\0';
					if (strlen(&acdeviceidstring[ullastindex]) > 0)
					{
						pstr = &acdeviceidstring[ullastindex];
						if (strstr(pstr,"MFG:"))
						{
							manustring = &pstr[4];
							snprintf(disk->vendor ,STRLEN,"%s" , manustring);
						}
						else if (strstr(pstr,"MDL:"))
						{
							productstring = &pstr[4];
							snprintf(disk->model ,STRLEN,"%s" , productstring);
						}
						ullastindex = i + 1;
					}
				}
		...
		/*end of modify for DTS201211300068unknown的情况，则从设备的deviceid中解析by y00181549*/
		memset(szUsbPrinerDevInfo, 0, sizeof(szUsbPrinerDevInfo));
		snprintf(szUsbPrinerDevInfo,sizeof(szUsbPrinerDevInfo),"Manufacturer:%s\nModelName:%s\nModelNumber:%d\n",manustring,productstring,usblp->dev->descriptor.idProduct);   
...
#ifdef CONFIG_ATP_USB_PRINT_INSTALL_DRIVER        
        {
          ...
          if ( pid != 0 )
          {
            printk("PID from usblp module, %d\n", pid);
            user_app = find_task_by_vpid(pid);
            /* send sigusr1 */
            if(user_app)
            {
              send_sig(2, user_app, 0);
            }
          }
        }   
#endif	
...
static int __init usblp_init(void)
{
    ...
    //创建进程文件usbmount_pid用来取得umount进程的ID
      printer_pid = create_proc_entry("usbmount_pid", S_IFREG | S_IRUSR , &proc_root ); 
      printer_pid->write_proc = proc_printer_pid_wr;
      printer_pid->read_proc = proc_printer_pid_rd;
    //add by zhouyu 20101029 支持hub方式同时访问打印机及USB存储设备
