...
int enable_irq_lock_recording = 0;
#endif
...
static void irq_dispatch_int(void)
{
    ...
    /* CPU 100%时，打印当前epc和ra, 通过在命令下sw 0x80xxxxxx 10 0x80xxxxxx是g_show_epc的内存地址 */
#ifdef CONFIG_ATP_COMMON
    if (g_show_epc > 0 && g_show_epc_cpu == cpu)
    {
       struct pt_regs *regs = get_irq_regs();
       if (!user_mode(regs))
       {
           printk("epc=0x%x ra=0x%x\n", (unsigned int)regs->cp0_epc, (unsigned int)regs->regs[31]);
           g_show_epc--;
       }
    } 
#ifdef CONFIG_ATP_COMMON
    if (enable_irq_lock_recording)
    {
        if (0 == cpu)
        {
            struct pt_regs *mm_regs = get_irq_regs();
            if (!user_mode(mm_regs))
            {
                *(unsigned int *)0xafecd010 = (unsigned int)mm_regs->cp0_epc;
                *(unsigned int *)0xafecd014 = (unsigned int)mm_regs->regs[31];
            }
        }
        else
        {
            ...
            if (!user_mode(mm_regs))
            {
                *(unsigned int *)0xafecd018 = (unsigned int)mm_regs->cp0_epc;
                *(unsigned int *)0xafecd01c = (unsigned int)mm_regs->regs[31];
            }
        }
...
static __attribute__((__always_inline__)) void enable_brcm_irq_data_locked(unsigned long irq, cpumask_t affinity)
{
    ...
    if (irq >= INTERRUPT_ID_EXTERNAL_0 && irq <= INTERRUPT_ID_EXTERNAL_3) {
        ...
        PERF->ExtIrqCfg1 |= (1 << (irq - INTERRUPT_ID_EXTERNAL_4 + EI_LEVEL_SHFT));      // Level triggered
            ...
            PERF->ExtIrqCfg1 &= ~(1 << (irq - INTERRUPT_ID_EXTERNAL_4 + EI_LEVEL_SHFT));     // Edge triggered        
        }
        if ( detectSense ) {
            PERF->ExtIrqCfg1 |= (1 << (irq - INTERRUPT_ID_EXTERNAL_4 + EI_SENSE_SHFT));      // High / Rising triggered
        } else {
            PERF->ExtIrqCfg1 &= ~(1 << (irq - INTERRUPT_ID_EXTERNAL_4 + EI_SENSE_SHFT));     // Low / Falling triggered        
        }
        PERF->ExtIrqCfg1 |= (1 << (irq - INTERRUPT_ID_EXTERNAL_4 + EI_CLEAR_SHFT));      // Clear
...
void disable_brcm_irq_data(struct irq_data *data)
{
    ...
    if (enable_irq_lock_recording)
    {
        *(unsigned int *)0xafecd004 = data->irq;
    }
#endif
