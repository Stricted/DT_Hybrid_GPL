...
#if !defined(CONFIG_BCM96816) && !defined(CONFIG_BCM96818) && !defined(CONFIG_BCM96828) && !defined(CONFIG_BCM96838)
/***************************************************************************
...
static void BcmMemReserveInit(void)
{
	...
	if(size < BL_LILAC_TM_DEF_DDR_SIZE)
		size = BL_LILAC_TM_DEF_DDR_SIZE;
	...
	if(size < BL_LILAC_TM_MC_DEF_DDR_SIZE)
		size = BL_LILAC_TM_MC_DEF_DDR_SIZE;
...
#if defined(CONFIG_BCM96838)
	BcmMemReserveInit();
#elif defined(CONFIG_BCM96816) || defined(CONFIG_BCM96818) || defined(CONFIG_BCM96828)
    add_memory_region(0, (getMemorySize()), BOOT_MEM_RAM);
#else
#ifdef CONFIG_ATP_COMMON
    printk("scratch memory:0x%x, DSL SDRAM reserved: 0x%x\n", (unsigned int)kerSysGetDslPhyMemory(),ADSL_SDRAM_IMAGE_SIZE);
    add_memory_region(0, (unsigned long)((kerSysGetDslPhyMemory() - 8*1024)), BOOT_MEM_RAM);
#else
