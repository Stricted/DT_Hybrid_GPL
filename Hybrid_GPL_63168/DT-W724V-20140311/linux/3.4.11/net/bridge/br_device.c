...
#if defined(CONFIG_BCM_KF_WL)
#if defined(PKTC)
        if (wl_pktc_req_hook && (dst->dst->dev->path.hw_port_type == BLOG_WLANPHY) && wl_pktc_req_hook(GET_PKTC_TX_MODE, 0, 0, 0)) 
        {
            ...
            if (chainIdx != INVALID_CHAIN_IDX)
            {
                ...
                if ((skb->blog_p != NULL) && (skb->blog_p->rx.info.phyHdrType != BLOG_XTMPHY))
                {
