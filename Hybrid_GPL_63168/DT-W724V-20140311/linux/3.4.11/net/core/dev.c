...
#ifdef CONFIG_ATP_MIRROR
#define MAX_MIRROR_PORT_MUM 8
static int port_id_mirror_from[MAX_MIRROR_PORT_MUM] = {0,0,0,0,0,0,0,0};
#if SUPPORT_ATP_WANETH
#ifdef CONFIG_LAN_PORT_REVERSE
static char *mirror_port_name[MAX_MIRROR_PORT_MUM] = {"eth0.5","eth0.4","eth0.3","eth0.2",ATP_WANETH_INTERFACE_NAME,"wl0","rmnet0","nas1"};
#else
static char *mirror_port_name[MAX_MIRROR_PORT_MUM] = {"eth0.2","eth0.3","eth0.4","eth0.5",ATP_WANETH_INTERFACE_NAME,"wl0","rmnet0","nas1"};
#endif
#else
...
unsigned int web_ipcapture_flag = 1;
EXPORT_SYMBOL(web_ipcapture_flag);
#endif
...
/* BEGIN: Added by c65985, 2013/9/30   PN:DTS2013090607656*/
static void bind_mark(struct sk_buff *skb);
/* END:   Added by c65985, 2013/9/30 PN:DTS2013090607656*/
...
int netdev_path_remove(struct net_device *dev)
{
	...
	/* BEGIN: Added by c65985, 2013/8/1   PN:DTS2013080904237*/
#ifdef CONFIG_ATP_COMMON
	if (netdev_path_next_dev(dev) == NULL) 
        return 0;
#endif
	/* END:   Added by c65985, 2013/8/1 PN:DTS2013080904237*/
...
static void dev_queue_xmit_nit(struct sk_buff *skb, struct net_device *dev)
{
	...
	unsigned int ulMark = 0;

    //过滤fon模块的报文，不让工程页面抓到
    //取skb->mark最低位,fon专用
    ulMark = skb->mark & DT_QOS_QUEUE_MASK;
    if (FON_TRAFFIC_MARK == ulMark)
    {
        // printk(KERN_DEBUG "DROP FON packets!\n");
        return; 
    }
...
{
	if (net_ratelimit()) {
		always_printk("%s: hw csum failure\n", dev ? dev->name : "<unknown>");
		dump_stack();
	}
}
...
static int __netif_receive_skb(struct sk_buff *skb)
{
	...
	{   
        for(i = 0; i < MAX_MIRROR_PORT_MUM; i++)
        {
			...
			/* START OF ADD BY y00181549 for DTS2013070505747 Hybrid capture lte packet from nas0 at 2013/9/5*/
            else if (6 == i)
            {
                if(1 == port_id_mirror_from[i] 
                   && !strcmp(skb->dev->name, "nas0")
                   && port_id_mirror_to != i)
                {
                    pstTmp = skb_copy(skb, GFP_ATOMIC);
                    skb_push(pstTmp, ETH_HLEN);
			...
			/* END OF ADD BY y00181549 for DTS2013070505747 Hybrid capture lte packet from nas0 at 2013/9/5*/
            else
            {
                if(1 == port_id_mirror_from[i] 
   ...
   if ( bcm_mcast_def_pri_queue_hook ) {
    ...
    /*START MODIFY: l00252599 FOR DTS2013083107204 2013-10-14*/
	if (skb->protocol == htons(ETH_P_8021AG)) {
    /*END MODIFY: l00252599 FOR DTS2013083107204 2013-10-14*/
         goto skip_vlanctl;
   }
...
#endif
	/* START MODIFY by y00181549 for DTS2013092901584,DTS2013121306644 at 2014/1/1*/
    // START MODIFY: l00252599 FOR DTS2014030600603 2014-03-15	
#ifdef CONFIG_ATP_HYBRID
    if ((1 == web_ipcapture_flag) && ((skb->protocol == htons(ETH_P_8021Q)) || (skb->protocol == htons(ETH_P_8023AH))) && ((!strcmp(skb->dev->name,"nas_p1")) || (!strcmp(skb->dev->name,"nas_1_32")) || (!strcmp(skb->dev->name,"nas1"))))
    {
        list_for_each_entry_rcu(ptype, &ptype_all, list) {
...
#endif
   /* END MODIFY by y00181549 for DTS2013092901584,DTS2013121306644 at 2014/1/1*/
...
static int ctclean_seq_show(struct seq_file *seq, void *v)
{
    //调试清连接跟踪,用户态执行cat /proc/net/ctclean
    if (v == SEQ_START_TOKEN)
    {
...
#ifdef CONFIG_DT_COMMON
static const struct seq_operations ctclean_seq_ops = {
	.start = dev_seq_start,
	.next  = dev_seq_next,
	.stop  = dev_seq_stop,
	.show  = ctclean_seq_show,
};
#endif
...
#ifdef CONFIG_DT_COMMON
static int ctclean_seq_open(struct inode *inode, struct file *file)
{
	return seq_open_net(inode, file, &ctclean_seq_ops,
			    sizeof(struct seq_net_private));
}
#endif
...
#ifdef CONFIG_DT_COMMON
static const struct file_operations ctclean_seq_fops = {
	.owner	 = THIS_MODULE,
	.open    = ctclean_seq_open,
	.read    = seq_read,
	.llseek  = seq_lseek,
	.release = seq_release_net,
};
...
#endif 
#ifdef CONFIG_DT_COMMON
    if (!proc_net_fops_create(net, "ctclean", S_IRUGO, &ctclean_seq_fops))
		goto out_ctclean;
#endif
	rc = 0;
out:
...
static void netdev_wait_allrefs(struct net_device *dev)
{
	...
	while (refcnt != 0) {
		...
		if (time_after(jiffies, warning_time + 10 * HZ)) {
			always_printk("unregister_netdevice: waiting for %s to become free. Usage count = %d\n",
			       dev->name, refcnt);
...
void netdev_run_todo(void)
{
	...
	while (!list_empty(&list)) {
		...
		if (unlikely(dev->reg_state != NETREG_UNREGISTERING)) {
			always_printk("network todo '%s' but state %d\n",
