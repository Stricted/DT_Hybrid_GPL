...
{
	always_printk(KERN_EMERG "skb_over_panic: text:%p len:%d put:%d head:%p "
...
{
	always_printk(KERN_EMERG "skb_under_panic: text:%p len:%d put:%d head:%p "
			  "data:%p tail:%#lx end:%#lx dev:%s\n",
	       here, skb->len, sz, skb->head, skb->data,
	       (unsigned long)skb->tail, (unsigned long)skb->end,
	       skb->dev ? skb->dev->name : "<NULL>");
...
struct sk_buff *skb_morph(struct sk_buff *dst, struct sk_buff *src)
{
#if defined(CONFIG_BCM_KF_NBUFF)
	struct sk_buff *skb;
    /*START ADD: l00252599 FOR BONDING分区死机 DTS2013112206916 2014-04-11*/	
    ...
    /*END ADD: l00252599 FOR BONDING分区死机 DTS2013112206916 2014-04-11*/	
	return skb;
#else /* CONFIG_BCM_KF_NBUFF */
	skb_release_all(dst);
