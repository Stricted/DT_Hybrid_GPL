...
#if 0
    /*Start modified by d00191326 for DTS2013091003729:组播站点地址回复目标不可达*/
    if (ipv6_addr_type(&ipv6_hdr(skb)->daddr) & IPV6_ADDR_SITELOCAL)
    {       
        icmpv6_send(skb, ICMPV6_DEST_UNREACH, ICMPV6_ADDR_UNREACH, 0);
        kfree_skb(skb);
        return 0;
    }
    /*End modified by d00191326 for DTS2013091003729:组播站点地址回复目标不可达*/
#endif
	err = ip6mr_fib_lookup(net, &fl6, &mrt);
	if (err < 0) {
		kfree_skb(skb);
