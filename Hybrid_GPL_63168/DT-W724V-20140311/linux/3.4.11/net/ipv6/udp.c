...
extern struct in6_addr* get_old_saddr(void);
#endif
...
#ifdef CONFIG_ATP_HYBRID
static struct sock *__udp6_lib_lookup_skb_ex(struct sk_buff *skb,
					  __be16 sport, __be16 dport,
					  struct udp_table *udptable)
{
	struct sock *sk;
	...
	if (unlikely(sk = skb_steal_sock(skb)))
		return sk;
	return __udp6_lib_lookup(dev_net(skb_dst(skb)->dev), &iph->saddr, sport,
				 get_old_saddr(), dport, inet6_iif(skb),
				 udptable);
}
#endif
		   ...
		   int proto)
{
    ...
    {
        sk = __udp6_lib_lookup_skb_ex(skb, uh->source, uh->dest, udptable);
    }
#endif
	if (sk == NULL) {
