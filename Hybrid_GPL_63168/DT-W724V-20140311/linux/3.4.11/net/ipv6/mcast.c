...
#ifdef CONFIG_DT_QOS
int ipv6_dev_mc_inc_qos(struct net_device *dev, const struct in6_addr *addr, unsigned char tclass);
#endif
...
#ifdef CONFIG_DT_QOS
    err = ipv6_dev_mc_inc_qos(dev, addr, np->tclass);
#else
...
int ipv6_dev_mc_inc_qos(struct net_device *dev, const struct in6_addr *addr, unsigned char   tclass)
{
	...
	/* initial mode is (EX, empty) */
#if defined(CONFIG_BCM_KF_MLD) && !defined(CONFIG_BCM_SKIPKF_MLD) && defined(CC_BRCM_KF_MULTI_MLD_GR_SUPPRESSION)
	mc->mca_osfmode = MCAST_INCLUDE;
#endif
	mc->mca_sfmode = MCAST_EXCLUDE;
	mc->mca_sfcount[MCAST_EXCLUDE] = 1;
    ...
    printk("ipv6_dev_mc_inc_qos,tclass = %d\n", tclass);
#ifdef CONFIG_DT_QOS
    mc->tclass = tclass;
#endif
    printk("ipv6_dev_mc_inc_qos,mc->tclass = %2x\n", mc->tclass);
...
#ifdef CONFIG_DT_QOS
/* QGT: HOMEGW-8359 add */
unsigned char sysctl_mld_query_tclass  = 0;
/* QGT: HOMEGW-8359 end */
#endif
...
#ifdef CONFIG_DT_QOS
    sysctl_mld_query_tclass  = (((__u8)(ipv6_hdr(skb)->priority)) << 4) | ((ipv6_hdr(skb)->flow_lbl[0] >> 4) & 0x0f);
#endif
...
#ifdef CONFIG_DT_QOS
    if ((pmc->tclass)>>2 == 0x18) 
    {
        ipv6_hdr(skb)->priority = 0x8;
        skb->mark = (skb->mark & 0xFFFFFF00) |0x03;
    }
    else
    {
        ipv6_hdr(skb)->priority = 0x0;
...
static void mld_send_report(struct inet6_dev *idev, struct ifmcaddr6 *pmc)
{
	...
	if (!skb)
        return;
#ifdef CONFIG_DT_QOS
if (0x80 == sysctl_mld_query_tclass)
{
