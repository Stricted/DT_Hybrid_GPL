...
extern unsigned int (*ipgre_kernel_napt_gre_process_hook)(struct sk_buff *skb, struct net_device *dev);
extern unsigned int (*ipgre_kernel_ipv6_gre_process_hook)(struct sk_buff *skb, struct net_device *dev);
extern int ip_fragment_gre(struct sk_buff *skb, netdev_tx_t (*output)(struct sk_buff *, struct net_device *));
#endif
...
#else
#define HASH_ADDR(addr) ((__force u32)((addr)->s6_addr32[0] ^ (addr)->s6_addr32[1] ^ \
...
static struct net_device_stats *ip6gre_get_stats(struct net_device *dev)
{
		...
		__be32 key, __be16 gre_proto)
{
	...
	unsigned int h0;	//unaligned access fix
#else
	...
	ip6addr.s6_addr16[0] = remote->s6_addr16[0];
	ip6addr.s6_addr16[1] = remote->s6_addr16[1];
	ip6addr.s6_addr16[2] = remote->s6_addr16[2];
	ip6addr.s6_addr16[3] = remote->s6_addr16[3];
	ip6addr.s6_addr16[4] = remote->s6_addr16[4];
	ip6addr.s6_addr16[5] = remote->s6_addr16[5];
	ip6addr.s6_addr16[6] = remote->s6_addr16[6];
	ip6addr.s6_addr16[7] = remote->s6_addr16[7];
...
#ifdef CONFIG_ATP_HYBRID_GREACCEL
struct ip6_tnl *ip6gre_tunnel_find_bydev(struct net_device *dev)
{
	struct ip6_tnl *t;
	t = netdev_priv(dev);
	return t;
}
EXPORT_SYMBOL(ip6gre_tunnel_find_bydev);
#endif
		...
		u8 type, u8 code, int offset, __be32 info)
{
	...
	case ICMPV6_DEST_UNREACH:
        if (net_ratelimit())
		    printk("%s: Path to destination invalid or inactive!\n",
				     t->parms.name);
		break;
	case ICMPV6_TIME_EXCEED:
		if (code == ICMPV6_EXC_HOPLIMIT) {
            if (net_ratelimit())
    			printk("%s: Too small hop limit or routing loop in tunnel!\n",
    					     t->parms.name);
		}
		break;
	case ICMPV6_PARAMPROB:
		...
		if (teli && teli == info - 2) {
			...
			if (tel->encap_limit == 0) {
				if (net_ratelimit())
		            printk("%s: Too small encapsulation limit or routing loop in tunnel!\n",
						     t->parms.name);
			}
		} else {
			if (net_ratelimit())
		        printk("%s: Recipient unable to parse tunneled packet!\n",
					     t->parms.name);
		}
		break;
...
#ifdef CONFIG_ATP_HYBRID_TUNNEL_IDLE
static void ip6gre_time_refresh(struct sk_buff *skb, struct ip6_tnl *tunnel, enum ip6gre_xmit_rcv_mask mask)
{
...
#ifdef CONFIG_ATP_HYBRID_GREACCEL
int ip6gre_rcv(struct sk_buff *skb)
#else
...
#endif
{
	...
	if (tunnel) {
		...
		if (unlikely(err)) {
			if (log_ecn_error)
				if (net_ratelimit())
		            printk("non-ECT from %pI6 with dsfield=%#x\n",
						     &ipv6h->saddr,
        ...
        {            
            ...
            {
#ifdef CONFIG_ATP_HYBRID_TUNNEL_IDLE
                ip6gre_time_refresh(skb, tunnel, IP6GRE_RCV_MASK);
#endif
			 ...
			 __u32 *pmtu)
{
	...
	if (tdev == dev) {
		stats->collisions++;
		    if (net_ratelimit())
		        printk("%s: Local routing loop detected!\n",
				     tunnel->parms.name);
	...
	*(__be32 *)ipv6h = fl6->flowlabel | htonl(0x60000000);
    ...
    if (IS_GRE2_DEV(dev->name) && (1 == hisi_sw_accel_flag))
    {
    	...
    	int len;
        if ((pkt_len <= dst_mtu(skb_dst(skb))) && 
            (dev_nas = dev_get_by_name(&init_net, "nas0")) != NULL)
        {
            ...
            pst_ethhdr->h_proto = htons(ETH_P_IPV6);
...
static inline int ip6gre_xmit_ipv4(struct sk_buff *skb, struct net_device *dev)
{
    ...
    if ((skb->len > dev->mtu) || ((dstmtu > 0) && (skb->len > dstmtu)))
    {
        /* skb has been freed in ip_fragment_gre, return 0 */
        ip_fragment_gre(skb, ip6gre_xmit_ipv4);
        return 0;
    }
#endif
...
#ifdef CONFIG_ATP_HYBRID_TUNNEL_IDLE
    ip6gre_time_refresh(skb, t, IP6GRE_XMIT_MASK);
#endif
	...
	struct ifreq *ifr, int cmd)
{
	...
	struct ip6gre_net *ign = net_generic(net, ip6gre_net_id);
#ifdef CONFIG_ATP_HYBRID 
	/*Protocol 2013-10-22 15:29 for DTS2013102205060 by t00189036*/
...
static const struct net_device_ops ip6gre_netdev_ops = {
	...
	.ndo_get_stats	= ip6gre_get_stats,
};
