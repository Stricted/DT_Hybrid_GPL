...
#ifdef CONFIG_ATP_COMMON
    /*Start:z00182249 for DTS2013081309427:wan测地址不能通过RA前缀生成2013.8.23*/
	/*accept_ra()此函数是用于判断ra报文是从网关br0来的，还是从wan侧来的，前面1302行已经判断了，此处在做同样的判断会导致
	wan侧地址不能通过ra前缀生成，以及前缀路由也不能生成*/
	if (accept_ra(in6_dev))
		goto out;
	/*End:z00182249 for DTS2013081309427:wan测地址不能通过RA前缀生成2013.8.23*/
#else
	if (!accept_ra(in6_dev))
		goto out;
#endif
...
skip_routeinfo:
...
#ifndef CONFIG_DT_COMMON
    /*Start of ATP 2013-5-10 for UDK0000287110 by t00189036: Data IPv6 PMTU packet to big messages based in RA MTU*/    
    ...
    /*End of ATP 2013-5-10 for UDK0000287110 by t00189036*/
#endif
