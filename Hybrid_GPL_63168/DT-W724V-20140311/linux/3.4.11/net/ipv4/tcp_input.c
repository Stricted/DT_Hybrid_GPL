...
extern atomic_t g_tcp_out_of_order_num;
#endif
...
#ifdef CONFIG_ATP_BRCM
    atomic_sub(skb_queue_len(&tp->out_of_order_queue), &g_tcp_out_of_order_num);
#endif
	if (tcp_is_sack(tp))
...
#ifdef CONFIG_ATP_BRCM
            atomic_dec(&g_tcp_out_of_order_num);
#endif
			__kfree_skb(skb);
			continue;
		}
...
#ifdef CONFIG_ATP_BRCM
        atomic_inc(&g_tcp_out_of_order_num);
#endif
        goto end;
	}
...
#ifdef CONFIG_ATP_BRCM
    /*is totol out of order num is overhead ?*/
    if (TCP_OUT_OF_ORDER_MAX <= atomic_read(&g_tcp_out_of_order_num))
    {
        goto drop;
    }
#endif
...
static struct sk_buff *tcp_collapse_one(struct sock *sk, struct sk_buff *skb,
					struct sk_buff_head *list, int flag)
...
#ifdef CONFIG_ATP_BRCM
            skb = tcp_collapse_one(sk, skb, list, flag);
#else
...
#ifdef CONFIG_ATP_BRCM
        /*is totol out of order num is overhead ?*/
        if (flag && (TCP_OUT_OF_ORDER_MAX <= atomic_read(&g_tcp_out_of_order_num)))
        {
            return;
        }
#endif
...
#ifdef CONFIG_ATP_BRCM
            /*with flag 1 ,will calc totol out of order numbers*/
