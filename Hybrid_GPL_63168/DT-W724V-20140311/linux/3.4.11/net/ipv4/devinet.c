...
#define PPP_DEMAND_IP          0x0a404140
/*路由变化后到连接跟踪清除前，小段时间内，会出现Wan侧发出去的报文源地址和源接口不匹配*/
/*将wan侧这种这种不匹配的报文丢弃*/
/*check_addr_and_dev用于校验报文源地址是否属于发包dev
不属于的报文返回0，其他返回1*/
int check_addr_in_dev(struct sk_buff *skb)
{
    ...
    for_ifa(in_dev)
    {
        if (PPP_DEMAND_IP == ifa->ifa_local)
        {
            return 1;
        }
        
        if (iph->saddr == ifa->ifa_local)
        {
            return 1;
        }
    }endfor_ifa(in_dev);
...
EXPORT_SYMBOL(check_addr_in_dev);
#endif
