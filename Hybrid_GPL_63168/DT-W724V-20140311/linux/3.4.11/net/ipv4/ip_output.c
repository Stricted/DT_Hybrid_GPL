...
#include "qosdtkrnl.h"
#endif
/*Start of Protocol 2014-4-24 10:55 for DTS2014032806341:网关自身业务的平滑切换 by t00189036*/
#ifdef CONFIG_ATP_HYBRID
#include <net/netfilter/nf_conntrack.h>
#define L2TP_PORT   (1701)
#endif
/*End of Protocol 2014-4-24 10:55 for by t00189036*/

extern int check_addr_in_dev(struct sk_buff *skb);
...
/*Start of Protocol 2014-4-24 10:56 for DTS2014032806341:网关自身业务的平滑切换 by t00189036*/
#ifdef CONFIG_ATP_HYBRID
    struct nf_conn *nfct;
	...
	struct udphdr *uh;
#endif
/*End of Protocol 2014-4-24 10:56 for by t00189036*/
    ...
    if (skb->mark & ATP_CONNTRACK_GET_OUTPUT_TEST_MARK)
    {
        ...
        int lRet = ATP_SENDTO_RET_INDEX_SHIFT_MIN - skb->dev->ifindex ;
        kfree_skb(skb);
	...
	if (IPPROTO_UDP == iph->protocol)
	{
		...
		if (htons(L2TP_PORT) == uh->dest)
		{
			isL2tp = 1;
		}
	}
    /*Start of Protocol 2014-4-24 10:57 for DTS2014032806341:网关自身业务的平滑切换 by t00189036*/
    nfct = (struct nf_conn *)skb->nfct;
    /*只处理有连接跟踪，没有打上平滑切换的mark，只处理网关自己发送的报文*/
    if (nfct && (!(nfct->mark & ATP_CONNTRACK_FLOW_MARK_MASK))
        && (!(skb->mark & PPP_TRIGER_MARK)) && (!(IPPROTO_GRE == iph->protocol))
		 && (fonip != (nfct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u3.ip >>8)) 
		 && !isL2tp)
    {
    ...
    /*End of Protocol 2014-4-24 10:57 for by t00189036*/
#endif  
