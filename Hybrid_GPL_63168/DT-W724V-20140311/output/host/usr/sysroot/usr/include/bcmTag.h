...
#define BootTagBackOffset 0x100000


 /*start: modify by j00127542 for DTS2011021500973 2011-02-15*/
#define BOOT_TAG_SIZE              16       /*flash分区最后16个存放升级标识数据的长度*/
#ifdef SUPPORT_ATP_FLASH_SIZE_16M
#define SECTOR_FLASHSIZE     0x20000
#else
#define SECTOR_FLASHSIZE     0x10000
#endif
...
#ifdef SUPPORT_ATP_FLASH_SIZE_16M
#define DOUBLE_FLASHSIZE       0x1000000
#else
#define DOUBLE_FLASHSIZE       0x800000
#endif
#else
#define DOUBLE_FLASHSIZE       0x400000
#endif
...
/*定义XML TAG FILE长度为4字节*/
#define XML_TAG_FILE_LEN    4

//#define HW_VDF_HG553        "7"
...
/*#define RESERVED_LEN    (TAG_LEN - TAG_VER_LEN - SIG_LEN - SIG_LEN_2 - \
                         CHIP_ID_LEN - BOARD_ID_LEN - (5*IMAGE_LEN) - \
                         (4*ADDRESS_LEN) - (3*FLAG_LEN) - (2*TOKEN_LEN) - \
                         PRODUCT_NAME_LEN - SOFT_VERSION_LEN)*/
...
/*Added by lvxin00135113@20100122 for support dect image upgrade*/
/*the total size is */
#define     DECT_TAG_LEN                128
#define     DECT_VER_LEN                24
#define     DECT_RESERVED_LEN      DECT_TAG_LEN - DECT_VER_LEN - IMAGE_LEN - CRC_LEN

typedef struct _DECT_FILE_TAG
{
    unsigned char dectTagVersion[DECT_VER_LEN];// tag version.  Will be 24 characters here.
    unsigned char imageLen[IMAGE_LEN];   
    unsigned char imageCrc[CRC_LEN];
    unsigned char reserved[DECT_RESERVED_LEN];
}DECT_FILE_TAG,*PDECT_FILE_TAG;
/*Added by lKF33113 for support NANDFLASH*/
#ifdef SUPPORT_ATP_NANDFLASH
/* Whole flash image TAG definitions. */
#define WFI_CHIPID             0x00006362
...
#define SIGN_LEN        (16*1024)
#define HEAD_TAG_SIZE   256

#define IMAGE_LEN_T  10
#define CRC_LEN_T       10
#define DEFAULTFLAGSIZE       4
typedef struct imageInfo
{
    char  size[IMAGE_LEN_T];
    char  imageCrc[CRC_LEN_T];
    char  defaultFlag[DEFAULTFLAGSIZE];
}imageInfo;
#define SIGN_LEN (16*1024)
#define JFFS2_MAGIC_BITMASK 0x1985
#define BOOTLODAER_BITMASK  0x1000027B
#endif
/*End add by lKF33113 for support NANDFLASH*/
// only included if for bcmTag.exe program
#if 1 //def BCMTAG_EXE_USE
