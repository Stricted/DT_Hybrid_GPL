...
#define ATP_UPG_TYPE_IMAGE_I2C              (ATP_UPG_TYPE_IMAGE_FILE + 0x200)
...
#define ATP_UPG_SIGNATURE_ISSUER_SIZE (ATP_UPG_DATE_LEN + ATP_UPG_DATE_LEN + ATP_UPG_STR_LEN+ATP_UPG_DATE_LEN)
#define ATP_SIGNATURE_MAGIC_NUM "372153721537215"
//!\brief 签名结构
/*!
 * 打在包尾的签名数据结构
 */ 
...
{    
    unsigned char                   aucMagicNum[ATP_UPG_DATE_LEN];    /*!< 魔鬼数字 */ 
...
typedef struct tagATP_UPG_SUBFILE_ST
{
    ...
    char                              *pcUrl;       /*!< 下载URL */    // 
    unsigned char                     ucNeedUpgSucc; /*!< 是否必须升级成功 1：需要；0：不需要 */    
    unsigned int                      bUpgResult;   /*!< 是否成功 */
    void                              *pvArg;    /*!< 扩展指针 */     
...
#define UPNP_UPGRADE_LED_COUNTER    (410)
#endif
