...
#define PKTDMA_ETH_LOW_BD_THRESHOLD     400
#define PKTDMA_XTM_LOW_BD_THRESHOLD     400
...
/* --------------------------------------------------------------------------
    Name: bcmPktDma_IsEthLowFreeRecvBuf_Iudma
 Purpose: Check if free receive buffer is under PKTDMA_ETH_LOW_BD_THRESHOLD.
-------------------------------------------------------------------------- */
static inline BOOL bcmPktDma_IsEthLowFreeRecvBuf_Iudma(BcmPktDma_LocalEthRxDma * rxdma)
{
    volatile DmaDesc * rxBd;
    DmaDesc            dmaDesc;
    uint32             rxIndex;

	if(!rxdma->rxEnabled) return FALSE;

    if(rxdma->rxAssignedBds < PKTDMA_ETH_LOW_BD_THRESHOLD)
        /* Can't tell if h/w about to run out of BDs */
        return FALSE;

    rxIndex = rxdma->rxTailIndex;
    if(rxIndex >= PKTDMA_ETH_LOW_BD_THRESHOLD)
        rxIndex -= PKTDMA_ETH_LOW_BD_THRESHOLD;
    else
        rxIndex = rxdma->numRxBds - (PKTDMA_ETH_LOW_BD_THRESHOLD - rxIndex);
...
#if defined(CONFIG_BCM_XTMCFG) || defined(CONFIG_BCM_XTMCFG_MODULE)
/* --------------------------------------------------------------------------
    Name: bcmPktDma_IsXtmLowFreeRecvBuf_Iudma
 Purpose: Check if free receive buffer is under PKTDMA_XTM_LOW_BD_THRESHOLD.
-------------------------------------------------------------------------- */
static inline BOOL bcmPktDma_IsXtmLowFreeRecvBuf_Iudma(BcmPktDma_LocalXtmRxDma * rxdma)
{
    volatile DmaDesc * rxBd;
    DmaDesc            dmaDesc;
    uint32             rxIndex;
    ...
    if(rxdma->rxAssignedBds < PKTDMA_XTM_LOW_BD_THRESHOLD)
        /* Can't tell if h/w about to run out of BDs */
        return FALSE;

    rxIndex = rxdma->rxTailIndex;
    if(rxIndex >= PKTDMA_XTM_LOW_BD_THRESHOLD)
        rxIndex -= PKTDMA_XTM_LOW_BD_THRESHOLD;
    else
        rxIndex = rxdma->numRxBds - (PKTDMA_XTM_LOW_BD_THRESHOLD - rxIndex);
...
#define FAP_WLAN_PRIORITY_BIT_POS  (1)
#define FAP_WLAN_PRIORITY_MASK     (0x7 << FAP_WLAN_PRIORITY_BIT_POS)
#define FAP_GET_WLAN_PRIORITY(VAL) ((VAL & FAP_WLAN_PRIORITY_MASK) >> FAP_WLAN_PRIORITY_BIT_POS)
#define FAP_SET_WLAN_PRIORITY(ENCODEVAL, PRIO) ((ENCODEVAL & (~FAP_WLAN_PRIORITY_MASK)) | (PRIO << FAP_WLAN_PRIORITY_BIT_POS))

#define FAP_WLAN_IQPRIO_BIT_POS    (0)
#define FAP_WLAN_IQPRIO_MASK       (0x1 << FAP_WLAN_IQPRIO_BIT_POS)
#define FAP_GET_WLAN_IQPRIO(VAL)   ((VAL & FAP_WLAN_IQPRIO_MASK) >> FAP_WLAN_IQPRIO_BIT_POS)
#define FAP_SET_WLAN_IQPRIO(ENCODEVAL, IQPRIO) ((ENCODEVAL & (~FAP_WLAN_IQPRIO_MASK)) | (IQPRIO << FAP_WLAN_IQPRIO_BIT_POS))
...
#define ENCODE_WLAN_PRIORITY_MARK(u8EncodeVal, u32Mark) \
    (u8EncodeVal = FAP_SET_WLAN_PRIORITY(u8EncodeVal, LINUX_GET_PRIO_MARK(u32Mark)) | FAP_SET_WLAN_IQPRIO(u8EncodeVal, SKBMARK_GET_IQPRIO_MARK(u32Mark)))
#define DECODE_WLAN_PRIORITY_MARK(encodedVal, u32Mark) \
    (u32Mark = (LINUX_SET_PRIO_MARK(u32Mark, FAP_GET_WLAN_PRIORITY(encodedVal)) | SKBMARK_SET_IQPRIO_MARK(u32Mark, FAP_GET_WLAN_IQPRIO(encodedVal))))
