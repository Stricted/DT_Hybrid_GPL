...
#define ATP_UTIL_LISTDELETEALL(plist, pfdestory) \
	ATP_UTIL_ListDeleteAll((ATP_UTIL_LIST_ST*)(plist), (PFUtilListItemDestroyProc)(pfDestroy));

VOS_VOID ATP_UTIL_ListRemoveItem(
						ATP_UTIL_LIST_ST		**ppstStart,
						ATP_UTIL_LIST_ST 		*pstRemoveItem);
#define ATP_UTIL_LISTREMOVEITEM(pphead, pitem) \
	ATP_UTIL_ListRemoveItem((ATP_UTIL_LIST_ST**)(pphead), (ATP_UTIL_LIST_ST*)(pitem));

VOS_VOID ATP_UTIL_ListDeleteItem(
                        ATP_UTIL_LIST_ST            **ppstStart,
                        ATP_UTIL_LIST_ST            *pstDelItem,
                        PFUtilListItemDestroyProc   pfDestroy);
#define ATP_UTIL_LISTDELETEITEM(pphead, pdelitem, pfdestory) \
	ATP_UTIL_ListDeleteItem((ATP_UTIL_LIST_ST**)(pphead), (ATP_UTIL_LIST_ST*)(pdelitem), (PFUtilListItemDestroyProc)(pfdestory))

VOS_UINT32 ATP_UTIL_ListGetCount(ATP_UTIL_LIST_ST *pstList);

#define ATP_UTIL_LISTGETCOUNT(plist, count) \
	count = ATP_UTIL_ListGetCount((ATP_UTIL_LIST_ST *)(plist))
	
VOS_VOID ATP_UTIL_ListAppend(
                        ATP_UTIL_LIST_ST            **ppstList,
                        ATP_UTIL_LIST_ST            *pstItem);
#define ATP_UTIL_LISTAPPEND(pphead, pitem) \
	ATP_UTIL_ListAppend((ATP_UTIL_LIST_ST**)(pphead), (ATP_UTIL_LIST_ST*)(pitem))
...
#define ATP_UTIL_LISTFINDITEM(phead, pvData, pfcompare) \
	ATP_UTIL_ListFindItem((const ATP_UTIL_LIST_ST*)(phead), (const VOS_VOID*)(pvData), (PFUtilListItemCompareProc)(pfcompare))
/*
 * Timer utilities
 */
...
/*Start by z00250609 DTS2014021809957: 网关周期上报和升级同时进行，出现升级写flash完成之后，网关不重启 */
/*!
  \brief 缓存同步写入文件
  \param[in]  *pcContent: 
  \param[in]  ulLen: 
...
/*End by z00250609 DTS2014021809957: 网关周期上报和升级同时进行，出现升级写flash完成之后，网关不重启 */	
...
extern VOS_BOOL g_bNotClearQueue;
