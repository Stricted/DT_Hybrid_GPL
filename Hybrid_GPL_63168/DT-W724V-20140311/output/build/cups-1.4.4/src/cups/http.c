...
////printf("=("\n=1111=httpConnectEncrypt(host=\"%s\", port=%d, encryption=%d)\n",
 /*
...
//printf("pail= cups==file:%s,line:%d=\n",__FILE__,__LINE__);
...
//      printf(("4httpGets: read %d bytes...", bytes));
//printf("pail= cups==file:%s,line:%d=\n",__FILE__,__LINE__);
...
//printf("pail= cups==file:%s,line:%d=\n",__FILE__,__LINE__);
  ...
  DEBUG_printf("3httpGets: No new line available!");
...
////printf("=("\n==file:%s,line:%d,func:%s=pid=%d=\n",__FILE__,__LINE__,__FUNCTION__,getpid());
////printf("=("7http_send(http=%p, request=HTTP_%s, uri=\"%s\")",
   //             http, codes[request], uri);
...
////printf("=("==file:%s,line:%d,func:%s=pid=%d=\n",__FILE__,__LINE__,__FUNCTION__,getpid());
  ...
  */
    ////printf("=("==file:%s,line:%d,func:%s=http->authstring=%s=\n",__FILE__,__LINE__,__FUNCTION__,http->authstring);
...
////printf("=("==file:%s,line:%d,func:%s=bytes=%d=\n",__FILE__,__LINE__,__FUNCTION__,bytes);
    if (bytes < 0)
    {
#ifdef WIN32
      if (WSAGetLastError() != http->error)
...
////printf("=("==file:%s,line:%d,func:%s=pid=%d=\n",__FILE__,__LINE__,__FUNCTION__,getpid());
////printf("=("8http_write: error writing data...");
      return (-1);
    }
...
#endif /* DEBUG */
////printf("=("==file:%s,line:%d,func:%s=tbytes=%d=\n",__FILE__,__LINE__,__FUNCTION__,tbytes);
  return (tbytes);
}
