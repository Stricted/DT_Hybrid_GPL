		  ...
		  const char *filename)	/* I - File to send or @code NULL@ for none */
{
  ...
  if (filename)
  {
    ...
    snprintf(request->filename,48,"%s",filename);
#endif
...
//printf("==file:%s,line:%d,func:%s=pid=%d=\n",__FILE__,__LINE__,__FUNCTION__,getpid());
 ...
 //printf("==file:%s,line:%d,func:%s=http->authstring=%s,pid=%d=\n",__FILE__,__LINE__,__FUNCTION__,http->authstring,getpid());
 /*
  ...
  */
  ...
  while (response == NULL)
  {
    ...
    */
    ...
    //printf("=333=2cupsDoIORequest: status=%d", status);
      ...
      */
      ...
      lseek(infile, 0, SEEK_SET);
        ...
        if (filelength < 48)
        {
            ...
            {
                //printf("==file:%s,line:%d,func:%s=pid=%d=\n",__FILE__,__LINE__,__FUNCTION__,getpid());
                snprintf(buffer,sizeof(buffer),"%s",request->filename);
                if (httpCheck(http))
                {
                //printf("==file:%s,line:%d,func:%s=pid=%d=\n",__FILE__,__LINE__,__FUNCTION__,getpid());
        ...
        else
        {
            while (filelength >0)
            {
                ...
                DBGYJ("bytes=%d,getpid()=%d",bytes,getpid());
                //printf("==file:%s,line:%d,func:%s=pid=%d=\n",__FILE__,__LINE__,__FUNCTION__,getpid());
                if (httpWrite2(http, buffer, bytes) < bytes)
                  break;
            }
 ...
 //printf("==file:%s,line:%d,func:%s=pid=%d=\n",__FILE__,__LINE__,__FUNCTION__,getpid());
    ...
    {
        //printf("==file:%s,line:%d,func:%s=status=%d,pid=%d=\n",__FILE__,__LINE__,__FUNCTION__,status,getpid());
  ...
  for (;;)
  {
    ...
    */
    ...
    if (!got_status && expect == HTTP_CONTINUE)
    {
     ...
     //printf("==file:%s,line:%d,func:%s=pid=%d,status=%d=\n",__FILE__,__LINE__,__FUNCTION__,getpid(),status);
