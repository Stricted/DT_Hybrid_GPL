...
// printf("cupsdCheckJobs: %d active jobs, sleeping=%d, reload=%d",
...
cupsdContinueJob(cupsd_job_t *job)	/* I - Job */
{
  ...
  */
  ...
  {
  job->job_sheets = ippFindAttribute(job->attrs, "job-sheets", IPP_TAG_ZERO);
  #ifdef OPEN_DEBUG
...
//printf("==file:%s,line:%d,func:%s=job->status=%d=\n",__FILE__,__LINE__,__FUNCTION__,job->status);
	...
	abort_message = "Stopping job because the sheduler could not execute "
      ...
      {
  #ifdef OPEN_DEBUG       //printf("==file:%s,line:%d,func:%s=job->status=%d=\n",__FILE__,__LINE__,__FUNCTION__,job->status);
             ...
             int         set_job_state)	/* I - 1 = set the job state */
{
  ...
  cupsdLogMessage(CUPSD_LOG_ERROR, "finalize_job(job=%p(%d))", job, job->id);
#endif
 /*
...
#endif
  #ifdef OPEN_DEBUG
cupsdLogMessage(CUPSD_LOG_ERROR, "=file:%s,line:%d,func:%s=job->state_value=%d=\n",__FILE__,__LINE__,__FUNCTION__,job->state_value);
  #endif
 /*
...
//printf("==file:%s,line:%d,func:%s=job->state_value=%d=\n",__FILE__,__LINE__,__FUNCTION__,job->state_value);
 //fputs("==file:%s,line:%d,func:%s=job->state_value=%d=\n",__FILE__,__LINE__,__FUNCTION__,job->state_value, stderr);
//fputs("\n====1111111111======\n", stderr);
...
//printf("==file:%s,line:%d,func:%s=job->status=%d=\n",__FILE__,__LINE__,__FUNCTION__,job->status);
  if (job->status < 0)
  {
   ...
   //printf("==file:%s,line:%d,func:%s=exit_code=%d=\n",__FILE__,__LINE__,__FUNCTION__,exit_code);
  ...
  for (attr = ipp->attrs; attr != NULL; attr = attr->next)
  {
    ...
    */
    ...
    switch (attr->value_tag)
    {
	  ...
	  */
          for (i = 0; (i < attr->num_values) && (&(attr->values[i]) != NULL) ; i ++)
            {
                if(attr->values[i].string.text != NULL)
                {
...
int pid=getpid();
//printf("\n==file:%s,line:%d,func:%s=job->status=%d=\n",__FILE__,__LINE__,__FUNCTION__,job->status);
                                   ...
                                   message, sizeof(message))) != NULL)
  {
    ...
    */
    ...
    else if (!strncmp(message, "recovered:", 10))
    {
	 ...
	 #else
     if(!job->printer->recoverable || strcmp(job->printer->recoverable, ptr))
     #endif
      {
	cupsdSetString(&(job->printer->recoverable), ptr);
    #ifdef NOT_CUT
