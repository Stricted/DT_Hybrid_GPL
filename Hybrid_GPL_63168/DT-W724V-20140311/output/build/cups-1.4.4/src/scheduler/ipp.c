...
//printf("\n==file:%s,line:%d,func:%s==\n",__FILE__,__LINE__,__FUNCTION__);
  ...
  */
  ...
  else
  {
	...
	*/
	  ...
	  cupsdLogMessage(CUPSD_LOG_DEBUG, "%s",
                	  ippOpString(con->request->request.op.operation_id));
      // printf("==file:%s,line:%d,func:%s==[%d]\n",__FILE__,__LINE__,__FUNCTION__,con->request->request.op.operation_id);

	switch (con->request->request.op.operation_id)
	{
	  case IPP_PRINT_JOB :
                //printf("n==file:%s,line:%d,func:%s==\n",__FILE__,__LINE__,__FUNCTION__);
              print_job(con, uri);
              break;
#ifdef NOT_CUT
  ...
  if (con->response)
  {
    ...
    */
//   printf( "=111==Returning IPP %s for %s (%s) from %s",
...
//printf("==file:%s,line:%d,func:%s=scheme=%s=\n",__FILE__,__LINE__,__FUNCTION__,scheme);
...
//    printf("==file:%s,line:%d,func:%s=\n",__FILE__,__LINE__,__FUNCTION__);
 ...
 //       printf("==file:%s,line:%d,func:%s=\n",__FILE__,__LINE__,__FUNCTION__);
    ...
    else
    {
      ...
      */
//    printf("==file:%s,line:%d,func:%s=scheme=%s,ServerBin=%s=\n",__FILE__,__LINE__,__FUNCTION__,scheme,ServerBin);
...
//    printf("==file:%s,line:%d,func:%s=old_device_uri=%s=\n",__FILE__,__LINE__,__FUNCTION__,old_device_uri);
//    printf("==file:%s,line:%d,func:%s=printer->sanitized_device_uri=%s=\n",__FILE__,__LINE__,__FUNCTION__,printer->sanitized_device_uri);
    if (printer->sanitized_device_uri)
      ...
      old_device_uri[0] = '\0';
//printf("==file:%s,line:%d,func:%s=old_device_uri=%s=\n",__FILE__,__LINE__,__FUNCTION__,old_device_uri);
//printf("==file:%s,line:%d,func:%s=attr->values[0].string.text=[[[%s]]]=\n",__FILE__,__LINE__,__FUNCTION__,attr->values[0].string.text);
    cupsdSetDeviceURI(printer, attr->values[0].string.text);
//    printf("==file:%s,line:%d,func:%s=&printer->device_uri=%s=\n",__FILE__,__LINE__,__FUNCTION__,printer->device_uri);
//    printf("=333=Setting %s device-uri to \"%s\" (was \"%s\".)",
...
//printf("==file:%s,line:%d,func:%s=&printer->device_uri=%s=\n",__FILE__,__LINE__,__FUNCTION__,printer->device_uri);
  ...
  */
  ...
  if (con->filename)
  {
    need_restart_job = 1;
    changed_driver   = 1;
//    printf("==file:%s,line:%d,func:%s==[%s]\n",__FILE__,__LINE__,__FUNCTION__,con->filename);
		  ...
		  ipp_attribute_t *uri)	/* I - Printer URI */
{
  ...
  cupsdLogMessage(CUPSD_LOG_DEBUG2, "get_printer_attrs(%p[%d], %s)", con,
                  con->http.fd, uri->values[0].string.text);
//printf("n==file:%s,line:%d,func:%s=con->response->request.status.status_code=%d=\n",__FILE__,__LINE__,__FUNCTION__,con->response->request.status.status_code);
//printf("get_printer_attrs(%p[%d], %s)", con,
//                  con->http.fd, uri->values[0].string.text);
 /*
  * Is the destination valid?
  */
