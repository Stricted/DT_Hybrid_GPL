...
#./Configure atp_b_config shared no-asm no-engines no-fips
...
do
	echo "modifying " $file
	cp $file "$file"_bk
	sed -e 's/^CC=/#CC=/' -e 's/ARD=ar/ARD=$(AR)/' -e 's/^RANLIB=/#RANLIB=/' -e 's/^AR=/#AR=/' -e 's/$(AR) $(LIB)/$(AR) r $(LIB)/' -e 's/$(ARX)/$(AR) r/' "$file"_bk > $file
	rm -f "$file"_bk
done
