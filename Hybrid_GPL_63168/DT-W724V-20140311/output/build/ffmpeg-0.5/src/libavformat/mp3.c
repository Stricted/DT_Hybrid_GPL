...
#define SWAPENDIAN(x) ((((x)&0xFF)<<8)|(((x)&0xFF00)>>8))
...
static unsigned int ChangeToUtf8( unsigned short tmp, unsigned char* q)
{
/*
...
*/
    if (tmp < 0x0080){
       ...
       *(unsigned char*)(q+2) = 0x80|(tmp&0x3F);
       *(unsigned char*)(q+1) = 0x80|((tmp&0xFC0)>>6);
       *(unsigned char*)(q+0) = 0xE0|((tmp&(0xF<<12))>>12);
       return 3;
    }
}
...
{// only for text tag.
    ...
    case 0:  //ISO-8859-1 just copy
    case 3:  /* UTF-8 */ //just copy
        len = FFMIN(taglen, dstlen-1);
        get_buffer(s->pb, dst, len);
        dst[len] = 0;
        break;
    case 1: //UTF-16 with BOM, change to UTF-8
        q = dst;
        if (taglen < 2)
            break;
        bom = get_be16(s->pb);
        ...
        while(taglen >= 2 && (q-dst) < (dstlen-1)) {//just tranfer UTF-16 to IS0-8859-1
           tmp = get_be16(s->pb);
           if (bigendian){
               tmp = SWAPENDIAN(tmp);
           }
           q += ChangeToUtf8( tmp, q );
           taglen -= 2;
        }
        *q = '\0';
        break;
...
static void id3v2_parse(AVFormatContext *s, int len, uint8_t version, uint8_t flags)
{
    ...
    while(len >= taghdrlen) {
        if(isv34) {
            tag  = get_be32(s->pb);
/*code with bug, the length  of the tag not format as the size of the ID3 tag size, only be easy format of integer 32.*/
/*by Huawei Corp, jianghaibing 214435*/
...
/*by Huawei Corp, laopeifeng 161677*/
        ...
        case MKBETAG(0,   'T', 'R', 'K'):
            ...
            break;
        case MKBETAG( 'T', 'O', 'R', 'Y' ):
        case MKBETAG( 'T', 'Y', 'E', 'R' ):        
            id3v2_read_ttag(s, tlen, "year" );
            break;
        case 0:
            /* padding, skip to end */
            url_fskip(s->pb, len);
