 ...
 * 仅仅只针对其他选项，设置出口port值
 * 仅被  proxy_rewrite_invitation_body 调用
 * RETURNS
 *    STS_SUCCESS on success
 *    STS_FAILURE on error
 */
/* DT c40163 modify for QOS
int rtp_relay_start_fwd (osip_call_id_t *callid,   //  call id 值
                         char *client_id,  //  应该是  contact 中的username值，为空则是host值
                         int rtp_direction,        //  应该是  in
...
/* DT c40163 end */
{
   ...
   for (i=0; i<RTPPROXY_SIZE; i++)
   {
         ...
         (strcmp(rtp_proxytable[i].client_id, client_id) == 0))     //可以考虑去掉  
      {
         ...
         /*当遇到fork的invite的时候，需要查找对应的remote_info信息记录或添加新的remote_info记录*/
         ...
         /*如果没有找到对应的记录，表明是一条新的记录，需要找一条空记录来记录wan_port*/
         ...
         /*找到原有记录或者新增加了remote_info信息记录后，需要将remote_ipaddr和remote_port进行更新*/
         if (k < MAX_BRANCH_SIZE)
         {
             DEBUGPC("rtp_proxytable[%d].remote_info[%d].wan_port:%d", i, k, rtp_proxytable[i].remote_info[k].wan_port);
         ...
         //  如果上面修改了比较选项，则此处改成  media_stream_no 判断赋值
    ...
    * 对于所有的 rtp_proxytable 表项前面均没有匹配，当前为新的连接
    */
   freeidx=-1;
   for (j=0; j<RTPPROXY_SIZE; j++)
   {
   ...
   //  否则 freeidx 指向当前 rtp_rx_sock 为空的那项
   ...
   /*初始建立rtp_proxytable记录的时候，也需要将remote_ipaddr和remote_port等记录下来*/
   ...
   */
   ...
   for (i=0; i<RTPPROXY_SIZE; i++) {
      ...
      if (rtp_proxytable[i].rtp_rx_sock &&
         ...
         /*当wan_port为-1时，表明是强制清空rtp_proxytable表项，而不是cancel导致的清除remote_info信息*/
         if (wan_port != -1)
         {
             for (k = 0; k < MAX_BRANCH_SIZE; k++)
             {
                DEBUGPC("stop rtp_proxytable[%d].remote_info[%d].wan_port:%d", i, k, rtp_proxytable[i].remote_info[k].wan_port);
                /*当invite fork的情况下收到cancel，会导致的rtp_relay_stop_fwd，此时需要清空对应的remote_info表项，
                后续再根据是否所有的remote_info都清空了，才会真正的关闭对应的sock*/
                if (rtp_proxytable[i].remote_info[k].wan_port == wan_port)
                {
                    DEBUGPC("stop rtp_proxytable[%d].remote_info[%d].wan_port:%d", i, k, rtp_proxytable[i].remote_info[k].wan_port);
             ...
             for (k = 0; k < MAX_BRANCH_SIZE; k++)
             {
                /*只要有一个remote_info表项的wan_port没有清空，就不关闭sock，防止invite fork的情况下收到分支的cancel将已建立的通话的rtp断开*/
                if (rtp_proxytable[i].remote_info[k].wan_port != 0)
                {
