...
#define MAX_SIPVIA_RECORD  50  //via全局链表的最大长度
#define MAX_SIPVIA_NUM  5     //每个sip报文里via的最大个数
typedef struct tagsip_via_t
{
    char *callid_tmp;  //存sip报文里的call id
    osip_via_t *via[MAX_SIPVIA_NUM];//存sip报文里的via 
    int via_num;  //每个sip报文里via的个数
	int sip_csep; //存储sip报文的序列号
}sip_via_t;
...
sip_via_node  *g_pstSipViaList = NULL;  //用于存储via的全局链表
int g_SipViaNum = 0;   //记录全局链表当前的长度
...
int proxy_request (sip_ticket_t *ticket) 
{
   ...
   int port;
   char *buffer;
   osip_message_t *request;      //  收到的报文
   struct sockaddr_in *from;     //  收到报文的源地址
  ...
  /* start of AU4D01936 by d00107688 2009-06-19 sip电话通过pppoe proxy接入网关获取地址，sip alg正常但通话过程中，网关开端口出错，导致RTP流不能互通 */
   ...
   request=ticket->sipmsg;         //  收到的报文
   from=&ticket->from;             //  收到报文的源地址
...
#define _OLD_DIRECTION_EVALUATION 0
   ...
   if (type == 0) 
   {
      for (i=0; i<URLMAP_SIZE; i++) 
      {
         ...
         {
            /* 对于第一个注册报文, 在 sts = register_client(&ticket, 1) 中已经处理urlmap[i]
            ...
            /* 如果数据包来自于 conntact 者，也就是是注册者发起的请求 */
            ...
            {
               // 如果urlmap[i].true_url主机ip 和 收到报文的源ip 相同 则为out方向
               type=REQTYP_OUTGOING;
   ...
   if (type == 0) 
   {
      ...
      /*为适配W724v 的sip server(报文里to url不带端口),优先比较request url,request url没有匹配上，再比较to url*/
      /* 解决不同pc注册同一个号码，从外部呼入此号码，网关转到lan侧错误的pc,没有根据wan侧端口匹配来转发*/
      if (i >= URLMAP_SIZE)
      {
          for (i=0; i<URLMAP_SIZE; i++) 
          {
...
/* Start of sipalg by c47036 20070429 */
   if (type == 0)
   {
      ...
      /* 2009-06-18 start of d00107688  AU4D01899  挂SIP电话通话过程中，去使能/使能sip alg后，sip电话挂机后再次进行sip呼叫，在收到对方200OKSDP时出现进程退出 */
...
#endif
      /* 2009-06-18 end of d00107688  AU4D01899  挂SIP电话通话过程中，去使能/使能sip alg后，sip电话挂机后再次进行sip呼叫，在收到对方200OKSDP时出现进程退出 */
...
/* start of by d00107688 2009-05-25 AU4D01736 下挂SIP电话通话过程中，去使能/使能sip alg后，sip电话挂机会导致sip alg进程退出 */
         return STS_FAILURE;
/* end of by d00107688 2009-05-25 AU4D01736 下挂SIP电话通话过程中，去使能/使能sip alg后，sip电话挂机会导致sip alg进程退出 */
      }
      ...
      if (i >= URLMAP_SIZE)
      {
         ...
         if (type == REQTYP_OUTGOING)
         {
            ...
            DEBUGPC("type (OUT) so will create a url map");
             ...
             /*需要free掉原先的内存，然后重新malloc分配新的内存来存放host和port，否则空间可能不够*/
             ...
             osip_uri_clone( ticket->sipmsg->to->url, 
                    &urlmap[i].reg_url);
             /*在已经注销完毕的情况下，如果再次收到注销报文，防止由于to_ip的赋值导致后面查找sendto_addr时进入HG522的分支
             从而不是直接获取的目的ip，而是重新进行dns解析得到sendto_addr的ip*/
             ...
             /*hometalk app在注册ok的情况下，如果什么都不修改再次注册的时候，会用原来的端口(如1880)注销一遍，然后用另外一个端口(如36519)再注销一遍，
             但是第二次注销的via(1880)与contact(36519)和报文的源地址(36519)中的端口不一致，还是用的原来的端口(1880)，这样就需要记住这个报文的源port和ip，
             后面在转发响应的时候就往ticket->from的ip和port转发*/
 ...
 */
   ...
   if (type == REQTYP_INCOMING) 
   {
      ...
      /*w724v对接过程中发现hometalk app对接不成功，因为Record-Route中的port带错了，lan侧应该为5060，wan侧应该为实时分配的端口，56005-56nnn*/
...
//          sts = get_ip_by_host(urlmap[Hg522].reg_url->host, &sendto_addr);
   ...
   if (type == REQTYP_INCOMING)
   {
      /*w724对接过程中发现hometalk app不接受带lan侧ip的via头域*/
#if 0
   ...
   else
   {
		...
		//因为前面将网关wan的via加了进来，且加到了首个位置0，所以删除lan侧的via要从1的位置开始删
		pos = 1;
		//将lan侧的所有via都删掉，只保留网关加的wan的via
        ...
        if (g_SipViaNum > MAX_SIPVIA_RECORD)
        {
            ...
            if ((NULL != pstCurNode) && (NULL != pstPreNode))
        	{
				...
				if (NULL != pstCurNode->stSaveSipVia.callid_tmp) //清call id
                {
                    osip_free(pstCurNode->stSaveSipVia.callid_tmp);
                    pstCurNode->stSaveSipVia.callid_tmp = NULL;
                }
				
				FreeSipVia(&(pstCurNode->stSaveSipVia)); //清空via
...
int proxy_response (sip_ticket_t *ticket, int fd) {
   ...
   struct sockaddr_in *from;          //  报文源ip地址
   int tmp_port = 0;
   int status_code = 0;
   struct timeval stTm;
   ...
   if (fd != sip_udp_lan_socket)
   {
       for (i = 0; i < URLMAP_SIZE; i++)
       {
           if ((fd == urlmap[i].wan_socket_fd) && (1 == urlmap[i].active))
           {
               ...
               /*后面判断type = RESTYP_INCOMING有的是根据response->from->url与urlmap表项比较，在forking的情况下会找错
               因此直接通过socket来判断index*/
               index = i;
   ...
   {
       /*LAN侧只开了一个端口56005(配置文件),PC所有到WAN的5060都重定向了到此端口*/
   ...
   if (sts == STS_FAILURE) {
      ...
      /*w724对接过程中服务器和hometalk app均不接受带lan侧地址的via头域，因此去掉local via的时候失败也要继续处理*/
      //return STS_FAILURE;
   }
    ...
    DEBUGPC("come here, incoming packet......");
      ...
      if (MSG_IS_RESPONSE_FOR(response,"REGISTER")) 
      {
        DEBUGPC("response for register ");
          ...
          */
         //根据200 OK的expire更新url_map的expire
         if (MSG_IS_STATUS_2XX(response))
         {
             //contact header的expire参数比 expire header优先级高
                   ...
                   ticket->sipmsg->contacts->node->element)
             {
                 ...
                 /*ims有可能在200ok响应中contact没有将对应的register报文中的contact放到第一条，
                 而将其他的forking的客户端的contact放到第一条了，因此需要进行查找对应的contact*/
                 int pos = 0;
                 while (!osip_list_eol (ticket->sipmsg->contacts, pos)) {
         ...
         /*在forking的情况下，需要利用没有改ip的wan侧的contact来判断哪一条urlmap需要刷新，
         因此sip_rewrite_contact放到刷新expires后进行*/
 ...
 */
   ...
   */
   ...
   //判断是WAN侧进来的status code 400~699,而且是对REGISTER的response,删urlmap
   //说明PC注册失败,此map已无用
   ...
   if ((RESTYP_INCOMING == type) && (index >= 0) && (MSG_IS_RESPONSE_FOR(response,"REGISTER")))
   {
       ...
       {
           /*urlmap[i].expires不能立即设置为0，否则收到401需要鉴权时，
           如果带鉴权信息的第二个register没有及时收到，又没有其他报文的时候urlmap会被register_agemap清掉，
           从而第二次的register会更换wan端口56nnn，导致服务器认为不是刷新的注册或不是对应鉴权的重注册，回复403*/
           time_t time_now;
           ATP_UTIL_GetSysTime(&stTm);
...
/* end of AU4D01936 by d00107688 2009-06-19 sip电话通过pppoe proxy接入网关获取地址，sip alg正常但通话过程中，网关开端口出错，导致RTP流不能互通 */
{
   ...
   /* start of AU4D00864 by d00107688 2008-10-17 解决HG323和SIP电话采用非快启无法通话问题 */
    char acSdpCaddr[HOSTNAME_SIZE] = {0};
    /* end of AU4D00864 by d00107688 2008-10-17 解决HG323和SIP电话采用非快启无法通话问题 */
...
{ /* just dump the buffer */
   ...
   if (local_ip == NULL) {
     ...
     /* add by d00107688 如果是代理那么需要选择合适的ppp接口 */
   ...
   if (sdp->c_connection==NULL || sdp->c_connection->c_addr==NULL) {
      ...
      while (!sdp_message_endof_media(sdp, media_stream_no))
      {
         ...
         media_stream_no++;             /* 统计出具体开了几个 媒体 端口 */
      } /* while */
   }
   ...
   for (media_stream_no=0;;media_stream_no++)
   {
      ...
      //  对media & audio 电话不通，此处 sdp_conn = NULL
      ...
      if (sdp_message_m_port_get(sdp, media_stream_no))
      {
         ...
         if (msg_port > 0)
         {
            ...
            if (have_c_media == 0)
            {
               //  如果 m= 选项中有 c_connection项，则取该值
               //  否则取  c= 选项中的 connection address值
			...
			/* DT c40163 modify for QOS
            sts = rtp_start_fwd(osip_message_get_call_id(mymsg),  //  call id 值
                                client_id,         //  应该是  contact 中的username值
                                                   //  如果 username为空，则为 host值
                                rtp_direction,     //  应该是  in
                                media_stream_no,   //  从0开始递增
                                map_addr,          //  对于dir=out 则为wan侧地址
                                &map_port,         //  传出来的参数
                                addr_media,    // 如上 c= 选项中的connection address值
                                msg_port           //  m= 选项中的port值
                                );
            */
            sts = rtp_start_fwd(osip_message_get_call_id(mymsg),  //  call id 值
                                client_id,         //  应该是  contact 中的username值
            ...
            if (sts == STS_SUCCESS)
            {
               ...
               //  sdp_med 指向当前 media_stream_no 对应 m= 指针
