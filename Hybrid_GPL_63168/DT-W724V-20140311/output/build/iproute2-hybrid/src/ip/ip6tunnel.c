...
{
	fprintf(stderr, "Usage: ip -f inet6 tunnel { add | change | del | show | idle } [ NAME ]\n");
...
static int do_show_idle(int argc, char **argv)
{
    unsigned long idle = 0;
	int err;
    
	err = tnl_idle_ioctl(argv[0], SIOCTUNNELGETIDLE, &idle);
...
int do_ip6tunnel(int argc, char **argv)
{
	...
	if (argc > 0) {
        ...
        if (matches(*argv, "idle") == 0)
            return do_show_idle(argc-1, argv+1);
		if (matches(*argv, "help") == 0)
			usage();
	} else
