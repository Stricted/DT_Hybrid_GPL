 ...
 *  Copyright (C) Jean Fran莽ois Micouleau      1998-2000,
...
#ifdef SUPPORT_ATP_SAMBA_PRINT_SIZE_LIMIT
#define MAXFILENUM 10
#define PRINTNAMELEN 64
#define MAXSPOOLBUF (43*1024*1024)
#define MAXSIGLEFILESIZE (40*1024*1024)
#define MEMSIZEGAP (2*1024*1024)

struct job_file{
    uint32 uljobid;
    uint32 ulfilesize;
    char    aszprintname[PRINTNAMELEN];
    bool    bIsUsed;
}ST_job_file;
uint32 g_spooltakensize = 0;
struct job_file g_jobfilelist[MAXFILENUM];

uint32 g_lastspooltakensize = 0;
#define MAXPROCESSNUM 10
#define STR_LEN_256 256
#define GLOBESTATFILE "/var/spooltakemem"
#endif
/********************************************************************
...
#ifdef SUPPORT_ATP_SAMBA_PRINT_SIZE_LIMIT
/*!
  \brief 检查/var/spool/cups/下文件的总大小
  \param[in]  jobid: jobid
  \param[in]  pulTotolSize: /var/spool/cups/总大小 
  \return 
  \retval
*/
uint32 getGlobeSize(int snum,uint32 *pulTotolSize,uint32 jobid)
{
    char acStringTemp[STR_LEN_256] = {0};
    int32 lPid = getpid();
    FILE *pfGlobeStatFile = NULL;
    char acPermission[STR_LEN_256] = {0};
    char acLastStr[STR_LEN_256] = {0};
    int32  lTemp[3]= {0};
    uint32  ulFileSize= {0};
    char acFileName[STR_LEN_256] = {0};
    char * pPath = NULL;

    snprintf(acFileName,STR_LEN_256,"%s%d%d",GLOBESTATFILE,lPid,jobid);
    pPath =  lp_pathname(snum);
    DBGYJ("[%s][%d]path = %s\r\n",__FILE__,__LINE__,pPath);
    if (NULL == pPath)
    {
        snprintf(acStringTemp,STR_LEN_256,"ls -l /var/spool/cups > %s",acFileName);
    }
    else
    {
        snprintf(acStringTemp,STR_LEN_256,"ls -l %s > %s",pPath,acFileName);
    }
    system(acStringTemp);
    
    pfGlobeStatFile = fopen(acFileName,"r");  
    if (NULL == pfGlobeStatFile) 
    {
        return False;
    }
    while (NULL != fgets(acStringTemp,STR_LEN_256,pfGlobeStatFile))
    {
        //-rw-------    1 0        0         2054572 Jan 31 11:16 smbprn.00000024.t4QrU7
        sscanf(acStringTemp,"%s %d %d %d %u %s",acPermission,&lTemp[0],&lTemp[1],&lTemp[2],&ulFileSize,acLastStr);
        DBGYJ("[%s][%d]%u\r\n",__FILE__,__LINE__,ulFileSize);
        if (0 !=ulFileSize)
        {
            *pulTotolSize += ulFileSize;
        }
    }
    fclose(pfGlobeStatFile);
    remove(acFileName);
    
    return True;
}
/*!
  \brief 检查/var/spool/cups/下文件的总大小，当总大小超过限制时则终止此次打印
  \param[in]  jobid: jobid
  \return 
  \retval
*/
uint32 checkGlobeStat(int snum,uint32 jobid)
{
    uint32 ulTotolSize = 0;
    
    if (False == getGlobeSize(snum,&ulTotolSize,jobid))
    {
        return False;
    }
        DBGYJ("[%s][%d]~~~ulTotolSize=%u\r\n",__FILE__,__LINE__,ulTotolSize);
    /*当此次要写的数据+已存的数据>总数据大小限制时则不允许再写入数据
    并且把此文件删除，因为需要释放此文件资源*/    
    if (g_spooltakensize - g_lastspooltakensize + ulTotolSize > MAXSPOOLBUF)  
    {
        printf("The jobid %d shoud stop because totol size is up to %u!\r\n",jobid,g_spooltakensize - g_lastspooltakensize +ulTotolSize);
        return False;
    }
    return True;
}

uint32 endjobidfile(char *sharename,uint32 jobid)
{
    uint32 i = 0;
    if (NULL == sharename)
    {
        printf("sharename is null,cant delete!!!!!job %d cause globle filesize %d \r\n",jobid,g_spooltakensize);
        return False;
    }
    for(i=0;i<MAXFILENUM;i++)
    {
        if ((g_jobfilelist[i].uljobid == jobid) && !strcmp(g_jobfilelist[i].aszprintname,sharename))
        {
            g_spooltakensize -= g_jobfilelist[i].ulfilesize;
            g_lastspooltakensize = g_spooltakensize;
            g_jobfilelist[i].bIsUsed = 0;
            g_jobfilelist[i].ulfilesize = 0;
            //g_jobfilelist[i].uljobid = 0;
            memset(g_jobfilelist[i].aszprintname,0,PRINTNAMELEN);
#if 0
            printf("[%s][%d]endjobidfile!!!!!!g_spooltakensize=%u,g_lastspooltakensize=%u\r\n",__FILE__,__LINE__,g_spooltakensize,g_lastspooltakensize);
            for(i=0;i<MAXFILENUM;i++)
            {
                if ((g_jobfilelist[i].bIsUsed == 1))
                {
                    printf("[%s][%d]endjobidfile!!!!!g_jobfilelist[%d].uljobid=%d,g_jobfilelist[%d].ulfilesize=%u,g_jobfilelist[%d].bIsUsed=%d\r\n",__FILE__,__LINE__
                        ,i,g_jobfilelist[i].uljobid,i,g_jobfilelist[i].ulfilesize,i,g_jobfilelist[i].bIsUsed);
                }
            }    
#endif
...
uint32 checkaddtofilelist(int snum,char *sharename,uint32 jobid, uint32 size)
{
    uint32 i = 0;
    if (NULL == sharename)
    {
        printf("sharename is null, can't add!!!!!job %d cause globle filesize %d \r\n",jobid,g_spooltakensize);
        return False;
    }
    for(i=0;i<MAXFILENUM;i++)
    {
        if (( 1 == g_jobfilelist[i].bIsUsed)&&(g_jobfilelist[i].uljobid == jobid) && !strcmp(g_jobfilelist[i].aszprintname,sharename))
        {
            g_jobfilelist[i].ulfilesize += size;
            g_spooltakensize +=size;
            if (g_spooltakensize >= MAXSPOOLBUF)
            {
                printf("%s job %d cause globle filesize %d > %d\r\n",sharename,jobid,g_spooltakensize,MAXSPOOLBUF);
                endjobidfile(sharename,jobid);
                return False;
            }
            else
            {
                if (g_jobfilelist[i].ulfilesize > MAXSIGLEFILESIZE)
                {
                    printf("%s jobid %d filesize %d > %d\r\n",sharename,jobid,g_jobfilelist[i].ulfilesize,MAXSIGLEFILESIZE);
                    endjobidfile(sharename,jobid);
                    return False;
                }
                /*start add by y00181549 at 2013/1/30 for multi-user printing*/
                if (g_spooltakensize - g_lastspooltakensize > MEMSIZEGAP)
                {
                    if ( False == checkGlobeStat(snum,jobid))//if this write of file will cause the globe size over flow, this job should stop.
                    {
                        endjobidfile(sharename,jobid);
                        return False;
                    }
                ...
                /*end add by y00181549 at 2013/1/30 for multi-user printing*/
                return True;
            }
        }
    }
    if ( MAXFILENUM == i)
    {
        /*规避对于xp系统下任务被终止了，但是还能接收的情况*/
        for(i=0;i<MAXFILENUM;i++)
        {
            if (( 0 == g_jobfilelist[i].bIsUsed) && (g_jobfilelist[i].uljobid== jobid))//tihis job is closed before, this come in is wrong. so do not accecpt
            {
                DBGYJ("jobid %d is delete before, so not accecpt",jobid);
                return False;
            }
        }
        /*规避对于xp系统下任务被终止了，但是还能接收的情况*/
        for(i=0;i<MAXFILENUM;i++)
        {
            if (( 0 == g_jobfilelist[i].bIsUsed))
            {
                g_jobfilelist[i].bIsUsed = 1;
                g_jobfilelist[i].uljobid = jobid;
                g_jobfilelist[i].ulfilesize += size;
                snprintf(g_jobfilelist[i].aszprintname,PRINTNAMELEN,"%s",sharename);
                g_spooltakensize +=size;
                if (g_spooltakensize >= MAXSPOOLBUF)
                {
                    printf("%s job %d cause globle filesize %d > %d\r\n",sharename,jobid,g_spooltakensize,MAXSPOOLBUF);
    ...
    printf("there is already %d files\r\n",MAXFILENUM);
    return False;
}
#endif
/********************************************************************
...
WERROR _spoolss_writeprinter(pipes_struct *p, SPOOL_Q_WRITEPRINTER *q_u, SPOOL_R_WRITEPRINTER *r_u)
{
	...
	if (!get_printer_snum(p, handle, &snum))
        ...
        if (checkaddtofilelist(snum,sharename,Printer->jobid,buffer_size) == False)
        {
              DBGYJ("WERR_NO_SPOOL_SPACE,jobid =%d ,g_spooltakensize=%d",Printer->jobid,g_spooltakensize);
        	//print_job_end(snum, Printer->jobid,ERROR_CLOSE);
                _spoolss_enddocprinter(p, q_u,r_u);
                return -1;
        }
        DBGYJ("%s Printer->jobid=%d,n=%d,g_spooltakensize=%d",sharename,Printer->jobid,buffer_size,g_spooltakensize);
#endif
