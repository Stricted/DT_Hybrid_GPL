...
static BOOL do_section(const char *pszSectionName)
{
	...
	if (bRetval) {
		...
		if ((iServiceIndex = add_a_service(&sDefault, pszSectionName))
			...
			if(( NULL!=pszSectionName)&&(0 ==strcmp(pszSectionName,GUESTNAME))){
...
/*START ADD BY y00181549 for DTS2013082709660, ipv6 download with "use sendfile = yes" is not possible under 3.4.11 kernel at 2013/8/27*/
	return (_lp_use_sendfile(snum) && (get_remote_arch() != RA_WIN95) && !srv_is_signing_active() && (False == bIsFromIPV6));
/*END ADD BY y00181549 for DTS2013082709660, ipv6 download with "use sendfile = yes" is not possible under 3.4.11 kernel at 2013/8/27*/
