...
/*added by qichenghong 20091020 */
#ifdef CUT_SMBPASSWD
#include "smbpasswd_proc.h"
#endif
/*added by qichenghong 20091020 end*/
...
{
/*added by qichenghong 20091021 */
#ifdef CUT_SMBPASSWD
	printf("Run only in format : smbpasswd -a username password");
#else
...
#endif
/*added by qichenghong 20091021 end*/
...
/*added by qichenghong 20091021 */
	...
	/*added by qichenghong 20091014 for command "smbpasswd -a username password"*/
	case 2:
		if (!(local_flags & LOCAL_AM_ROOT)) {
			usage();
		} else {
			...
			/*	fstrcpy(user_name, argv[0]);*/
DebugPrintLine(2);
				//memcpy((void *)user_name,(void *)(argv[0]),sizeof(fstring)-1);
				//memcpy((void *)user_name,(void *)(argv[0]),FSTRING_LEN-1);
//printf(" \n argv[0] = %s \n",argv[0]);
				smb_strcpy(user_name,argv[0]);
//printf(" user name = %s \n",user_name);

DebugPrintLine(2);
				new_passwd = smb_strdup(argv[1]);
DebugPrintLine(2);
                        /*暂时去掉字符相关*/
                        /*这里导致samba无法支持大写字符密码设置*/
                        #if 0
                        strlower_m_smbpw(new_passwd);
                        #endif
DebugPrintLine(2);
		...
		break;
	/*added by qichenghong 20091014 for command "smbpasswd -a username password" end*/
	default:
		usage();
	}
...
{
/*changed by qichenghong 20091019 */
#ifndef CUT_SMBPASSWD
	char *p;
	fstring new_pw;
	...
	return NULL;
#endif
/*changed by qichenghong 20091019 end*/
}
...
{
	NTSTATUS ret;
	char err_str[PSTRING_LEN];
	char msg_str[PSTRING_LEN];
...
/*added by qichenghong 20091019 */
		...
		return ret;
#endif
/*added by qichenghong 20091019 end */
	}
DebugPrintLine(2);
/*added by qichenghong 20091020 */
#ifdef CUT_SMBPASSWD
	ret = local_password_add(username, local_flags, new_pw , err_str, sizeof(err_str), msg_str, sizeof(msg_str));
#else
	ret = local_password_change(username, local_flags, new_pw, err_str, sizeof(err_str), msg_str, sizeof(msg_str));
#endif
/*added by qichenghong 20091020 end */
	if('\0' != msg_str[0])
		printf("%s", msg_str);
