...
/*	TALLOC_FREE( pwd );*/
	...
	if (local_flags & LOCAL_SET_PASSWORD) {
        ...
        {
			snprintf(p_err_str, ulErrStrSize - 1, "Failed to set password for user %s.\n", user_name); 
            DebugPrintLine(2);
    ...
    {
		snprintf(p_msg_str, ulMsgStrSize - 1, "Added user %s.\n", user_name);
		DebugPrintLine(2);
		return NT_STATUS_OK;
	} 
    else 
    {
		snprintf(p_err_str, ulErrStrSize - 1, "Failed to add entry for user %s.\n", user_name);
		DebugPrintLine(2);
		return NT_STATUS_UNSUCCESSFUL;
	}
