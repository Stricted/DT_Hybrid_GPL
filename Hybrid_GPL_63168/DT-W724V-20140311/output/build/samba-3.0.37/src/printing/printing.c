...
extern uint32 endjobidfile(char *sharename,uint32 jobid);
#endif
...
#else
    struct printif *printer_fns =NULL;
#endif
#ifdef HAVE_CUPS
	if ( type == PRINT_CUPS ) {
...
BOOL print_job_end(int snum, uint32 jobid, enum file_close_type close_type)
{
	...
	if (pjob->spooled || pjob->pid != sys_getpid())
        ...
        DBGYJ("end job %s,%d, g_spooltakensize=%d",sharename,jobid,g_spooltakensize);
#endif
