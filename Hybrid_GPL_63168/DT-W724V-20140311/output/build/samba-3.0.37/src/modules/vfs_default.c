...
//deleted by qichenghong 20100112
//changed by sKF19819:20100512:for SAMBA coulden`t do .txt type file content deleteing operation
       SMB_VFS_FSTAT(fsp, fd, &sttmp);
	if(sttmp.st_size >= len)
	{
		   result = sys_ftruncate(fsp->fh->fd, len);
		   if (result == 0)
		   {
			  printf("=file %s line %d function %s len=%d\n", __FILE__, __LINE__, __FUNCTION__, (int)len);
			  goto done;
		   }
	}
//changed by sKF19819:20100512:for SAMBA coulden`t do .txt type file content deleteing operation
...
//		goto done;//deleted by qichenghong 20100112 for replace file or folder
	}

	if (SMB_VFS_LSEEK(fsp, fd, len-1, SEEK_SET) != len -1)
		goto done;

//delete this by qichenghong 20100112
