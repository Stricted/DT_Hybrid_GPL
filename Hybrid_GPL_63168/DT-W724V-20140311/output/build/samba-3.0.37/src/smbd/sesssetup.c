...
#ifdef SUPPORT_ATP_SAMBA_GUEST //add by y00181549 for samba guest 账户，用户名"guest"，密码任意字符
              if((True == there_is_guest_account) && (NULL != user)&&(!(strcmp(GUESTNAME,user))))
#else
			  ...
			  int length,int bufsize)
{
	...
	if (!*user) {
        /*modify by l0193145 for HOMEGW-14392 linux下不能用匿名登陆 2012.12.26*/
#ifdef SUPPORT_ATP_SAMBA_ANONYMOUS
        nt_status = check_guest_password(&server_info);
#else
        return ERROR_NT(nt_status_squash(NT_STATUS_LOGON_FAILURE));
#endif
        /*end modify by l0193145 for HOMEGW-14392 linux下不能用匿名登陆 2012.12.26*/
