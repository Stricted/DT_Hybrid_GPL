 ...
 int main(int argc, char *argv[])
{
    ...
    /*begin of add by y00181549 for DTS2013032906446在使能guest用户时需要将主smbd进程重启以加载新的smb.conf文件 at 20130329*/ 
#ifdef SUPPORT_ATP_SAMBA_GUEST
    {
        int iPid = -1;
        FILE * pSmbdPidFile = NULL;
        if ( 0 != access(SMBD_PROCESS_PID_FILE,F_OK))
        {
          pSmbdPidFile = fopen(SMBD_PROCESS_PID_FILE,"w+");
          if (NULL !=pSmbdPidFile)
          { 
              iPid = getpid();
              fprintf(pSmbdPidFile,"smbd:%d",iPid);
              fflush(pSmbdPidFile);
              fclose(pSmbdPidFile);
              DEBUG(0, ("notice: main smbd pid :%d",iPid));
          }
          else
          {
              printf("%d notice: can't creat %s \r\n",iPid,SMBD_PROCESS_PID_FILE);
          }
        }
        else
        {
          DEBUG(0, ("%d notice: %s is exist!\r\n",getpid(),SMBD_PROCESS_PID_FILE));
        }
    }
#endif
    /*end of add by y00181549 for DTS2013032906446在使能guest用户时需要将主smbd进程重启以加载新的smb.conf文件 at 20130329*/
#if HAVE_SETPGID
	/*
	...
	message_register(MSG_SMB_FORCE_TDIS, msg_force_tdis, NULL);
       ...
       /*START ADD BY y00181549 for DTS2013082709660, ipv6 download with "use sendfile = yes" is not possible under 3.4.11 kernel at 2013/8/27*/
       check_fd_ipv6();
       /*END ADD BY y00181549 for DTS2013082709660, ipv6 download with "use sendfile = yes" is not possible under 3.4.11 kernel at 2013/8/27*/
