...
extern uint32 g_spooltakensize;
extern uint32 checkaddtofilelist(int snum,char *sharename,uint32 jobid, uint32 size);
#endif
...
#ifdef SUPPORT_ATP_SAMBA_PRINT_SIZE_LIMIT
        if (checkaddtofilelist(SNUM(fsp->conn),sharename,jobid,n) == False)
        {
            DBGYJ("WERR_NO_SPOOL_SPACE,sharename=%s, jobid =%d ,g_spooltakensize=%u",sharename,jobid,g_spooltakensize);
            print_fsp_end(fsp, ERROR_CLOSE);
            return -99;
        }
        DBGYJ("sharename=%s,Printer->jobid=%d,n=%d,g_spooltakensize=%u,pos=%d",sharename,jobid,n,g_spooltakensize,pos);
#endif
