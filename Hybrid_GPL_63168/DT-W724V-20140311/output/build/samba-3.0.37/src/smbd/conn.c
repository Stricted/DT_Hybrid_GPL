...
BOOL conn_snum_used(int snum)
{
	...
	for (conn=Connections;conn;conn=conn->next) {
        /* Modified by y00183935@2013.11.20 for CVE-2013-0454 */
        /* Modify nothing, just make a record. */
        /* Modified by y00183935@2013.11.20 for CVE-2013-0454 */
...
/****************************************************************************
process the /var/samba/smbd_user_pid file,include add or delete
flag ==SMBD_USER_PID_ADD : add one pid
flag ==SMBD_USER_PID_DEL : delete one pid
****************************************************************************/
void process_smbd_user_pid(int flag, char *user,int pid)
{

    FILE *pstPid = NULL;
    char acFileBuffer[SMBD_MAX_BUFFER_LEN];
    char acPid[SMBD_USER_PID_CMD_LEN+SMBD_USER_PID_CMD_LEN];
    int strLen =0;
    char *pcDest = NULL;
    memset(acFileBuffer,0,sizeof(acFileBuffer));
    memset(acPid,0,sizeof(acPid));
    if((SMBD_USER_PID_ADD != flag) && (SMBD_USER_PID_DEL != flag))
    {
          DEBUG(2,("pass the wrong flag to process pid.\n"));
          return ;
    }

    become_root();
    if(0 != access(SMBD_USER_PID_FILE, F_OK))
    { 
        DEBUG(2,("pass the wrong flag to process pid.\n"));
        unbecome_root();

        if(SMBD_USER_PID_DEL == flag)
        {            
            return ;
        }
        
        sprintf(acFileBuffer,"0=0%s",":");        
        become_root();
        pstPid = fopen(SMBD_USER_PID_FILE, "w");
        unbecome_root();
        if(NULL== pstPid)
        {
            DEBUG(2,("open file error.\n"));
            return ;          
        } 

        fprintf(pstPid, acFileBuffer);
        fflush(pstPid);
        fclose(pstPid);
    ...
    become_root();
    pstPid= fopen(SMBD_USER_PID_FILE, "r");
    unbecome_root();
    if(NULL== pstPid)
    {
        DEBUG(2,("open file error.\n"));
    ...
    fgets(acFileBuffer, SMBD_MAX_BUFFER_LEN, pstPid);
    fclose(pstPid);
    if(SMBD_USER_PID_ADD == flag)
    {
        sprintf(acPid,"%d",pid);
        if(NULL == strstr(acFileBuffer,acPid))
        {
            become_root();
            pstPid= fopen(SMBD_USER_PID_FILE, "r");            
            unbecome_root();
            ...
            sprintf(acPid,"%s=%d%s",user,pid,":");
            pcDest = acFileBuffer;
            safe_strcat(pcDest,acPid,strlen(acPid)+strlen(acFileBuffer));            
            become_root();
            pstPid = fopen(SMBD_USER_PID_FILE, "w");
            unbecome_root();
            if(NULL== pstPid)
    ...
    else if(SMBD_USER_PID_DEL == flag)
    {
        sprintf(acPid,"%d",pid);
        if(NULL != strstr(acFileBuffer,acPid))
        {
            char *pcPtr = NULL;
            char *pcToken = NULL;
            char acTmpBuff[SMBD_MAX_BUFFER_LEN];
            memset(acTmpBuff,0,sizeof(acTmpBuff));
            pcToken = strtok_r(acFileBuffer, ":", &pcPtr);
            if(NULL != pcToken)
            {
                if(NULL == strstr(pcToken,acPid))
                {
                    pcDest = acTmpBuff;
                    safe_strcat(pcDest,pcToken,strlen(pcToken)+strlen(acTmpBuff));
                    acTmpBuff[strlen(acTmpBuff)]=':';
                }
                else
                {
