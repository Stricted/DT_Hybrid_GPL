...
static int ScanDirFilterEx(const struct dirent *dir)
{
			...
			SMB_BIG_UINT *dsize)
{
    ...
    {
        if ( ( 0x01021994 == (statfsbuf.f_type) ) || ( 0x7275 == (statfsbuf.f_type) ) || ( 0x858458f6 == (statfsbuf.f_type) ) )//tmpfs and romfs
        {
            memorypath = 1;
        }
    }
    DEBUG ( 0, ( "get_dfree_info: memorypath is %d \n", memorypath ) );

    if( 1 == memorypath )
    {
        SMB_BIG_UINT bsizeTmp;
        SMB_BIG_UINT dfreeTmp;
        SMB_BIG_UINT dsizeTmp;
        char dirpathTmp[512];
		char *ldirpath = NULL;
        ...
        *dfree=0;
        *dsize=0;
        n = scandir( ".", &namelist, ScanDirFilterEx, NULL );
        if (n >= 0)
        {
            while (n--) 
            {
                snprintf(dirpathTmp,sizeof(dirpathTmp),"%s",namelist[n]->d_name);
    
                DebugPrintLine(3);
                ldirpath=NULL;
                string_append(&ldirpath , "./");
                string_append(&ldirpath , dirpathTmp);
                if(ldirpath )
                {
                    //printf("@line %d =========ldirpath==[%s]\n",__LINE__,ldirpath);
                    DebugPrintStr(3, ldirpath);
                    dfree_ret = SMB_VFS_DISK_FREE(conn,ldirpath,small_query,&bsizeTmp,&dfreeTmp,&dsizeTmp);
                    if(dfree_ret != (SMB_BIG_UINT)-1)
                    {
                        ...
                        *dfree=*dfree+dfreeTmp;
                        *dsize=*dsize+dsizeTmp;
                        //printf("===== : SMB_QUERY_FS_SIZE_INFO bsize=%u, cUnitTotal=%u, cUnitAvail=%d\n", (unsigned int)bsizeTmp, (unsigned int)dsizeTmp, (unsigned int)dfreeTmp);
                    }
                    safe_free(ldirpath);
                }
                free(namelist[n]);
