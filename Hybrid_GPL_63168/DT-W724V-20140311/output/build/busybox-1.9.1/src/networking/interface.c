...
static void ife_print(struct interface *ptr)
{
	...
	printf("%-9.16s Link encap:%s  ", ptr->name, hw->title);
...
/* BRCM change, JIRA 10503 - add extended formats, accomodate print_bytes_scaled() change */
		...
		if (can_compress)
            ...
            // RX packet counts
            printf("          ");
            printf("RX packets:%llu multicast:%lu unicast:%lu broadcast:%lu unknown:%lu ",
                   ptr->stats.rx_packets, ptr->stats.rx_multicast,
                   ptr->stats.rx_unicast_packets, ptr->stats.rx_broadcast_packets, ptr->stats.rx_unknown_packets);
            if (can_compress)
                printf(" compressed:%lu\n", ptr->stats.rx_compressed);
            else
                printf("\n");
