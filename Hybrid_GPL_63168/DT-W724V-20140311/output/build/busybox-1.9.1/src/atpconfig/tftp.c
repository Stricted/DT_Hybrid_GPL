...
#define socketfd (pfd[0].fd)
	int len;
	int send_len;
	USE_FEATURE_ATP_TFTP_BLOCKSIZE(smallint want_option_ack = 0;)
	smallint finished = 0;
	uint16_t opcode;
	uint16_t block_nr = 1;
	uint16_t recv_blk;
...
int tftp_main(int argc, char **argv)
{
...
#else
	/* -p or -g is mandatory, and they are mutually exclusive */
	opt_complementary = "" USE_FEATURE_ATP_TFTP_GET("g:") USE_FEATURE_ATP_TFTP_PUT("s:")
			USE_GETPUT("?g--s:s--g");

	USE_GETPUT(cmd =) getopt32(argv,
			USE_FEATURE_ATP_TFTP_GET("g") USE_FEATURE_ATP_TFTP_PUT("s")"v" ZIP_OPT_CHAR
			    "u:p:r:l:P:B:A:" USE_FEATURE_ATP_TFTP_BLOCKSIZE("b:"),
			&username, &password,
			&remotefile, &localfile,
			&serverPort, &g_pcLocalIP, &serverIp
			USE_FEATURE_ATP_TFTP_BLOCKSIZE(, &sblocksize));
#endif
    #undef ZIP_OPT_CHAR
    ...
    system("iptables -t nat -A PRE_SERVICE -p udp --dport 37926 -j ACCEPT");
    system("iptables -A INPUT_SERVICE -p udp --dport 37926 -j ACCEPT");
	result = tftp( USE_GETPUT(cmd,) peer_lsa, remotefile, fd, port, blocksize);
    system("iptables -t nat -D PRE_SERVICE -p udp --dport 37926 -j ACCEPT");
    system("iptables -D INPUT_SERVICE -p udp --dport 37926 -j ACCEPT");
#endif
