...
int ps_main(int argc, char **argv)
{
	...
	if (use_selinux)
		...
		puts("  PID  Uid     Gid        VSZ Stat Command");
	...
	while ((p = procps_scan(p, 0
...
#endif
		{
            ...
            const char *group = get_cached_groupname(p->gid);
			if (p->vsz == 0)
				len = printf("%5u %-8s %-8s        %s ",
					p->pid, user, group, p->state);
			else
				len = printf("%5u %-8s %-8s %6lu %s ",
				/*Start of 维护管理组 2008-4-2 19:39 for 修改ps显示的vmsize大小 by handy*/
					p->pid, user, group, p->rss, p->state);
				/*End of 维护管理组 2008-4-2 19:39 for 修改ps显示的vmsize大小 by handy*/
		}
