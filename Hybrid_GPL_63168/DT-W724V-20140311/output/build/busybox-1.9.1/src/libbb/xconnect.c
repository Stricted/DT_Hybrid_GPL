...
#include "libbb.h"
#include "atpconfig.h"  /*added by w00182511,2011/7/23   PN:TR143功能需求开发*/
#include <net/if.h>
...
/* BEGIN: Modified by weiyue w00182511, 2011/7/23   PN:TR143功能需求开发*/
#ifdef SUPPORT_ATP_TR143
...
#endif
/* END:  Modified by weiyue w00182511, 2011/7/23 */
...
static int create_and_bind_or_die(const char *bindaddr, int port, int sock_type)
{
	...
	setsockopt_reuseaddr(fd);
       /*修改绑定IPv6链路地址问题--使用链路地址时，必须指定参数"sin6_scope_id"*/
       if ( AF_INET6 == lsa->sa.sa_family )
       {
            if (IN6_IS_ADDR_LINKLOCAL(&(((struct sockaddr_in6 *)(&(lsa->sa)))->sin6_addr)))
            {
                ((struct sockaddr_in6 *)(&(lsa->sa)))->sin6_scope_id = if_nametoindex("br0");
            }
        }
       /*修改绑定IPv6链路地址访问问题--使用链路地址时，必须指定参数"sin6_scope_id"*/
