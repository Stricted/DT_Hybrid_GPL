...
extern  int br_set_mld_snooping(int enable);
extern int br_show_mld_snooping(void);
...
extern  int br_show_igmp_snooping(void);
extern int br_set_igmp_snooping(int enable);
...
#ifdef SUPPORT_ATP_ATH_WRAP
extern int br_add_interface(const char *br, const char *dev,int ptype);
#else
...
extern int br_set_port_priority(const char *br, const char *p, 
				int port_priority);
extern int get_infname_by_portno(const char *bridge, const u_int16_t portno, char *ifname );
