...
/*
BRCTL_SET_MLD_SNOOPING与BRCTL_SHOW_MLD_SNOOPING
在内核版本版本linux/if_bridge.h中定义
在此定义，主要是为了让应用模块不依赖内核版本，而导致编译不通过。
*/
#ifdef SUPPORT_ATP_HYBRID
#define BRCTL_SET_MLD_SNOOPING  (34)
#define BRCTL_SHOW_MLD_SNOOPING (35)
#else
#define BRCTL_SET_MLD_SNOOPING  (25)
#define BRCTL_SHOW_MLD_SNOOPING (26)
#endif

int br_show_mld_snooping(void)
{
	int ret;

	{
		unsigned long arg[3] 
			= { BRCTL_SHOW_MLD_SNOOPING,0,0 };
...
int br_set_mld_snooping(int enable)
{
	int ret;

	{
		unsigned long arg[3] 
			= { BRCTL_SET_MLD_SNOOPING, 0, 0};
...
#else if defined (SUPPORT_ATP_BR_MLD_SNOOP)
...
/*
BRCTL_SET_IGMP_SNOOPING与BRCTL_SHOW_IGMP_SNOOPING
在内核版本版本linux/if_bridge.h中定义
在此定义，主要是为了让应用模块不依赖内核版本，而导致编译不通过。
*/
...
int br_show_igmp_snooping(void)
{
	int ret;

	{
		unsigned long arg[3] 
			= { BRCTL_SHOW_IGMP_SNOOPING,0,0 };
...
int br_set_igmp_snooping(int enable)
{
	int ret;

	{
		unsigned long arg[3] 
			= { BRCTL_SET_IGMP_SNOOPING, 0, 0};
...
#else if defined (SUPPORT_ATP_BR_IGMP_SNOOP)
...
int get_infname_by_portno(const char *bridge, const u_int16_t portno, char *ifname )
{
   int n=-1;
   int ret=0;
   ...
   if (ifindices[portno] == 0)
   {
      printf("ifindices[portid] is zero!, portid=%d", portno);
      return -1;
   }

   if (!if_indextoname(ifindices[portno], ifname))
   {
      printf("if_indextoname failed, ifindices[portid]=%d\n", ifindices[portno]);
      return -1;
   }

   if (strlen(ifname) == 0)
   {
      printf("empty portName?\n");
      ret = -1;
   }
   else
   {
