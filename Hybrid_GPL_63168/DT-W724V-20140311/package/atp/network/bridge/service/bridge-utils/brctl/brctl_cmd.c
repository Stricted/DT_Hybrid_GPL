...
static int br_cmd_addif(int argc, char *const* argv)
{
	...
	while (argc-- > 0) {
		...
		argc--;
		err = br_add_interface(brname, ifname,ptype);
#else
...
#endif /* SUPPORT_ATP_BR_IGMP_SNOOP */

#ifdef SUPPORT_ATP_BR_MLD_SNOOP
...
#endif /* SUPPORT_ATP_BR_MLD_SNOOP */
...
#define CHUNK 128
	...
	qsort(fdb, offset, sizeof(struct fdb_entry), compare_fdbs);
       //如果要改这个输出格式，记得修改atputil中根据mac取接口名的函数。ATP_UTIL_GetInterfaceNameByMacAddr
	printf("port no\tmac addr\t\tis local?\tageing timer\tinterface\n");
	for (i = 0; i < offset; i++) {
		const struct fdb_entry *f = fdb + i;
              get_infname_by_portno(brname,f->port_no,ifname);
		printf("%3i\t", f->port_no);
...
#ifdef SUPPORT_ATP_IGMP_SNOOPING
static int br_cmd_showigmpsnooping(int argc, char*const* argv) 
{
	int err;

	err = br_show_igmp_snooping();
	if (err)
		fprintf(stderr, "show igmp snooping failed: %s\n",
			strerror(errno));

	return err != 0;
}

static int br_cmd_setigmpsnooping(int argc, char*const* argv) 
{
	int err;
	int enable;
	...
	err = br_set_igmp_snooping(enable);
	if (err)
		fprintf(stderr, "enable igmp snooping failed: %s\n",
			strerror(errno));
...
#ifdef SUPPORT_ATP_MLD_SNOOPING
static int br_cmd_showmldsnooping(int argc, char*const* argv) 
{
	int err;

	err = br_show_mld_snooping();
	if (err)
		fprintf(stderr, "show igmp snooping failed: %s\n",
			strerror(errno));
...
static int br_cmd_setmldsnooping(int argc, char*const* argv) 
{
	int err;
	int enable;
	...
	err = br_set_mld_snooping(enable);
	if (err)
		fprintf(stderr, "enable igmp snooping failed: %s\n",
			strerror(errno));
...
// brcm end
#ifdef SUPPORT_ATP_IGMP_SNOOPING
        { 0, "showigmpsnooping", br_cmd_showigmpsnooping, 
          "<bridge>\tshow igmp snooping" },
        { 1, "enableigmpsnooping", br_cmd_setigmpsnooping, 
          "<enable>\t\tenable igmp snooping" },
#endif
    
#ifdef SUPPORT_ATP_MLD_SNOOPING
        { 0, "showmldsnooping", br_cmd_showmldsnooping, 
          "<bridge>\tshow mld snooping" },
        { 1, "enablemldsnooping", br_cmd_setmldsnooping, 
