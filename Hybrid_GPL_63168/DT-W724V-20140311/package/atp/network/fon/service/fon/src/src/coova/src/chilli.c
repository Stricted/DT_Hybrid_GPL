...
/*连到了 FON SSID的用户数，如果为0，OnDemand的PPPoE才允许超时后掉线*/
static int gFonSsidUsers = 0;
...
static void write_int_to_file(int num,  const char *file_path);
...
int runscript(struct app_conn_t *appconn, char* script) {  
  ...
  set_env("CLASS", VAL_STRING, appconn->s_state.redir.classbuf, strlen((char*)appconn->s_state.redir.classbuf));
...
int chilli_req_attrs(struct radius_t *radius, 
  ...
  if (state->redir.classbuf[0][0]) {
    log_dbg("RADIUS Request + Class");
    int i;
    for(i=0; i < NUMBER_OF_CLASS_ATTRS; i++) {
	if (state->redir.classbuf[i][0]=='\0')
		break;
	int len = strlen((char*)state->redir.classbuf[i]);
	log_dbg("adding class(%d) len: %d",i, len);
    	radius_addattr(radius, pack, RADIUS_ATTR_CLASS, 0, 0, 0,
		state->redir.classbuf[i], len);
    }
  }
...
int cb_radius_auth_conf(struct radius_t *radius, 
  ...
  /* Class */
  for(i=0;i < NUMBER_OF_CLASS_ATTRS; i++) {
   if(!radius_getattr(pack, &classattr, RADIUS_ATTR_CLASS, 0, 0, i)) {
     memcpy(appconn->s_state.redir.classbuf[i], classattr->v.t, classattr->l-2);
     appconn->s_state.redir.classbuf[i][classattr->l-1] = '\0';
   } else {
     int k;
     for(k=i; k<NUMBER_OF_CLASS_ATTRS; k++)
       appconn->s_state.redir.classbuf[i][0] = '\0';
     break;
   }
  }
...
int cb_dhcp_connect(struct dhcp_conn_t *conn) {
  ...
  gFonSsidUsers++;
  write_int_to_file(gFonSsidUsers, FON_SSID_USERS_NUMBER_FILE);
...
int chilli_main(int argc, char **argv) {
  ...
  /*初始化为0个用户连上了*/
  write_int_to_file(0, FON_SSID_USERS_NUMBER_FILE);
  options_init();
  ...
  /*tun0创建后，为fon加上专用路由表*/
  snprintf(acCmd, sizeof(acCmd), "%s", "ip route replace 172.17.2.0/24 dev tun0 table 270");
  system(acCmd);
