  ...
  for(n=0; n < mx; n++) {
    if (this->queue[n].state) {
      safe_snprintf(line, sizeof(line), 
		    ...
		    (int) this->queue[n].retryitertation);
      safe_write(fd, line, strlen(line));
    }
  }
...
int radius_queue_in(struct radius_t *this, 
 ...
 try_again:
  ...
  this->queue[qnext].retrans = 1;
  this->queue[qnext].retryitertation= 0;
...
int radius_timeout(struct radius_t *this) {
  ...
  if (this->first != -1 && 
    ...
    if(!memcmp(&zero_addr.sin_addr, &this->hisaddr0, sizeof(zero_addr.sin_addr)) || 
        !memcmp(&zero_addr.sin_addr, &this->hisaddr1, sizeof(zero_addr.sin_addr)))
        radiusretry = _options.radiusretrysec + 1;
    else
        radiusretry = _options.radiusretry + 2;
        
    
    if (this->queue[this->first].retrans < radiusretry) {
	...
	if (this->queue[this->first].lastsent) {
	  ...
	  addr.sin_addr = this->hisaddr0;
        ...
        if(!memcmp(&zero_addr.sin_addr, &this->hisaddr1, sizeof(zero_addr.sin_addr)))
            addr.sin_addr = this->hisaddr0;
      }

      log_err(0, "addr is :%s", inet_ntoa(addr.sin_addr));
   ...
   { 
      /* Add for parameter radiusretryiteration */
      this->queue[this->first].retryitertation++;  
      log_err(0, "this->queue[this->first].retryitertation: %d!", this->queue[this->first].retryitertation);
      log_err(0, "_options.radiusretryiteration: %d!", _options.radiusretryiteration);
      
      if(this->queue[this->first].retryitertation <= _options.radiusretryiteration)
      {
            this->queue[this->first].retrans = 0;
...
int radius_new(struct radius_t **this,
  ...
  if (setsockopt(new_radius->fd, SOL_SOCKET, SO_BINDTODEVICE, PPPOEIFNAME, strlen(PPPOEIFNAME)+1) < 0)
  {
      log_err(errno, "bind to ppp256 failed!");
  }
  /*mark fon 报文*/
  ulMark  |= DNS_FON_MARK;
  setsockopt(new_radius->fd, SOL_SOCKET, SO_MARK, (char *)(&ulMark), sizeof(ulMark));
	       ...
	       void *cbp)
{
  ...
  if (!this->lastreply) {
    if(!memcmp(&addr.sin_addr,  &this->hisaddr0, sizeof(addr.sin_addr)))
        addr.sin_addr = this->hisaddr1;
    else
        addr.sin_addr = this->hisaddr0;
  }
  else {
    if(!memcmp(&addr.sin_addr,  &this->hisaddr1, sizeof(addr.sin_addr)))
        addr.sin_addr = this->hisaddr0;
    else    
        addr.sin_addr = this->hisaddr1;
  }
...
static int fon_pppoeok()
{
    ...
    //判断是否已经连接成功
    snprintf(acCmd, sizeof(acCmd), "/var/wan/%s/status", PPPOEIFNAME);
    pFile = fopen(acCmd,"r");
    if (NULL == pFile) 
    {
...
void radius_trigger(void)
{
    ...
    //判断是否是按需拨号
    snprintf(acCmd, sizeof(acCmd), "/var/wan/%s/dial", PPPOEIFNAME);
    pFile = fopen(acCmd,"r");
    if (NULL == pFile) 
    {
        log_err(0, "\n %s,%s,%d,open file %s\n",__FILE__,__FUNCTION__,__LINE__,acCmd);
    ...
    log_warn(0, "\n===Now FON Radius will trigger PPPoE!\n");


    snprintf(acCmd,sizeof(acCmd), "/var/wan/%s/pid", PPPOEIFNAME);
