  ...
  for ( ; p1; p1 = p2) {
	...
	*t = ':';
	...
	log_err(0, "Too many pass-throughs! skipped %s", s);
      ...
      if (!(host = ATP_FON_Gethostbyname(p1, AF_INET, NULL))) {
...
void garden_load_domainfile() {
  ...
  /* start :DTS2013031409748 添加uamdomain正则表达式的支持by l00214441 */
  if (_options.uamdomains && _options.uamdomains[0]) 
  {
    ...
    {
      char * pline = _options.uamdomains[id];
      read = strlen(pline);

      if (!pline[0] || pline[0] == '#' || isspace(pline[0])) 
      {
          continue;
      }
      else
      {
	    uamdomain_regex * uam_re = (uamdomain_regex *)calloc(sizeof(uamdomain_regex), 1);
        ...
        log_dbg("compiling uamdomains %s", pline);
        if (regcomp(&uam_re->re, pline, REG_EXTENDED | REG_NOSUB)) {
          log_err(0, "could not compile regex %s", pline);
          free(uam_re);
          continue;
        }
  ...
  /* end: DTS2013031409748 添加uamdomain正则表达式的支持by l00214441 */
