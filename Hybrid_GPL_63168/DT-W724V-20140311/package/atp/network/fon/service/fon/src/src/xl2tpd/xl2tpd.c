...
int g_lChilloutSend = CHILLOUT_MSG_NOTSEND;
    ...
    clear_network();
        ...
        if (signal != SIGTERM) {
                ...
                while (st)
                {
                        ...
                        {
                            close(st->kernel_fd);
                            l2tp_log(LOG_CRIT, "Closing ppol2tp management socket fd=%d", st->kernel_fd);
                            st->kernel_fd = -1;
                        }  
#endif                        
                        st = st2;
...
void fon_clear_stat()
{
    remove(ATP_FON_FILE_STAT_SESSION);
}
#endif
...
#ifdef USE_KERNEL
    /*close pppol2tp management socket if opened before*/
    if (t->kernel_fd >= 0)
    {
        close(t->kernel_fd);
        l2tp_log(LOG_INFO, "Closing ppol2tp management socket fd=%d", t->kernel_fd);
        t->kernel_fd = -1;
    }
#endif
                          ...
                          struct lns *lns)
{
    ...
    //for host name can't be resolved
    for ( i = 0; i< 3; i++)
    {
        hp = ATP_FON_Gethostbyname(host, AF_INET, &bnosuchname);
        if (hp ||bnosuchname)
            break;
...
#endif
        waiting_first_cid = 1;
        g_lChilloutSend = CHILLOUT_MSG_NOTSEND;
        l2tp_log (LOG_DEBUG, "%s : tunnel not up!  Connecting!\n", __FUNCTION__);
...
/* BEGIN: add for UDK0000287751 FON: The Fon client does not calculate the TR-069 statistics parameter correctly by l00214441 2013.06.28 */ 
#if 0
unsigned int fon_get_total_bytes(unsigned int *pulTotalBytesSent, unsigned int *pulTotalBytesReceived)
{
    VOS_CHAR *pszIfname = NULL;
    struct rtnl_link_stats stDevStats;

    memset(&stDevStats, 0, sizeof(stDevStats));  
    pszIfname = "wl0.1";
    ATP_UTIL_get_dev_stats(pszIfname, &stDevStats);
    
    *pulTotalBytesSent = stDevStats.tx_bytes;
    *pulTotalBytesReceived = stDevStats.rx_bytes;

    l2tp_log(LOG_DEBUG, "Get_Total_Sent_Receive_Byte: wl0.1: sent=%d, recevied=%d.\n", stDevStats.tx_bytes, stDevStats.rx_bytes);
    ...
    *pulTotalBytesSent += stDevStats.tx_bytes;
    *pulTotalBytesReceived += stDevStats.rx_bytes;
    l2tp_log(LOG_DEBUG, "Get_Total_Sent_Receive_Byte: wl1.1: sent=%d, recevied=%d.\n", stDevStats.tx_bytes, stDevStats.rx_bytes);
    return VOS_OK;
}

unsigned int fon_start_stat()
{
    FILE *fp = NULL;
    VOS_CHAR  acLine[ATP_SYS_CMD_LEN] = {0};
    ATP_FON_FILE_STAT_TOTAL_ST stFileStatTotal;
    ATP_FON_FILE_STAT_SESSION_ST stFileStatSession;
    
    memset(&stFileStatTotal, 0, sizeof(stFileStatTotal));
    memset(&stFileStatSession, 0, sizeof(stFileStatSession));

    //Increase Total Sessions
    fp = fopen(ATP_FON_FILE_STAT_TOTAL, "r");
    if(NULL == fp)
    {
    	l2tp_log (LOG_DEBUG, "%s: open file %s error", __FUNCTION__, ATP_FON_FILE_STAT_TOTAL);
    	return VOS_NOK;
    }

    fscanf(fp, "%d %d %d", &stFileStatTotal.ulSessions, &stFileStatTotal.ulUsers, &stFileStatTotal.ulAF);

    fclose(fp);

    stFileStatTotal.ulSessions++;

    fp = fopen(ATP_FON_FILE_STAT_TOTAL, "w+");
    if(NULL == fp)
    {
    	l2tp_log (LOG_DEBUG, "%s: open file again %s error", __FUNCTION__, ATP_FON_FILE_STAT_TOTAL);
    	return VOS_NOK;
    }

    //Sessions Users AF
    snprintf(acLine, sizeof(acLine), "%d %d %d", stFileStatTotal.ulSessions, stFileStatTotal.ulUsers, stFileStatTotal.ulAF);
    fputs(acLine, fp);
    
    fclose(fp);

    //Create StatSession File
    stFileStatSession.ulStart = (unsigned int)(time(NULL));
    stFileStatSession.ulUsers = stFileStatTotal.ulUsers;
    stFileStatSession.ulAF = stFileStatTotal.ulAF;

    //Get TotalBytesSent and TotalBytesReceived
    if(VOS_OK != fon_get_total_bytes(&stFileStatSession.ulTotalBytesSent, &stFileStatSession.ulTotalBytesReceived))
    {
        l2tp_log(LOG_DEBUG, "%s: ATP_FON_Get_Total_Bytes error", __FUNCTION__);
        return VOS_NOK;            
    }
    //NAU    
    fp = fopen(WLAN_FON_ASSOCDEV_FILE, "r");
    if(NULL == fp)
    {
        l2tp_log(LOG_DEBUG, "%s: open file %s error", __FUNCTION__, WLAN_FON_ASSOCDEV_FILE);
        return VOS_NOK;        
    }

    fscanf(fp, "%d", &stFileStatSession.ulNAU);

    fclose(fp);


    fp = fopen(ATP_FON_FILE_STAT_SESSION, "w+");
    if(NULL == fp)
    {
    	l2tp_log (LOG_DEBUG, "%s: open file %s error", __FUNCTION__, ATP_FON_FILE_STAT_SESSION);
    	return VOS_NOK;
    }    

    //START USERS NAU AF TOTALBYTESSENT TOTALBYTESRECEIVED
    snprintf(acLine, sizeof(acLine), "%d %d %d %d %d %d", stFileStatSession.ulStart, stFileStatSession.ulUsers, stFileStatSession.ulNAU,
                  stFileStatSession.ulAF, stFileStatSession.ulTotalBytesSent, stFileStatSession.ulTotalBytesReceived);
    fputs(acLine, fp);
    fclose(fp);
...
/* BEGIN: add for UDK0000287751 FON: The Fon client does not calculate the TR-069 statistics parameter correctly by l00214441 2013.06.28 */  
#if 0
    if(VOS_OK != fon_start_stat())
    {
        l2tp_log (LOG_DEBUG, "%s: fon_start_stat error", __FUNCTION__);
    }
#endif   
/* END: add for UDK0000287751 FON: The Fon client does not calculate the TR-069 statistics parameter correctly by l00214441 2013.06.28 */   
    return tmp;
