...
#ifdef USE_KERNEL
            close(server_socket);
            init_server_socket();  /* LNS错误导致无法建立隧道 DTS2012103105714 */
#endif            
            return;
        }
...
/*	if (c->dethrottle) deschedule(c->dethrottle); */
	...
	*/
    ...
    {
        /* 
           当ppp掉线和用户登录失败同时发生时，通过设置下面参数，
           来发送error tunnel消息给chilloutd进程，让其退出，
           保证后续用户可以正常登录 
         */
        l2tp_log (LOG_NOTICE, "=======%s %d waiting_first_cid %d g_lChilloutSend %d\n", __FUNCTION__, __LINE__, waiting_first_cid, g_lChilloutSend);
        if (CHILLOUT_MSG_NOTSEND == g_lChilloutSend)
        {
            waiting_first_cid = 1;
        }
    }
