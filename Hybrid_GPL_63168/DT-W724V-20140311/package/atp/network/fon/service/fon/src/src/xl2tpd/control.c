...
#include <unistd.h>
#include <stdlib.h>
/* <add by cbp 2008-7-25  begin*/
#include <sys/types.h>
#include <sys/stat.h>
#include <fcntl.h>
/* add by cbp 2008-7-25  end>*/
#include "chillout.h"
#include "l2tp.h"
...
/*                                      c->ourrws = c->lac->call_rws;
    ...
    case SCCRP:
        ...
        if (kernel_fd >= 0)
        {
            ...
            for (;;)
            {
                ret = connect(kernel_fd, (struct sockaddr *)&sax, sizeof(sax) );
                if ( ret < 0 ) 
                {
                    ...
                    l2tp_log(LOG_DEBUG, "## connect to PPPoL2TP management socket failed, errno=%d(%s)", errno,strerror(errno));
                }
                break;
            }
            t->kernel_fd = kernel_fd;
            if (ret < 0)
            {
                l2tp_log(LOG_DEBUG, "kernel pppol2tp management socket connect faile, errno=%d", errno);
            }
            else
            {
                l2tp_log(LOG_DEBUG, "kernel pppol2tp management socket connect OK, kernel_fd=%d", kernel_fd);
                l2tp_log(LOG_DEBUG, "kernel pppol2tp management , peer_addr=%X, port=%X", 
                        sax.pppol2tp.addr.sin_addr.s_addr, sax.pppol2tp.addr.sin_port);
             }
        } 
#endif        
        break;
...
struct call * find_closecall (struct tunnel *t)
{
    struct call *sc = NULL;
    ...
    while (sc)
    {
        if (sc->closing && sc->needclose && (sc->container->cLr >= sc->closeSs))
        {
           l2tp_log (LOG_DEBUG, "%s: find_closecall, call is found, id=%d, c->container->cLr=%d, c->closeSs=%d, \n", 
                          __FUNCTION__, sc->ourcid, sc->container->cLr, sc->closeSs);
...
int get_callnum (struct tunnel *t)
{
    struct call *sc = NULL;
...
*/
    ...
    if (CTBIT (*((_u16 *) buf->start)))
    {
        ...
        if (!check_control (buf, t, c))
        {
            ...
            {
#ifdef DEBUG_ZLB
                l2tp_log (LOG_DEBUG, "%s: control ZLB received, needclose=%d, closing=%d, c->ourcid=%d\n", __FUNCTION__, c->needclose, c->closing, c->ourcid);
#endif
                
                t->control_rec_seq_num--;
                c->cnu = 0;

                closecall = find_closecall(t);
                if (NULL != closecall)
                {    
                    closecall->needclose = 0;
                    call_close(closecall);
                }

                l2tp_log (LOG_DEBUG, "%s: ============= total call after is %d\n", 
                      __FUNCTION__, get_callnum(t));  
                //最后一个call已关闭 ，关闭隧道
                if (0 == get_callnum(t))
                {
                    call_close(c);
