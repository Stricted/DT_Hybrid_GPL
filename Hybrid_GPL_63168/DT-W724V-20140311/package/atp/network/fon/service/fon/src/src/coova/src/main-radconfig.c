 ...
 // endtime = now + ADMIN_TIMEOUT; 
  ...
  while (1) {
    ...
    case -1:
      ...
      break;  
    case 0:
      ret_radiustimeout = radius_timeout(radius);
    default:
      break;
    }


    if (RET_RADIUS_TIMEOUT == ret_radiustimeout)
    {
        break;
    }    
...
int main(int argc, char **argv)
{
  ...
  //for radius server can't be resolved
  system("rm -fr /var/fon/fon_DownloadConfig");

  options_init();

  if (process_options(argc, argv, 1)) {
    log_err(errno, "Exiting...");
    system("echo 1 >/var/fon/fon_DownloadConfig");
    exit(1);
  }
