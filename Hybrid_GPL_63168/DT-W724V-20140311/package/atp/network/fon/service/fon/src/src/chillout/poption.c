 ...
 *
 * Created: Sept 2005 Pablo Martín <pablo@fon.com>
 *
 * $Rev: 57 $
 */
#include <getopt.h>
...
OP_ERROR OP_parse_full(OP_option * options_array, int argc, char **argv, int *next_argc)
{
	...
	if ((program_name_full = (char *)strdup(argv[0])) == NULL)
	...
	for (i = 0; options_array[i].description != NULL; i++) {
		if ((options_array[i].type == OP_TYPE_TITLE)
		    || (options_array[i].type == OP_TYPE_PARAMETERS))
			continue;
		if (options_array[i].long_option != NULL) {
			...
			long_options[j].name =
			    (const char *)strdup(options_array[i].long_option);
	...
	short_options[k++] = 'h';
	short_options[k] = '\0';
	long_options[j].name = (char *)strdup("help");
	long_options[j].has_arg = no_argument;
	long_options[j].flag = (int *)NULL;
	long_options[j].val = '?';
	j++;
	...
	while ((c =
		getopt_long(argc, argv, short_options, long_options, &indexptr)) != -1) {
		if (c == 0) {
...
static OP_ERROR OP_show_usage(OP_option * options_array, char *program_name)
{
	...
	if (index_parameters == -1)
		...
		printf(" Usage: %s [OPTIONS] %s\n\n", program_name,
		       options_array[index_parameters].description);
	for (i = 0; options_array[i].description != NULL; i++) {
		if ((options_array[i].type == OP_TYPE_TITLE)
		    || (options_array[i].type == OP_TYPE_PARAMETERS))
			continue;
		if (options_array[i].short_option != '\0')
			sprintf(buffer_short_option, "-%c, ",
				options_array[i].short_option);
		else
		...
		printf("  %s%s%s\n", buffer_short_option, buffer_long_option,
		       options_array[i].description);
	}
