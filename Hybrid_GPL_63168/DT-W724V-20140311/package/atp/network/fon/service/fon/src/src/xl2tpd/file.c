...
int init_config ()
{
    ...
    gconfig.hello_retransmission_delay = DEFAULT_HELLO_RETRANSMISSION_DELAY;
    lnslist = NULL;
    laclist = NULL;
...
struct iprange *set_range (char *word, char *value, struct iprange *in)
{
    ...
    hp = ATP_FON_Gethostbyname(value, AF_INET, NULL);
    if (!hp)
    {
...
int set_hello_retransmission_delay(char *word, char *value, int context, void *item)
{
        set_int (word, value, &(((struct global *) item)->hello_retransmission_delay));
	return 0;
}
...
struct keyword words[] = {
    ...
    {"hello-retransmission-delay", &set_hello_retransmission_delay },
    {NULL, NULL}
};
