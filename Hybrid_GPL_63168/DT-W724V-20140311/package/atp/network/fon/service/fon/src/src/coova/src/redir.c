...
#ifndef WAN_CMDWORD_LEN
#define WAN_CMDWORD_LEN   256
#endif

static void fon_trigger(void)
{
    FILE *pFile = NULL;
    char acLine[WAN_CMDWORD_LEN] = {0};
    char acIntfaceName[WAN_CMDWORD_LEN] = {0};
    char acCmd[WAN_CMDWORD_LEN] = {0};
    unsigned int ulDial = 0;
    int  iPid = -1;
    int ulRetValue = 0;

     strcpy(acIntfaceName,"ppp256"); //W724v接口名为ppp256

    //判断是否是按需拨号
    snprintf(acCmd, sizeof(acCmd), "/var/wan/%s/dial", acIntfaceName);
    pFile = fopen(acCmd,"r");
    if (NULL == pFile) 
    {
    ...
    if(fgets(acLine,WAN_CMDWORD_LEN,pFile))
    {
        ulDial = atol(acLine);
    }
    fclose(pFile);
...
//判断是否已经连接成功
    snprintf(acCmd, sizeof(acCmd), "/var/wan/%s/status", acIntfaceName);
    pFile = fopen(acCmd,"r");
    if (NULL == pFile) 
    {
        log_dbg("\n %s,%s,%d,open file %s\n",__FILE__,__FUNCTION__,__LINE__,acCmd);
...
#if 0//改为信号触发，防止wan up瞬间有1.1.1.1的ping包出去
//用ping一个包触发拨号，由pppc过滤该ping包不发到WAN侧
    snprintf(acCmd, sizeof(acCmd),"%s", "ping -c 1 1.1.1.1 &");
#endif
    snprintf(acCmd,sizeof(acCmd), "/var/wan/%s/pid", acIntfaceName);
    ...
    if(-1 == iPid)
    {
        printf("\n trigger demand get pid -1\n");
        return;
    }
    
    snprintf(acCmd,sizeof(acCmd), "kill -FPE %d", iPid);

    printf("\n FON trigger demand cmd : %s \n",acCmd);
    system(acCmd);
    return;
...
static int bytetosphex(uint8_t *src, const size_t IN_LEN, char *dst, const int MAX_OUT_SIZE) {
  char x[3];
  int i = 0;
  int o = 0;
...
static void redir_wispr1_reply (struct redir_t *redir, struct redir_conn_t *conn, 
				int res, long int timeleft, char* hexchal, 
				char* reply, char* redirurl, bstring b, bstring loginurl) {
  bstring bt = bfromcstr("");;
  ...
  case REDIR_NOTYET:
    ...
    /* BEGIN: add for DTS2013110600719 【FON】302报文中XML格式不正确 by l00214441 2013.11.6 */ 
    bassignformat(bt, "<LoginURL>"); 
    bconcat(b, bt);
    bconcat(b, loginurl);
    bassignformat(bt, "</LoginURL>\r\n"); 
    bconcat(b, bt);	
    /* END: add for DTS2013110600719 【FON】302报文中XML格式不正确 by l00214441 2013.11.6 */ 
...
static void redir_wispr2_reply (struct redir_t *redir, struct redir_conn_t *conn, 
				int res, long int timeleft, char* hexchal, 
				char* reply, char* redirurl, bstring b, bstring loginurl) {
  bstring bt = bfromcstr("");
...
static void redir_buildurl(struct redir_conn_t *conn, bstring str,
			   ...
			   long int timeleft, char* hexchal, char* uid, 
			   char* userurl, char* reply, char* redirurl,
			   uint8_t *hismac, struct in_addr *hisip, int bloginurl) {
  ...
  bstring_buildurl(str, conn, redir, redir_url, resp, timeleft, 
		   hexchal, uid, userurl, reply, redirurl, hismac, hisip, amp);
}
...
int redir_reply(struct redir_t *redir, struct redir_socket_t *sock, 
  ...
  if (resp) {
	...
	redir_wispr1_reply(redir, conn, REDIR_NOTYET, timeleft, hexchal, reply, redirurl, bbody, loginurl);

      else if (!_options.no_wispr2)
	redir_wispr2_reply(redir, conn, REDIR_NOTYET, timeleft, hexchal, reply, redirurl, bbody, loginurl);
	...
	redir_wispr2_reply(redir, conn, res, timeleft, hexchal, reply, redirurl, bbody, loginurl);
      else
	redir_wispr1_reply(redir, conn, res, timeleft, hexchal, reply, redirurl, bbody, loginurl);
...
int redir_listen(struct redir_t *redir) {
  ...
  for (n = 0; n < 2 && redir->fd[n]; n++) {
    ...
    if (setsockopt(redir->fd[n], SOL_SOCKET, SO_LINGER, &so_linger, sizeof(so_linger))) {
      log_err(errno, "setsockopt() failed");
      safe_close(redir->fd[n]);
      redir->fd[n]=0;
      return -1;
...
static int redir_cb_radius_auth_conf(struct radius_t *radius,
  ...
  int i; 
  /* Class */
  for(i=0;i < NUMBER_OF_CLASS_ATTRS; i++) {
   if(!radius_getattr(pack, &attr, RADIUS_ATTR_CLASS, 0, 0, i)) {
     memcpy(conn->s_state.redir.classbuf[i], attr->v.t, attr->l-2);
     conn->s_state.redir.classbuf[i][attr->l-1] = '\0';
   } else {
     int k;
     for(k=i; k<NUMBER_OF_CLASS_ATTRS; k++)
       conn->s_state.redir.classbuf[i][0] = '\0';
     break;
   }
  }
  ...
  if (_options.debug) {
    ...
    if (!radius_getattr(pack, &attr, RADIUS_ATTR_VENDOR_SPECIFIC,
      ...
      if (!radius_keydecode(radius, dstbuffer, RADIUS_ATTR_VLEN, &dstlen, 
			    ...
			    pack_req->authenticator, radius->secret, radius->secretlen) != 0) {
...
static int redir_radius(struct redir_t *redir, struct in_addr *addr,
  ...
  case REDIR_AUTH_EAP:
    ...
    break;
  default:
    log_err(0, "Invalid authentication type: %d", conn->authdata.type);
    radius_free(radius);
    return -1;
  } 
...
int is_local_user(struct redir_t *redir, struct redir_conn_t *conn) {
  ...
  case REDIR_AUTH_CHAP:
    ...
    break;
  default:
    log_err(0, "Authentication method not supported for locally authenticated users: %d", conn->authdata.type);
    return 0;
  }
...
int redir_accept(struct redir_t *redir, int idx) {
  ...
  if ((new_socket = safe_accept(redir->fd[idx], (struct sockaddr *)&address, &addrlen)) < 0) {
  ...
  usleep(10000);/*等10ms让父进程先执行，避免safe_close阻塞*/
...
#define redir_msg_send(msgopt) \
  ...
  */
  ...
  case REDIR_LOGIN: {
	...
	//this is not a function. its a MACRO
	  redir_memcopy(REDIR_CHALLENGE);
	}
      } else {
  ...
  case REDIR_LOGOUT:
    {
      /*没有认证过，不生成新challenge,避免后续取密码失败*/
      if (conn.s_state.authenticated)
      {
  ...
  if (redir->homepage ||
    ...
    redir_buildurl(&conn, url, redir, resp, 0, hexchal, NULL,
		   conn.s_state.redir.userurl, NULL, NULL, conn.hismac, &conn.hisip, 0);
