...
#ifdef DEBUG
#define DPRINTK(format, args...) printk
#else
...
#if defined(CONFIG_MIPS_BRCM) && defined(CONFIG_BLOG)
      blog_link(IF_DEVICE, blog_ptr(skb),
...
static void smux_mark_to_priority(struct sk_buff *skb, u16 *puVlanId)
{    
#if 0
    struct iphdr *pstHdr = NULL;
#endif
    ...
    if (pstHdr)
    {
        if (IPPROTO_IGMP == pstHdr->protocol)
        {
            *puVlanId = ((*puVlanId & 0x1fff) | (0x6000)); 
            return ;
        }
    }
#endif
    ...
    /* mark -----|-----|-------|------|-------|-----|------|------  */
    /*       lan |dscp |default|  wmm |policer|queue|802.1p|lanprio */
    if (skb->mark & 0x010)        
    {            
        *puVlanId = (((*puVlanId) & 0x1fff) | ((skb->mark & 0xe0) << 8));        
    }
	...
	if (__constant_htons(ETH_P_PPP_SES) == skb->protocol) 
	{
        ...
        {
            *puVlanId = (((*puVlanId) & 0x1fff) | (0xe000));
            return;
        }
	}     

    /* 修改配置802.1p remark为7，实际上remark成3问题 */
    if (0x100 == (skb->mark & 0xf00))
    {       
        *puVlanId = ((*puVlanId & 0x1fff) | (0x6000));        
    }
#endif
...
int smux_dev_hard_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
  ...
  //printk("\n ==== > [%s=%d] (%0x) ", __FILE__, __LINE__, dev->priv_flags);
  ...
  // add vlan tag for it
    ...
    {        
        ...
        {
            smux_mark_to_priority(skb, &vlan_tci);
                             ...
                             void *addr_struct_p)
{
  ...
  if (memcmp(SMUX_DEV_INFO(dev)->smux_grp->real_dev->dev_addr,
    ...
    if (!(SMUX_DEV_INFO(dev)->smux_grp->real_dev->flags & IFF_PROMISC)) {
      ...
      flgs |= IFF_PROMISC;
      DPRINTK("SMUX (%s):  Setting underlying device (%s) to promiscious mode.\n",
            dev->name, SMUX_DEV_INFO(dev)->smux_grp->real_dev->name);
      dev_change_flags(SMUX_DEV_INFO(dev)->smux_grp->real_dev, flgs);
    }
  } else {
    DPRINTK("SMUX (%s):  Underlying device (%s) has same MAC, not checking promiscious mode.\n",
            dev->name, SMUX_DEV_INFO(dev)->smux_grp->real_dev->name);
  }
...
#endif
#ifdef SUPPORT_ATP_BT_VLAN0 
static  int smux_set_vlan_device(const char *pszIfname, int iVlan)
{
    struct net_device *dev = NULL;
    int iErr = EINVAL;

#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,29)
    dev = dev_get_by_name(&init_net, pszIfname);
#else
    dev = dev_get_by_name(pszIfname);
#endif
    if (!dev) {
        iErr = -ENODEV;
    ...
    else
    {
        ...
        //printk("\n ==== > [=%d] (%0x ) %d \n", __LINE__, dev->priv_flags, g_vlanid);
    }
    ...
    if(smux_proto == SMUX_PROTO_BRIDGE) {
      ...
      if (list_empty(&grp->virtual_devs)) {
        ...
        if(!mac_reused) {
          ...
          {
            DPRINTK( " Unable to read MAC address from persistent storage.  Using default address.\n" );
            memcpy( new_dev->dev_addr, "\x02\x10\x18\x02\x00\x01", ETH_ALEN );
          }
        }
      }
  ...
  if(grp && new_dev) {
    ...
    if(!__find_smux_in_smux_group(grp, nifname)) {
      ...
      if (register_netdev(new_dev)) {
        DPRINTK("register_netdev failed\n");
        free_netdev(new_dev);
      }
      else {
...
static int smux_unregister_device(const char* vifname)
{
  ...
  if(vdev) {
    ...
    if (vdev->priv_flags & IFF_802_1Q_VLAN)
    {
        vdev->priv_flags &= ~IFF_802_1Q_VLAN;
    }
...
static int smux_ioctl_handler(void __user *arg)
{
  ...
  //printk("\n ==== > [%s=%d]  cmd:%d  vlanid:%d ", __FILE__, __LINE__, args.cmd, args.vlanid);
  ...
  switch (args.cmd) {
    ...
    case SPE_SMUX_CMD: 
        if (!capable(CAP_NET_ADMIN))
          return -EPERM;
/*modify same as busybox smuxctl.c  ,the vlanid process is only BT use*/
#ifdef SUPPORT_ATP_BT_VLAN0  
        err = smux_set_vlan_device(args.u.ifname, args.vlanid);
        if (err)
        {
            //printk("\n ======== > [%s=%d]", __FILE__, __LINE__);
            return -EPERM;
        }
#endif        //g_vlanid = (u16)args.vlanid;            
      break;

    default:
      DPRINTK("%s: Unknown SMUX CMD: %x \n",
