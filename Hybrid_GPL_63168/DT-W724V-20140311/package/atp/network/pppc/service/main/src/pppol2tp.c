...
int setdevname_pppol2tp(const char *acFd)
{
	union {
		char buffer[128];
	...
	pppol2tp_fd = atoi(acFd);

_DEBUG(LOG_PPPC, "the pppol2tp_fd is %d", pppol2tp_fd);
	...
	if (getsockopt(pppol2tp_fd, SOL_PPPOL2TP, PPPOL2TP_SO_DEBUG,
		       &tmp, &tmp_len) < 0) {
	       printf("get PPPOL2TP_SO_DEBUG option failed, pppol2tp_fd=%d", pppol2tp_fd);
...
_DEBUG(LOG_PPPC, "the channel is now pppol2tp channel");
			      ...
			      int accomp)
{
	...
	if (pppol2tp_ifname[0]) {
		...
		if (fd >= 0) {
			...
			if (pppol2tp_debug_mask & PPPOL2TP_MSG_CONTROL) {
				printf("ppp%d: interface name %s",   ifunit, ifname);
			}
		}
		close(fd);
	}
	...
	printf("PPPoL2TP options: %s%s%s%s%s%s%s%s%sdebugmask %d",
...
static void pppol2tp_lcp_snoop(unsigned char *buf, int len, int incoming)
{
	...
	while (len > 0) {
		...
		buf += opt_len;
		if (pppol2tp_debug_mask & PPPOL2TP_MSG_DEBUG) {
			printf("Found option type %02x; len %d", opt, opt_len);
		}
		...
		if (opt == CI_ASYNCMAP && opt_len == 0x06) {
			if (reject) {
				...
				if (pppol2tp_debug_mask & PPPOL2TP_MSG_DATA) {
					printf("Rejected ACCM negotiation; "
					       "defaulting (%s)",
				...
				if (pppol2tp_debug_mask & PPPOL2TP_MSG_DATA) {
					printf("Found ACCM of %08x (%s)", accm,
					       incoming ? "incoming" : "outgoing");
				}
				if (incoming) {
					recv_accm = accm;
			...
			if (got_recv_accm && got_send_accm) {
				if (pppol2tp_debug_mask & PPPOL2TP_MSG_CONTROL) {
					printf("Telling L2TP: Send ACCM = %08x; "
