...
char *mygetpwuid(int uid, FILE * file, char *name, int namelen)
{
    int _uid;
    char foo[256];
    int i;
...
int bftpd_login(char *password)
{
        ...
        (strcasecmp(user , "anonymous")))  //anonymous 不参与用户登录控制
    {
        ...
        if(ATP_LOGINRET_OK != enLoginRet)
        {
            ...
            if(home_directory)free(home_directory);
            return -1;
        }
        g_LoginPid = getpid();
    }
...
int check_file_password(char *my_filename, char *my_username, char *my_password, char **my_home_dir)
{
   ...
   if (found_user)
   {
...
#endif
	{
      ...
      ulLen = strlen(pchomedir) + 1;
      *my_home_dir = calloc( ulLen, sizeof(char) );
      if (! *my_home_dir)
          return -1;
      snprintf(*my_home_dir, ulLen, "%s", pchomedir);
   }
   else
	{
		return -2;  //add by y00181549 for 输入错误用户名返回331，防止尝试用户名
	}
