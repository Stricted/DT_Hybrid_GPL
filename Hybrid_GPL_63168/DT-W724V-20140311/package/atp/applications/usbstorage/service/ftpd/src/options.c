            ...
            if ((strstr(str, "directory")) && (strchr(str, '{')) && (directories)) {
                ...
                create_options(configfile, &(dir->options), NULL);
       			...
       			// opt->value = (char *) malloc(strlen(str));
                        ...
                        /*add by y00181549 for saftly using of the malloced mem, otherwise the opt may contain garbled*/
                        if(opt->value)
                        {
                        ...
                        if(opt->name)
                        {                        
                            memset(opt->name,0,sizeof(opt->name));
                        }
       			sscanf(str, "%[^=]=\"%[^\n\"]", opt->name, opt->value);
            }
   		}
...
void expand_groups()
{
    ...
    if ((grp = config_groups)) {
        do {
            strncat(grp->temp_members, ",", MAX_STRING_LENGTH - strlen(grp->temp_members) - 1);
	...
	while ((str = config_read_line(configfile))) {
		if (strchr(str, '{')) {
            ...
            if (!strcasecmp(str, "global{\n")) {
                ...
                if (usr) {
                    usr = usr->next = malloc(sizeof(struct bftpd_user));
                } else {
                    config_users = usr = malloc(sizeof(struct bftpd_user));
                }
                /* avoid memory errors */
                if (! usr)
                {
...
char *getoption_directories(struct directory *dir, char *name) {
    ...
    do {
        len = strlen(dir->path) + 2;
        bar = malloc(len);
        if (! bar)
           return NULL;

        snprintf(bar, len, "%s", dir->path);
        strncat(bar, "/", len - strlen(bar) - 1);
...
void Reread_Config_File()
{
    ...
    while ( line )
    {
       ...
       if ( (config_value) && (new_value) && (section == 1) )
       {
           ...
           {
                snprintf(config_value, 256, "%s", new_value);
           }
       }
