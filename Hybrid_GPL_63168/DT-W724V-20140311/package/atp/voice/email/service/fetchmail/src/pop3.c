	...
	if (maybe_tls(ctl)) {
		...
		* (see below). */
	       if (gen_transact(sock, "STLS") == PS_SUCCESS
		       && (set_timeout(mytimeout), SSLOpen(sock)) != -1)//, ctl->sslcert, ctl->sslkey, "tls1", ctl->sslcertck,
...
/* is the given message old? */
{
    ...
    if (!ctl->oldsaved)
    {
       // printf("pop3 1264 ctl->oldsaved =%s\n\n",ctl->oldsaved->id);
	    return (num <= last);
    }
    else if (dofastuidl)
    {
...
/* request nth message */
{
    int ok;
    char buf[POPBUFSIZE+1];
	gen_send(sock, "UIDL %d", number);
	if ((ok = pop3_ok(sock, buf)) != 0)
	{
		//printf("[pop3 MailUID=%s.]\n",buf);
	}
	snprintf(MailUID,sizeof(MailUID),"%s",buf);
