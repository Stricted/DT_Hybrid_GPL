...
long long emailentryid=1;
extern long long g_llScanAccount;

void *g_pvMailDb = NULL;
...
extern int scanagain;

static struct query *find_ctl(char *pollname, char *account, char *proto)
{
	struct query *ctl;
	...
	{
		if ((0 == strcmp(ctl->server.pollname,pollname)) &&
		   (0 == strcmp(ctl->remotename,account)) &&
		   (0 == strcasecmp(showproto(ctl->server.protocol), proto)))
		{
			return ctl;
		}
	}
...
static void load_account_idx()
{
	...
	file = fopen("/var/fetchmail/.idx", "rb");
	if (NULL == file)
	{
		return;
	}
	...
	{
		sscanf(buf, "%s %s %s %lld", server, account, prot, &idx);
		ctl = find_ctl(server,account,prot);
		if (NULL != ctl)
		{
			ctl->idx = idx;
		}
...
int main(int argc, char **argv)
{
    ...
    pid_t pid;
    int lastsig = 0;
    printf(ATP_CBBID_EMAIL_NAME "init return %x.\n", ATP_MSG_Init(ATP_CBBID_EMAIL_NAME));
#if defined(__FreeBSD__) && defined(__FreeBSD_USE_KVM)
    dropprivs();
#endif
    envquery(argc, argv);
...
#endif
    //每次开始扫描的时候建立sql文件，以后写到初始化中去时去掉
	FILE *MailContentFile;
	char mailcontent[1024]={0};
	MailContentFile=fopen(VDB_SQL_DUMMY_PATH, "w");
	strcat(mailcontent,"CREATE TABLE [emailsub] ([id] INTEGER  PRIMARY KEY AUTOINCREMENT NOT NULL,");
	strcat(mailcontent,"[account] INTEGER NOT NULL,[mailnumber] INTEGER NOT NULL,[UID] VARCHAR(128)  NULL,[MailDatetime] VARCHAR(64)	NULL,");
	strcat(mailcontent,"[emailAddr] VARCHAR(64)  NULL,[subject] NVARCHAR(128)  NULL,[text] NVARCHAR(650)  NULL);");
	fputs(mailcontent,MailContentFile); 
	fclose(MailContentFile);
	
	g_pvMailDb =(void*)ATP_DummyVDB_Init();
...
#undef NETRC_FILE
	...
	//再次扫描的时候清除数据库中上次扫描的信息
	ATP_VDB_DeleteAllEmailEntries(g_pvMailDb);
	emailentryid=1;
    //获取数据存储指针
	...
	batchcount = 0;
	for (ctl = querylist; ctl; ctl = ctl->next)
	    if (((0 == g_llScanAccount) && (ctl->active)) ||
			((0 != g_llScanAccount) && (ctl->idx == g_llScanAccount)))
	    {
	    ctl->mailNumber = 1;
		activecount++;
		...
		if((ctl->idx == g_llScanAccount)&&(scanagain))
		{
		    int msgsendresult;
			long long msgbody=g_llScanAccount;
	        if((PS_SUCCESS != querystatus) && (PS_NOMAIL != querystatus))
	        {	  		
				msgsendresult=ATP_MSG_SendMsgData(ATP_CBBID_CMS_NAME, ATP_MSG_EMAIL_SCAN_FAILED, &msgbody, sizeof(msgbody));        
				if(msgsendresult==0)
				{
					printf("[ok!fetch failed msg send succ.]\n");
				}
				else
				{
					printf("[sorry!fetch failed	msg send failed.]\n");
				}
			}
			else
			{
			    msgsendresult=ATP_MSG_SendMsgData(ATP_CBBID_CMS_NAME, ATP_MSG_EMAIL_GETEMAIL_RES, &msgbody, sizeof(msgbody));	
	            if(msgsendresult==0)
				{
					printf("[ok!fetch special account msg send succ.]\n");
				}
				else
				{
					printf("[sorry!fetch special account	msg send failed.]\n");
				}
			}
			scanagain=0;//清除重新扫描标志
			break;
    	}
	}

	/* close connections cleanly */
    if(0 == g_llScanAccount)
    {
	    int msgsendresult;
	    msgsendresult=ATP_MSG_SendMsgData(ATP_CBBID_CMS_NAME, ATP_MSG_EMAIL_NEW_NOTIFY, NULL, 0);
		if(msgsendresult==0)
		{
		    printf("[ok!daemon.]\n");
		}
