...
/* fetch messages in lockstep mode */
{
    ...
    for (num = 1; num <= count; num++)
    {
		...
		if (ctl->fetchall || force_retrieval) {
		    ...
		    if (ctl->server.base_protocol->is_old && (ctl->server.base_protocol->is_old)(sock,ctl,num)) {//mailserver_socket
			    msgcode = MSGLEN_OLD;
		    }
	        }
		    ...
		    lastnum < num)
		{
		    ...
		    err = (ctl->server.base_protocol->getpartialsizes)(sock, num, lastnum, *msgsizes);//mailserver_socket
		    if (err != 0) {
			return err;
		    }
		    stage = oldstage;
	...
	/*	else if (msgsize == 512)
		...
		else
		{
		  /* XXX FIXME: make this one variable, wholesize and
		    ...
		    /* request a message */
		    err = (ctl->server.base_protocol->fetch_headers)(sock,ctl,num, &len);//mailserver_socket
		     ...
		     * output sink.  
		     */
		    err = readheaders(sock, len, msgsize,//mailserver_socket
				     ctl, num,
		    ...
		    {
				err = eat_trailer(sock, ctl);//mailserver_socket
				if (err) return(err);
		    }
		    ...
		    if (!suppress_readbody)
		    {
				if (separatefetchbody)
				{
				    ...
				    len = -1;
				    if ((err=(ctl->server.base_protocol->fetch_body)(sock,ctl,num,&len)))//mailserver_socket
				    {
						return(err);
				    }
				    /*
					...
					{
						if (outlevel > O_SILENT)
						    report_build(stdout,GT_(" (%d body octets)"), len);
						if (want_progress()) 
						{
						    report_flush(stdout);
						    putchar(' ');
				...
				err = readbody(sock,//mailserver_socket
				      ctl,
				      !suppress_forward,
				      len);
				...
				if( attachment )
				{
					...
					//printf("tmpmailtext=%s\n\n",tmpmailtext);
					//snprintf(mailtext,sizeof(mailtext),"%s",tmpmailtext);
                    ...
                    snprintf(text,sizeof(text),"[This is a big mail,maybe contain attachment(s) or others] %s",mailtext);
					snprintf(uid,sizeof(uid),"%lld_%s",ctl->idx,MailUID);
					//printf("[write vdb]\n");
					ATP_MAIL_AddEntry(g_pvMailDb,ctl->idx,ctl->mailNumber,uid,maildate,mailfrom,Subject,text);
					ctl->mailNumber++;
					memset(mailtext,0,sizeof(mailtext));
					...
					{	   
						...
						if(sock == -1)
						{
							//printf("[new socket create failed sock]\n");
						}
						...
						{     
						    //printf("[new socket create success sock=%d]\n",sock);
							...
							{	
								...
								{   
								    char buf2[512];
								    gen_recv(sock, buf2, sizeof(buf2));
							...
							if(ctl->server.protocol == P_IMAP)
							{
								if ((ok = gen_transact(sock, "CAPABILITY")) == PS_SUCCESS)
								{
									if(sock!=-1)
									{ 
										gen_transact(sock, "LOGIN \"%s\" \"%s\"", ctl->remotename, ctl->password); 
	...
	flagthemail:
			     ...
			     || (msgcode == MSGLEN_TOOLARGE && ctl->limitflush)))
		{
		    ...
		    err = (ctl->server.base_protocol->delete_msg)(sock, ctl, num);//mailserver_socket
		    if (err != 0)
			return(err);
		}
		else
		{
			...
			&& (msgcode >= 0 && ctl->keep))
		    {
			err = (ctl->server.base_protocol->mark_seen)(sock, ctl, num);//mailserver_socket
			if (err != 0)
			    return(err);
		    }
		}
...
//#if 1
	...
	if (ctl->use_ssl && (SSLOpen(mailserver_socket)) == -1)
	{
	    set_timeout(0);
