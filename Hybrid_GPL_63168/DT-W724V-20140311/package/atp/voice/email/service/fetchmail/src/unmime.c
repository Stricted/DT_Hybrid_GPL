...
int MimeBodyType(char *hdrs, int WantDecode)
{
	...
	if ( (CntType != NULL) &&
		((strncasecmp(CntType, "multipart/mixed", 15) == 0) ||
		 (strncasecmp(CntType, "multipart/alternative", 21) == 0) ||
...
//printf("603 bufp=%s BodyState=%d\n\n",*bufp,BodyState);
  ...
  case S_BODY_DATA:
    //printf("\nbuf=%s\n\n",buf);  	
  	//printf("\nMultipartDelimiter=%s\n\n",MultipartDelimiter);
    if ((*MultipartDelimiter) && 
	(strncmp(buf, MultipartDelimiter, strlen(MultipartDelimiter)) == 0)) {
      BodyState = S_BODY_HDR;
      CurrEncodingIsQP = CurrTypeNeedsDecode = 0;
    }
//printf("CurrEncodingIsQP=%d  CurrTypeNeedsDecode=%d\n\n",CurrEncodingIsQP,CurrTypeNeedsDecode);
...
int main(int argc, char *argv[])
{
 ...
 printf("729 bodytype=%x buffer=%s\n\n",bodytype,buffer);
  i = strlen(buffer);
