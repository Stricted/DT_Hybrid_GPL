...
//#ifdef SSL_ENABLE
		...
		|| must_tls(ctl)) /* if TLS is mandatory, ignore capabilities */
	{
	    ...
	    if (gen_transact(sock, "STARTTLS") == PS_SUCCESS
		    && (set_timeout(mytimeout), SSLOpen(sock)) != -1)
	    {
		/*
		 * RFC 2595 says this:
		 *
...
/* capture the sizes of messages #first-#last */
{
	...
	gen_send(sock, "FETCH %d:%d (UID)", first, last);//RFC822.SIZE
...
/* request body of nth message */
{
    ...
    {
    case IMAP4rev1:	
	      //原来FETCH %d BODY.PEEK[TEXT]//FETCH %d (UID FLAGS BODY.PEEK[])
		gen_send(sock, "FETCH %d (UID FLAGS BODY.PEEK[])",number);//(  正常的FETCH %d (UID FLAGS BODY.PEEK[])  )
		break;
    ...
    } while(!strstr(buf+4, "FETCH") || sscanf(buf+2, "%d", &num) != 1);
    char *uid;
	int ui=0;
    if(strstr(buf,"UID"))
	{
	    uid=strstr(buf,"UID")+4;
		if (NULL != uid)
		{
		  while(*uid!=' ')
		  {
