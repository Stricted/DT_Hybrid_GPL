...
/*ISO-8859-1为西欧语言字符集,含德文,法文*/
static const char* g_pacCodeType[] = {"GBK","BIG5","ISO-8859-1"};
...
/* ship a line to the given control block's output sink (SMTP server or MDA) */
{
	...
	*tp = '\0';
	...
	{
		n =1;//fwrite(buf, 1, last - buf, sinkfp);	
		if (ferror(sinkfp)) n = -1;
    } 
...
/* open a stream to a local MDA */
{
    ...
    if (NULL == sinkfp)
    {
        sinkfp = fopen("/var/fetchmail/mail", "a");
    }
    if (NULL != before)
    {
        free(before);
    ...
    if (!sinkfp)
    {
    	report(stderr, GT_("MDA open failed  errorcode %d:%s\n"), errno, strerror(errno));
    	return(PS_IOERR);
    }
...
extern char mailtext[640];
//extern int exict;
extern int maillength;
...
extern char maildate[64];
extern char mailfrom[64];
extern char Subject[128];
...
// 解码函数
void Base64Decode( char *OrgString, char *Base64String, int Base64StringLen)
{
...
/*Linux系统文件全路径名最大长度为4096*/
#define MAIL_MAX_LEN    650
char        g_acUTF8Output[MAIL_MAX_LEN];
VOS_BOOL IsUTF8(const char* pszStr)
{
    VOS_BOOL bIsUTF8 = VOS_TRUE;
    VOS_UINT8* pcStart = (VOS_UINT8*)pszStr;
    VOS_UINT8* pcEnd = (VOS_UINT8*)pszStr + strlen(pszStr) + 1;
    
    while (pcStart < pcEnd)
    {
        if (*pcStart < 0x80) /* (10000000): 值小于0x80的为ASCII字符*/
        {
            pcStart++;
        }
        else if (*pcStart < (0xC0)) /* (11000000): 值介于0x80与0xC0之间的为无效UTF-8字符*/
        {
            bIsUTF8 = VOS_FALSE;
            break;
        }
        else if (*pcStart < (0xE0)) /* (11100000): 此范围内为2字节UTF-8字符*/
        {
            if (pcStart >= pcEnd - 1) 
                break;
            if ((pcStart[1] & (0xC0)) != 0x80)
            {
                bIsUTF8 = VOS_FALSE;
                break;
            }
        ...
        else if (*pcStart < (0xF0)) /* (11110000): 此范围内为3字节UTF-8字符*/
        {
            if (pcStart >= pcEnd - 2) 
                break;
            if ((pcStart[1] & (0xC0)) != 0x80 || (pcStart[2] & (0xC0)) != 0x80)
            {
                bIsUTF8 = VOS_FALSE;
                break;
            }
...
char *ScanIconvConvert (const char *pszInput, const int ulCodeIdx)
{      
    char        *pszRet = VOS_NULL;
    char        *pcOut = VOS_NULL;
    char        *pcIn = VOS_NULL;
    int       ulOutLeft = MAIL_MAX_LEN;
    int       ulInLeft = strlen(pszInput) + 1;
    static iconv_t   cdIconv;

    cdIconv = iconv_open(UTF8, g_pacCodeType[ulCodeIdx]);
    if (cdIconv == (iconv_t) (-1))
    {
        perror("iconv_open");
        cdIconv = 0;
        return VOS_NULL;
    ...
    //printf("[Code:%s]\n", g_pacCodeType[ulCodeIdx]);
    
    pcOut = g_acUTF8Output;
    pcIn   = (char*)pszInput;
    memset(g_acUTF8Output,0,MAIL_MAX_LEN);

    //lint -save -e64
    if( (size_t)(-1)  == iconv(cdIconv, &pcIn, (size_t *)(&ulInLeft), &pcOut, (size_t *)(&ulOutLeft)) )
    //lint restore
    {
        /*转换错误判断*/
        switch  (errno)                  
        {
	        case  EILSEQ: /*输入中含有无效的多字节序列,可能是字符集不对*/      
	            //printf ("[error iconv EILSEQ]\n");
	            break;
	        case  EINVAL: /*输入以不完整多字节序列结尾*/
	            //printf ("[error iconv EINVAL]\n");          
	            break;
	        case  E2BIG:  /*输出缓冲长度不足*/
	            //printf ("[error iconv E2BIG]");          
	            break;
	        default:
	            //printf ("[error iconv UNKNOWN]");   
	            break;
        }
    }
    else
    {
        pszRet = g_acUTF8Output;
    }
    
    if (iconv_close (cdIconv) < 0)
	{
	    perror ("iconv_close");
	}
	cdIconv = 0;
...
/*转换结果字符串在全局变量数据区，不要释放*/
char *ScanIconvTryConvert(const char *pszInput)
{
    char        *pcRet;
    int         lIdx;
    if(!pszInput)
	...
	{
		if(VOS_TRUE == IsUTF8(pszInput))
	    {        
	        memset(g_acUTF8Output,0,sizeof(g_acUTF8Output));
	        snprintf(g_acUTF8Output,sizeof(g_acUTF8Output),"%s",pszInput);    
	        return g_acUTF8Output;
	    }

	    for (lIdx = 0; lIdx < (int)(sizeof(g_pacCodeType)/sizeof(g_pacCodeType[0])); lIdx++)
	    {
	        pcRet = ScanIconvConvert(pszInput, lIdx);
	        if (NULL != pcRet)
	        {
	            return pcRet;
	        }
...
//字符串替换函数
char *replace(char *source,char *destination)
{
    char  tmpsource[650];          //用来存储真正的结果
    char  tmp[650];                //用来临时存储结果
    char  *tmp2;                   //用来指示特定字符串的位置
    char  *tmp3;                   //用来指示余下的字符串
    int    k=0;                    //用来指定当前查找的特定字符串   
    char  *HTML[]={"&Auml;","&auml;","&Ouml;","&ouml;","&Uuml;","&szlig;","&nbsp;","&lt;","&gt;","&quot;"};
    char  *change[]={"脛","盲","脰","枚","脺","脽"," ","<",">","\""};
	if(!source)
	{
	    char *emptysource="null";
	    memset(destination,emptysource,strlen(emptysource));
		destination[strlen(emptysource)]='\0';
		return destination;
	}
    //保存source到tmpsource中
    memset(tmpsource,0,sizeof(tmpsource));
    memset(tmp,0,sizeof(tmp));
    snprintf(tmpsource,sizeof(tmpsource),"%s",source);
    for(k=0;k<6;k++)
    {
        tmp2=NULL;
        tmp2=strstr(tmpsource,HTML[k]);
        //一次遍历，将指定的字符串全部替换掉
        while(tmp2)
        {
            //先将前面正常的字符临时拷贝下来
            memset(tmp,0,sizeof(tmp));
            strncat(tmp,tmpsource,strlen(tmpsource)-strlen(tmp2));
            strcat(tmp,change[k]);
            tmp3=tmp2+strlen(HTML[k]);
            strcpy(tmpsource,tmp);
            strcat(tmpsource,tmp3);
            tmp2=strstr(tmpsource,HTML[k]);//指向下一处
        }
    }
    memcpy(destination,tmpsource,strlen(tmpsource));
	destination[strlen(tmpsource)]='\0';
    return destination;
}

extern int ATP_MAIL_AddEntry(void *pvVdbHdl,long long accountIdx, int mail_number,char *Uid,
		char *datetime, char *emailAddr,char *subject,char *text);
char MailUID[128]={0};
extern char mail[3072];
...
/* perform end-of-message actions on the current output sink */
{
	...
	if (rc || err)
	{
		...
		//进行字符串的编码转换全部转换为UTF-8
		char *mailbody;
		mailtext[strlen(mailtext)]='\0';
		mailbody=ScanIconvTryConvert(mailtext);
		memset(mailtext,0,sizeof(mailtext));
		replace(mailbody,mailtext);

		//重新处理uid
        char uid[128];
		snprintf(uid,sizeof(uid),"%lld_%s",ctl->idx,MailUID);
		char mailcontent[650]={0};
        if(attachment)
       	{	
       	    memset(mailcontent,0,sizeof(mailcontent));
       	    snprintf(mailcontent,sizeof(mailcontent),"[This mail contains attachment(s)] mail text:%s",mailtext);  
       	    attachment=0;
        }
		else
		{
		    memset(mailcontent,0,sizeof(mailcontent));
       	    snprintf(mailcontent,sizeof(mailcontent),"%s",mailtext); 
		}
		//printf("[write vdb]\n");//MailUID
        ATP_MAIL_AddEntry(g_pvMailDb,ctl->idx,ctl->mailNumber,uid,maildate,mailfrom,Subject,mailcontent);
		ctl->mailNumber++;

        memset(uid,0,sizeof(uid));
		memset(mailtext,0,sizeof(mailtext));	
		memset(maildate,0,strlen(maildate));
		memset(mailfrom,0,strlen(mailfrom));
		memset(Subject,0,strlen(Subject));
		memset(MailUID,0,strlen(MailUID));
		memset(mail,0,strlen(mail));
		memset(mailcontent,0,strlen(mailcontent));
		maillength=0;
	    return(FALSE);
	    }
	}
