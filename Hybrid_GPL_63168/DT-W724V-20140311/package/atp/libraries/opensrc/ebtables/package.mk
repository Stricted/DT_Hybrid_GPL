...
define EBTABLES_INSTALL_TARGET_CMDS
	...
	if test $(ATP_EBTABLES_VERSION) = "2.0.10.4" ; then \
		install -m 777 $(SERVICE_EBTABLES_SRC_DIR)/extensions/libebt*.so $(STAGING_DIR)/lib; \
		install -m 777 $(SERVICE_EBTABLES_SRC_DIR)/extensions/libebt*.so $(TARGET_DIR)/lib; \
		install -m 777 $(SERVICE_EBTABLES_SRC_DIR)/libebt*.so $(STAGING_DIR)/lib; \
		install -m 777 $(SERVICE_EBTABLES_SRC_DIR)/libebt*.so $(TARGET_DIR)/lib; \
		install -m 0644 $(SERVICE_EBTABLES_SRC_DIR)/ethertypes $(STAGING_DIR)/etc; \
		install -m 0644 $(SERVICE_EBTABLES_SRC_DIR)/ethertypes $(TARGET_DIR)/etc; \
	fi; \
...
ifneq ($(ATP_EBTABLES_VERSION),"2.0.10.4")
# Target name
EBTABLES_TARGET = dynamic
else
EBTABLES_TARGET = 
