...
void local_init(message_t *message)
{
	...
	if (!mda)
	{
		fprintf(stdout, "Local delivery not possible without a MDA\n");
		exit(EX_OSFILE);
	}
	...
	{
		fprintf(stdout, "Failed to connect to MDA\n");
		exit(EX_OSERR);
	}
	...
	if(mda_fp)
	{
		...
		if((status = pclose(mda_fp)))
		{
			if(WIFSIGNALED(status)) 
				fprintf(stdout, "MDA died of signal %d\n", WTERMSIG(status));
			else if(WIFEXITED(status))
				fprintf(stdout, "MDA returned nonzero status %d\n", WEXITSTATUS(status));
			else
				fprintf(stdout, "MDA failed\n");
			exit(EX_OSERR);
		}
