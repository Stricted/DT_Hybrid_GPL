...
static void event_cb (smtp_session_t session, int event_no, void *arg, ...)
{
	...
	switch (event_no) {
		case SMTP_EV_EXTNA_DSN:
			fprintf(stdout, "Delivery Status Notification extension not supported by MTA\n");
			break;

		case SMTP_EV_EXTNA_8BITMIME:
			fprintf(stdout, "8bit-MIME extension not supported by MTA\n");
			break;
			
		case SMTP_EV_EXTNA_STARTTLS:
			fprintf(stdout, "StartTLS extension not supported by MTA\n");
			break;
		...
		case SMTP_EV_WEAK_CIPHER:
		{
			...
			fprintf(stdout, "Weak cipher (%d bits)\n", bits);
		...
		case SMTP_EV_INVALID_PEER_CERTIFICATE:
		{
			...
			fprintf(stdout, "Invalid peer certificate (error %ld)\n", result);
		...
		case SMTP_EV_WRONG_PEER_CERTIFICATE:
		{
			...
			fprintf(stdout, "Wrong peer certificate\n");
...
static char *get_username(void)
{
	...
	if (!user)
	{
		fprintf (stdout, "Could not determine the username of uid %d!\n", (int)uid);
		exit (EX_NOUSERNAME);
	}
...
void smtp_send(message_t *msg, identity_t *identity)
{
	...
	if (identity->preconnect)
	{
		...
		if (WIFSIGNALED(ret))
		{
			...
			{
				fprintf (stdout, "Pre-connect command received signal %d\n", sig);
				exit (EX_SOFTWARE);
			}
		}
		...
		if (exit_status != 0)
		{
			fprintf (stdout, "Pre-connect command \"%s\" exited with non-zero status %d\n",
	...
	if (!smtp_start_session (session))
	{
		...
		fprintf (stdout, "SMTP server problem %s\n",
	...
	if (identity->postconnect)
	{
		...
		if (WIFSIGNALED(ret))
		{
			...
			if (sig == SIGINT || sig == SIGQUIT)
			{
				fprintf (stdout, "Post-connect command received signal %d\n", sig);
				exit (EX_SOFTWARE);
			}
		}
		...
		if (exit_status != 0)
		{
			fprintf (stdout, "Post-connect command \"%s\" exited with non-zero status %d\n",
