...
static void drop_sgids( void )
{
	...
	{
		fprintf (stdout, "Could not drop setgid: %s\n", strerror(errno));
		exit(EX_DROPPERM);
	}
}
...
int main (int argc, char **argv)
{
	...
	while ((c = getopt (argc, argv, "A:B:b:C:cd:e:F:f:Gh:IiL:M:mN:nO:o:p:q:R:r:sTtV:vX:W:")) != EOF)
		switch (c)
		{
			case 'A':
			...
			case 'B':
				...
				{
					fprintf (stdout, "Unsupported body type %s\n", optarg);
					exit (EX_USAGE);
				}
				break;
            ...
            case 'W':
                /* email file name */
                message->fp = fopen(optarg, "r");
                if (NULL==message->fp)
                {
                    fprintf (stdout, "Open %s fail... %s\n", optarg, strerror(errno));
				    exit (-1);
                }
                break;
			case 'I':
			...
			case 'b':
				...
				switch (c)
				{
					...
					case 'v':
						/* Verify names only */
						fprintf (stdout, "Unsupported operation mode %c\n", c);
						exit (EX_USAGE);
						break;

					default:
						fprintf (stdout, "Invalid operation mode %c\n", c);
						exit (EX_USAGE);
						break;
				 }
				 break;
	...
	{
		fprintf(stdout, "Recipient names must be specified\n");
		exit (EX_USAGE);
	}
	...
	if (parse_headers)
	{
		...
		{
			fprintf(stdout, "No recipients found\n");
			exit(EX_DATAERR);
		}
	}
