...
#define MAX_PROPERTY_NUM 5
#define MAX_PROPERTY_VALUE_LEN 128
char g_pchMultiValue[MAX_PROPERTY_NUM][MAX_PROPERTY_VALUE_LEN];
			 ...
			 const ne_prop_result_set *set)
{
    ...
    memset(session.propname, 0x00, sizeof(session.propname));
    memset(session.propvalue, 0x00, sizeof(session.propvalue));
    ...
    snprintf(session.propname, sizeof(session.propname)-1, "%s", dname);
    snprintf(session.propvalue, sizeof(session.propvalue)-1, "%s", dval);
	free(dval); 
    } else {
...
/* BEGIN: add by 00203771 for get multi-property in one request */
static void pget_multi_results(void *userdata, const ne_uri *uri, 
			 const ne_prop_result_set *set)
{
    ne_propname *pname = userdata;
    const char *value;
    ...
    for (i = 0; pname[i].name != NULL; i++)
    {
        dname = utf8_decode(pname[i].name);

        value = ne_propset_value(set, pname+i);
        if (value != NULL) 
        {
	        char *dval = utf8_decode(value);
	        printf(_("Value of %s is: %s\n"), dname, dval);

            snprintf(&g_pchMultiValue[i][0], MAX_PROPERTY_VALUE_LEN, "%s", dval);

	        free(dval); 
        } 
        else 
        {
	        const ne_status *status = ne_propset_status(set, pname+i);
	
	        if (status) 
            {
	            printf(_("Could not fetch property[%s]: %d %s\n"), 
		           dname, status->code, status->reason_phrase);
	        } 
            else 
            {
	            printf(_("Server did not return result for %s\n"),
...
/* END: add by 00203771 for get multi-property in one request */
...
/* BEGIN: add by 00203771 for get multi-property in one request */
static void execute_multi_propget(const char *res, const char **name)
{
    ne_propname *pnames = NULL;
    char *remote = resolve_path(session.uri.path, res, false);
    ne_propname *props;
    int i;
    int ret;
    ...
    else 
    {
        ...
        pnames = (ne_propname *)malloc(sizeof(ne_propname) * (i+1));
        if (NULL == pnames)
        {
            free(remote);
            return;
        ...
        for (i = 0; *(name + i) != NULL; i++)
        {
            pnames[i].nspace = (const char *)get_option(opt_namespace);
	        pnames[i].name = utf8_encode(*(name + i));
        }

        pnames[i].nspace = NULL;
        pnames[i].name   = NULL;
        props = pnames;
    }

    out_start(_("Fetching multi-properties for"), res);
    ret = ne_simple_propfind(session.sess, remote, NE_DEPTH_ZERO, props,
 		          pget_multi_results, props);
...
static void execute_propget(const char *res, const char *name)
{
    ...
    if (strcmp(name, "ATP_FILE_STATE") == 0)
    {
        char *name[] = {{"getlastmodified"}, {"resourcetype"}, {"getcontentlength"}, {NULL}};
        free(remote);
        return execute_multi_propget(res, name);
    }
    else
    {
...
static void execute_get(const char *remote, const char *local)
{
    ...
    pchWorkMode = get_option(opt_workmode);
    if (NULL != pchWorkMode && 0 == strcmp(pchWorkMode, "command_mode"))
    {
...
VOS_UINT32  ATP_NAS_TranslateError(struct session sess)
{
...
VOS_UINT32 ATP_NAS_Execute_MultiPropget(VOS_CHAR *res, VOS_CHAR **name, VOS_CHAR **value)
{
    ...
    memset(g_pchMultiValue, 0x00, sizeof(g_pchMultiValue));
    
    execute_multi_propget(res, name);

    for (i = 0; name[i]!=NULL && i<MAX_PROPERTY_NUM; i++)
    {
        *(value + i) = &g_pchMultiValue[i][0];

        if (g_pchMultiValue[i][0] == '\0' && strcmp(name[i], "resourcetype")!=0)
        {
            iFail = 1;
        }
    }
