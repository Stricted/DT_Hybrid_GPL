...
#define CADAVER_NEON_IPV4_ONLY        0
#define CADAVER_NEON_IPV6_ONLY        1
#define CADAVER_NEON_IPV4IPV6_BOTH    2
#define CADAVER_NEON_IPV4IPV6_NEITHER 3

/* Print DNS result */
void ne_print_dns_result(struct addrinfo *result)
{
    struct addrinfo *temp = NULL;
    struct sockaddr_in  *pstAddrIn  = NULL;
    struct sockaddr_in6 *pstAddrIn6 = NULL;
    char buf[128];
    ...
    for (temp=result; temp!=NULL; temp=temp->ai_next)
    {
        ...
        {
            pstAddrIn = (struct sockaddr_in *)temp->ai_addr;
                
            printf("IPv4 address: %s\n", inet_ntoa(pstAddrIn->sin_addr));
        }
        else
        {
            pstAddrIn6 = (struct sockaddr_in6 *)temp->ai_addr;
                
            printf("IPv6 address: %s\n", inet_ntop(AF_INET6, (const void *)(&(pstAddrIn6->sin6_addr)), buf, sizeof(buf)));
        }
    }
...
struct addrinfo *ne_choose_dns_result(struct addrinfo *result, int IpType)
{
    struct addrinfo *temp = NULL;
    ...
    if (NULL == temp)  /* can not find target */
    {
        return result;
    }

    if (temp == result) /* need not to resort */
    {
        return result;
    }
    else
    {
        if (NULL != prev)
        {
            prev->ai_next = temp->ai_next;  /*put the prefer IP on the head*/
            temp->ai_next = result;
            return temp;
        }
        else
...
ne_sock_addr *ne_addr_resolve(const char *hostname, int flags)
{
...
#else
        switch(g_lCadaverPPP256Mode)
        {
            case CADAVER_NEON_IPV4_ONLY:
            {
                printf("PPP256 mode: IPv4 Only\n");
                hints.ai_family = AF_INET;
                break;
            }
            case CADAVER_NEON_IPV6_ONLY:
            {
                printf("PPP256 mode: IPV6 only\n");
                hints.ai_family = AF_INET6;
                break;
            }
            case CADAVER_NEON_IPV4IPV6_BOTH:
            {
                printf("PPP256 mode: IPV4 && IPV6\n");
                hints.ai_family = AF_UNSPEC;
                break;
            }
            case CADAVER_NEON_IPV4IPV6_NEITHER:
            {
                printf("PPP256 mode: IPV4 nor IPV6\n");
                hints.ai_family = AF_UNSPEC;
                break;
            }
            default:
            {
                printf("PPP256 mode: unknown");
                hints.ai_family = AF_UNSPEC;
                break;
            }
        }
	...
	addr->errnum = getaddrinfo(hostname, NULL, &hints, &addr->result);
    {
        printf("******* Cadaver DNS Result Begin *******\n");
        ne_print_dns_result(addr->result);       
        printf("******* Cadaver DNS Result End *******\n");

        if (CADAVER_NEON_IPV4_ONLY != g_lCadaverPPP256Mode && CADAVER_NEON_IPV6_ONLY != g_lCadaverPPP256Mode)
        {
            /* if request both for ipv6 and ipv4, prefer use ipv6 address first */
            addr->result = ne_choose_dns_result(addr->result, AF_INET6); 
           
            printf("======= DNS Result after sort ======= \n");
            ne_print_dns_result(addr->result);   
            printf("===================================== \n");
        }
	}
                    ...
                    const ne_inet_addr *addr, unsigned int port)
{
  ...
  && defined(FD_CLOEXEC)
    ...
    if (0 != ATP_UTIL_SetFilterlistControlMark(fd))
    {
        set_error(sock, _("[Hybrid NAS]: Set FilterlistControlMark fail!!!"));
		ne_close(fd);
        return NE_SOCK_ERROR;
    }
#endif
