...
#define DEFAULT_NAMESPACE        "DAV:"
#define DEFAULT_MEDIENCENTER_URI "https://webdav.mediencenter.t-online.de:443"
...
int g_lCadaverPPP256Mode = ATP_CADAVER_IPV4IPV6_BOTH;
...
/* user config */
#define CADVER_MEDIENCENTER  0
#define CADAVER_PROXY        1
#define CADAVER_USERNAME     2
#define CADAVER_PASSWORD     3
#define CADAVER_PRINT        4

typedef struct ATP_CADAVER_CONFIG_TAG
{
    char *pchName;
    char *pchValue;
}ATP_CADAVER_CONFIG;

static ATP_CADAVER_CONFIG g_stCadaverConfig[] = {{"mediencenter", NULL},
                                                 {"proxy",        NULL},
                                                 {"username",     NULL},
                                                 {"password",     NULL},
                                                 {"print",        NULL}};
...
tmp = ne_ssl_readable_dname(dn); printf(str, tmp); free(tmp)
    ...
    if (strcmp(get_option(opt_workmode), "command_mode")==0)
    {
...
int open_connection(const char *url)
{
    ...
    ATP_NAS_GetCurrentWorkMode(&mode);
	...
	if (ne_uri_parse(url, &session.uri) || session.uri.host == NULL) {
	    ...
	    return ATP_WEBDAV_URI_PARSE_ERR;
	}
	...
	if (strcasecmp(session.uri.scheme, "https") == 0) {
		...
		return ATP_WEBDAV_SSL_NOTSUPPORT_ERR;
	    }
	    use_ssl = 1;
	}
    }
    ...
    ne_set_useragent(session.sess, "HUAWEI/ATP WEBDAV");
...
int cadaver_execute_command(const char *line)
{
...
static VOS_VOID cadaver_ConfigClean(VOS_VOID)
{
    unsigned int i = 0;

    for (i=0; i < (sizeof(g_stCadaverConfig)/sizeof(ATP_CADAVER_CONFIG)); i++)
    {
        if (g_stCadaverConfig[i].pchValue != NULL)
        {
            free(g_stCadaverConfig[i].pchValue);
            g_stCadaverConfig[i].pchValue = NULL;
        }
    }
}

static int cadaver_readconfig(void)
{
    unsigned int i    = 0;
    FILE *pFile = NULL;
    char *pcStart = NULL;
    ...
    if (access("/var/nas/nasconfig.cfg", F_OK) != 0)
    {
        return VOS_NOK;
    }

    pFile = fopen("/var/nas/nasconfig.cfg", "r");
    if (NULL == pFile)
    {
        pFile = fopen("/var/nas/nasconfig.cfg","r");
        if (NULL == pFile)
    ...
    while (NULL != fgets(szReadLine, sizeof(szReadLine) - 1, pFile))
    {
        for (i = 0; i < (sizeof(g_stCadaverConfig)/sizeof(ATP_CADAVER_CONFIG)); i++)
        {
            if (VOS_NULL_PTR != strstr(szReadLine, g_stCadaverConfig[i].pchName))
            {
                break;
            }
        }
        
        if (i >= (sizeof(g_stCadaverConfig)/sizeof(ATP_CADAVER_CONFIG)))
        {
            continue;
        }
        
        pcStart = strchr(szReadLine, '{');
        if (NULL == pcStart)
        {
            continue;
        }
        pcStart++;
        pcEnd   = strchr(szReadLine, '}');
        if (NULL == pcEnd)
        {
            continue;
        }
        *pcEnd =  '\0';
        ...
        g_stCadaverConfig[i].pchValue = strdup(pcStart);
        if (NULL == g_stCadaverConfig[i].pchValue)
        {
...
static void init_options(void)
{
    ...
    set_option(opt_workmode, NULL);

    memset(acLine, 0x00, sizeof(acLine));
    memset(acTemp, 0x00, sizeof(acTemp));

    /* Get the user configuration */
    if (g_stCadaverConfig[CADVER_MEDIENCENTER].pchValue != NULL)
    {
        set_option(opt_mediencenter,  g_stCadaverConfig[CADVER_MEDIENCENTER].pchValue);
    }
    else
    {
        ...
        if (NULL == pFile)
        {
            set_option(opt_mediencenter, ne_strdup(DEFAULT_MEDIENCENTER_URI));
        }
        else
        {
    ...
    if (g_stCadaverConfig[CADAVER_PROXY].pchValue!=NULL 
     && g_stCadaverConfig[CADAVER_USERNAME].pchValue!=NULL 
     && g_stCadaverConfig[CADAVER_PASSWORD].pchValue!=NULL)
    {
        set_option(opt_proxy, g_stCadaverConfig[CADAVER_PROXY].pchValue);
        set_option(opt_proxy_port, strdup("8080")); /* 代理端口号必为8080 */
        set_option(opt_proxyusername, g_stCadaverConfig[CADAVER_USERNAME].pchValue);
        set_option(opt_proxypassword, g_stCadaverConfig[CADAVER_PASSWORD].pchValue);
    }
}

void set_cadavermode(int mode)
{    
    switch(mode)
    {
        case ATP_WEBDAV_COMMAND_STATE:
        {
            set_option(opt_workmode, ne_strdup("command_mode"));
            break;
        }
        case ATP_WEBDAV_WEBDAV_STATE:
        {
            set_option(opt_workmode, ne_strdup("webdav_mode"));
            break;
        }
        case ATP_WEBDAV_SUBCAD_STATE:
        {
            set_option(opt_workmode, ne_strdup("subdav_mode"));
            break;
        }
        default:
        {
...
void cadaver_command_mode(int argc, char **argv)
{
    int ret   = 0;
...
int cadaver_judge_ip_status(char *pchStateFile)
{
    int  status = 0;
    FILE *pFile = NULL;
    char chBuff[32];

    if (NULL == pchStateFile)
    {
        return 0;
    }
    ...
    if (access(pchStateFile, F_OK) == 0)
    {
        pFile = fopen(pchStateFile, "r");
        if (NULL == pFile)
        {
            pFile = fopen(pchStateFile, "r");
            if (NULL == pFile)
        ...
        if(NULL != fgets(chBuff, sizeof(chBuff), pFile))
        {
            if(NULL != strchr(chBuff, '3'))
            {
                status = 1;
            }
        }    
...
int cadaver_judge_ppp256(void)
{
    int ipv4_status = 0;
    int ipv6_status = 0;
    int ipv4_lte_status = 0;
    int ipv6_lte_status = 0;

    ipv4_status = cadaver_judge_ip_status("/var/wan/ppp256/status");

    ipv6_status = cadaver_judge_ip_status("/var/wan/ppp256/status6");

    ipv4_lte_status = cadaver_judge_ip_status("/var/wan/gre2/status");

    ipv6_lte_status = cadaver_judge_ip_status("/var/wan/gre2/status6");

    if ((ipv4_status + ipv4_lte_status) >= 1 && 0 == (ipv6_status + ipv6_lte_status))
    {
        g_lCadaverPPP256Mode = ATP_CADAVER_IPV4_ONLY;
    }
    else if (0 == (ipv4_status + ipv4_lte_status) && (ipv6_status + ipv6_lte_status) >= 1)
    {
        g_lCadaverPPP256Mode = ATP_CADAVER_IPV6_ONLY;
    }
    else if (1 <= (ipv4_status + ipv4_lte_status) && 1 <= (ipv6_status + ipv6_lte_status))
    {
        g_lCadaverPPP256Mode = ATP_CADAVER_IPV4IPV6_BOTH;
    }
    else 
    {
        g_lCadaverPPP256Mode = ATP_CADAVER_IPV4IPV6_NEITHER;
    }
...
int main(int argc, char *argv[])
{
    ...
    if(0 != cadaver_readconfig())
    {
        cadaver_ConfigClean();
    }

    if (NULL == g_stCadaverConfig[CADAVER_PRINT].pchValue)
    {
         ne_debug_init(stdout, 0);
    }
    else
    {
        ne_debug_init(stdout, 0xFFFFFFFF);
    }
    ...
    ATP_NAS_FindOutCadaverWorkMode(&mode, argc, argv);

    set_cadavermode(mode);

    switch (mode)
    {
        case ATP_WEBDAV_COMMAND_STATE:
        {
            cadaver_command_mode(argc, argv);
            break;
        }
        case ATP_WEBDAV_WEBDAV_STATE:
        case ATP_WEBDAV_SUBCAD_STATE:
        {
            ATP_NAS_WebDavClient_Entry(argc, argv);
            break;
        }
        default:
        {
            printf(_("Unexpected process parameter! argc[]%d "), argc);
    ...
    if (ATP_WEBDAV_COMMAND_STATE == mode)
    {
        cadaver_ConfigClean();
    }
...
static int supply_creds(const char *prompt, const char *realm, const char *hostname,
			char *username, char *password, int AuthMethod)
{
    char *tmp;
    ...
    if (AuthMethod==ATP_WEBDAV_MANNUAL_AUTH)
    {
        tmp = readline(_("Username: "));
    }else if (AuthMethod==ATP_WEBDAV_SERVER_AUTH)
    {
        tmp = get_option(opt_medusername);
        printf("mediencenter user name is %s\n", tmp);
    }else
    {
        tmp = get_option(opt_proxyusername);
        printf("proxy user name is %s\n", tmp);
    }
    ...
    {
        tmp = get_option(opt_medpassword);
        printf("mediencenter password is %s\n", tmp);
    }else
    {
        tmp = get_option(opt_proxypassword);
        printf("proxy password is %s\n", tmp);
    }
			       ...
			       char *username, char *password)
{
	...
	session.uri.host, username, password, ATP_WEBDAV_MANNUAL_AUTH);
    }else{
    return supply_creds(
    _("Authentication required for %s on server `%s':\n"), realm,
	session.uri.host, username, password, ATP_WEBDAV_SERVER_AUTH);
    }
}
			      ...
			      char *username, char *password) 
{
	...
	proxy_hostname, username, password, ATP_WEBDAV_MANNUAL_AUTH);
    }else{
    return supply_creds(
	_("Authentication required for %s on proxy server `%s':\n"), realm,
	proxy_hostname, username, password, ATP_WEBDAV_PROXY_AUTH);
    }
}
