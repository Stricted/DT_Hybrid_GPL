...
#if defined(CONFIG_ATP_MIRROR)
static char acMirrorInLanDevice[MIRROR_INTF_SIZE];
static char acMirrorOutLanDevice[MIRROR_INTF_SIZE];
#endif
...
#if defined(CONFIG_ATP_MIRROR)
    MirrorCfg mirrorCfg;
    memset(&mirrorCfg, 0, sizeof(mirrorCfg));
#endif
    switch (nCmd)
    {
    case SIOCGLINKSTATE:
    ...
    case SIOCPORTMIRROR:
        ...
        else
        {
            if ( mirrorCfg.nDirection == MIRROR_DIR_IN )
            {
                ...
                {
                    snprintf(acMirrorInLanDevice,MIRROR_INTF_SIZE,mirrorCfg.szMirrorInterface);
                    snprintf(acMirrorOutLanDevice,MIRROR_INTF_SIZE,mirrorCfg.szMirrorInterface);
                }
                else
                {
                    memset(acMirrorInLanDevice,0x00,MIRROR_INTF_SIZE);                    
                    memset(acMirrorOutLanDevice,0x00,MIRROR_INTF_SIZE);
                }
            }
            else 
            {
...
//spin_lock_bh(&pGi->xtmlock_tx);
        ...
        if( pDevCtx->ulTxQInfosSize )
        {
            ...
            if (isAtmCell)       /* Necessary for ASM/OAM cells */
               uMark |= 0x7;
#ifdef CONFIG_DT_QOS
            if (DtQosFonAtmImqEnqueue(uMark, pNBuffSkb, pNBuff, dev, &pGi->xtmlock_tx) != 0)
            {
                goto unlock_done_xmit;
            }
#endif            
        	    ...
        	    (pDevCtx->Addr.ulTrafficType == TRAFFIC_TYPE_PTM_BONDED))
        	{
        		...
        		/* QOS: Check speed and remark priority */
        		uPriority = DtQosCheckRemark(uPriority, len);
        		
        		/* For PTM, bit 2-0 of the 32-bit nfmark is the queue id. */
        		txdma = pDevCtx->pTxQids[uPriority];
            ...
            if (ulTxAvailable && QueuePacket(pGi, txdma, isAtmCell))
            {
                ...
                /*start modified by t00191685 for HG658B ÄÚ´æÐ¹Â©,2012-1-5*/
#if defined(CONFIG_ATP_MIRROR)
                if (('\0' != acMirrorOutLanDevice[0])                 	
                    && pNBuffSkb
                    && !isAtmCell &&
                    (ulHdrType ==  HT_PTM 
                     ...
                     || ulHdrType ==  HT_VC_MUX_ETHERNET))
                {
                    ...
                    {
                        MirrorPacket( pNBuffSkb, acMirrorOutLanDevice, 1 );
                        pNBuff = SKBUFF_2_PNBUFF( pNBuffSkb );
                        nbuff_get_context( pNBuff, &pData, &len );
                    }
                }
#endif
                /*end modified by t00191685 for HG658B ÄÚ´æÐ¹Â©,2012-1-5*/
...
static inline BOOL isLcpPacket(UINT8  *pBuf)
{
    /* ´øvlan */
    if (0x81 == pBuf[12] && 0x00 == pBuf[13])
    {
        if (0xc0 == pBuf[24] && 0x21 == pBuf[25])
        {
            return 1;
        }
    }
...
/*start modified by t00191685 for HG658B ÄÚ´æÐ¹Â©,2012-1-5*/
   ...
   if (('\0' != acMirrorInLanDevice[0]) 	
                  ...
                  ulHdrType ==  HT_VC_MUX_ETHERNET))
    {
       ...
       {
          spin_unlock_bh(&pGi->xtmlock_rx);
          MirrorPacket(skb_m, acMirrorInLanDevice, 0);
                           ...
                           BcmXtm_RxDma *rxdma, UINT8 *pucData)
{
    ...
    FlushAssignRxBuffer(rxdma->pktDmaRxInfo.channel, pucData, pucData + BCM_MAX_PKT_LEN);
...
//           unit = simple_strtoul(&(dev->name[i]), (char **)NULL, 10);
        ...
        default:
            ...
            if (pDevCtx->ulHdrType == HT_PTM)
                dev->mtu = PTM_MAX_MTU_PAYLOAD_SIZE;
            else
                dev->mtu = XTM_MAX_MTU_PAYLOAD_SIZE;
            break;
        }
