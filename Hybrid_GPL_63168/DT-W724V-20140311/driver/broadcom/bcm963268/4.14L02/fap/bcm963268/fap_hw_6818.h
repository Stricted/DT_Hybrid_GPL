#ifndef __FAP_HW_6818_H_INCLUDED__
#define __FAP_HW_6818_H_INCLUDED__
...
/*
 *******************************************************************************
 * File Name  : fap_hw_6828.h
 *
 * Description: This file contains 6828 specific hardware changes for FAP.
 *
 *******************************************************************************
 */
...
//#define PERF_MEAS_OFFSET      0x0600  //Not there
...
//#define HW_CNTR_OFFSET        0x9000  //not in reg spec
//#define DPE_BASIC_OFFSET      0x3F000 //not in reg spec --- what is this??? TBD
...
#ifdef USE_SMISBUS
#define PHYS_FAP_BASE_0           0x10c01000 /* 6818 using SMISBUS */
#define PHYS_FAP_BASE_1           0x10e01000 /* 6818 using SMISBUS */
#else
//#define PHYS_FAP_BASE_0           FAP0_CNTRL_BASE /* 6818 using UBUS    */
//#define PHYS_FAP_BASE_1           FAP1_CNTRL_BASE /* 6818 using UBUS    */
#define PHYS_FAP_BASE_0           0xb0801000 /* 6818 using UBUS    */
#define PHYS_FAP_BASE_1           0xb0a01000 /* 6818 using UBUS    */
...
static const uint32 FAP_INTERRUPT_IRQS[]       = { 26, 27 };
...
#define _4keHwCntrReg         ((volatile HwCntrs_S * )(_4KE_BASE + HW_CNTR_OFFSET))
...
#define FAP_QSM_SIZE                                   0x8000  /* 32K */
...
#if defined(CONFIG_BCM_FAP_PWRSAVE)
typedef enum {
    FapState_Awake,
    FapState_TuckingIn,
    FapState_TuckedIn,
...
#if defined(CONFIG_BCM_FAP_PWRSAVE)
    Fap_4keState    state;      /* power state (sleeping/waking up/awake, etc) of fap */
#endif /* CONFIG_BCM_FAP_PWRSAVE */
} Fap_FapInfo;


#if defined(CONFIG_BCM_FAP_PWRSAVE)
#define FAP_UNFORCE_AWAKE_TIMEOUT (3*HZ)
#define FAP_LOCK() spin_lock_bh(&gFaps.pwrLock)
#define FAP_UNLOCK() spin_unlock_bh(&gFaps.pwrLock)
#endif /* CONFIG_BCM_FAP_PWRSAVE */
...
{
#if defined(CONFIG_BCM_FAP_PWRSAVE)
    Fap_4keState    state;                  /* power status of both FAP cores */
    uint32          bWakeupAfterSleep;      /* set to true if a wakeup should after right after getting to sleep state */
    spinlock_t      pwrLock;                /* semaphore used for locking for purposes of powering up/down */
    uint32          sleepMask;              /* bitmask (shifted by Fap_FapSleepTriggers) */
    uint32          forcedAwake;            /* 0 if not forced awake, non-zero otherwise */
#endif /* CONFIG_BCM_FAP_PWRSAVE */
} Fap_FapSharedInfo;
...
#define pHostPacketShared(fapIdx)              ( pHostQsmGbl(fapIdx)->packet.shared )
...
convertFapAddrToSdram(uint32 fapIdx, void *fapAddr)
{
    if ( (fapAddr & ~gFap[fapIdx].blockMask[0]) == gFap[fapIdx].blockFapAddr[0])
    {
        /* address translator 0: */
        return (void *)((uint32)fapAddr & gFap[fapIdx].blockMask[0] + gFap[fapIdx].hostFapAddr[0]);        
    }
    else if ( (fapAddr & ~gFap[fapIdx].blockMask[1]) == gFap[fapIdx].blockFapAddr[1])
    {
        /* address translator 1: */
        return (void *)((uint32)fapAddr & gFap[fapIdx].blockMask[1] + gFap[fapIdx].hostFapAddr[1]);        
    }
    return fapAddr;    
}
#endif
...
#endif  /* defined(__FAP_HW_6818_H_INCLUDED__) */
