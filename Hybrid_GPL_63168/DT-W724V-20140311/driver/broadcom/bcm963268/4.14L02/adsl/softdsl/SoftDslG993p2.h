...
struct g993p2CommonDataPumpCapabilities
    ...
    unsigned long       annex;     /* 1->Annex A, 2->Annex B, 4->Annex C */
	};
...
#define AFE_CHIP_MAX				AFE_CHIP_6306
...
#define SoftDslAfeIsUs0AnnexBJ(x) (((gAfeInfo(x).boardAfeId & AFE_FE_ANNEX_MASK) >> AFE_FE_ANNEX_SHIFT) == 4)
...
struct ntrCntStruct {
    ...
    ulong   ncoCntAtNtr;
    /* 6362/63268 */
#if (defined(BCM6362_CHIP) || defined(BCM63268_CHIP) || defined(BCM6318_CHIP))
    long    phaseError;
    long    VCOAdjInfo;
#endif
};
...
struct ntrCfgStruct {
    ...
    ulong   updatePeriod;        /* PLL update period in milliseconds  */
...
typedef struct {
    ...
    unsigned long  pmeRemoteDiscoveryLow;
	unsigned long  pmeAggregationReg;
	long  timeOfLastCL;
...
#if !defined(__KERNEL__) && !defined(WINNT) && !defined(__ECOS)
