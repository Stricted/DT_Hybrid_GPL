...
#if (defined(__KERNEL__) && !defined(LINUX_DRIVER)) || defined(VXWORKS) || defined(_WIN32_WCE) || defined(TARG_OS_RTEMS) || defined(_CFE_) || defined(__ECOS)
...
typedef struct
    ...
    ulong       impdet_error_norm[kG992EqualizerImpDetMaxToneSetSize];
    ulong       impdet_impulse_norm;
...
struct FramerDeframerOptions
{
  ...
  unsigned long         ETR_kbps;   /* G.Inp: ETR - Expected throughput in kbits/sec */
...
struct _dslFramerBufDesc;
	...
	typedef	void*   (SM_DECL *dslHeaderHandlerType) (ulong hdr, uchar hdrHec);
	...
	typedef	ulong	(SM_DECL *dslLinkVc2IdHandlerType) (void *);
	typedef	void*	(SM_DECL *dslLinkVcId2VcHandlerType) (ulong);
	...
	typedef	void	(SM_DECL *dslLinkStatusHandler) (ulong statusCode, ...);
...
#define kDslSystemControlCmd		(kFirstDslCommandCode + 65)
  #define kDslSysCtlDiv0			  0x00000001
  #define kDslSysCtlFgTimeout		  0x00000002
...
#define kDslInteropCarryOverMask            (kDslInteropRetrainWithVDSL2Only|kDslInteropRetrainFor6314Power|kDslInteropEnableIkanosCO4ModeFull)
#else
...
#define kDslTestFreezeDuringRTones          22
...
#define kDslTestForce1storderHpOn           28
...
typedef struct
  ...
  short                           AfeForcePpmOffsetFlag;
	long int                        QLNsymCount;
	long int                        AfeLBsymCount;
#if defined(FIRE_RETRANSMISSION) || defined(FIRE_XMT_6368)
 ...
 short       ifxAdsl2pUSDetectFail; 
  ...
  long dslG994p1RevisionSwitchCounter;
    long dslG994p1NAKCounter;
  ...
  uchar g994p1FalseCToneCarrCount[12];
...
#ifdef	T1P413
	long		nextStartupMode;
	ulong		currentModeCounter;
#endif
	...
	ulong		t1p413HighLowPowerCounter;
...
#endif
    ulong   interopRetrainFlags;              /* Number of retrain flags are growing. Use bit map in interopRetrainFlags register */
...
extern ulong	SoftDslGetSymCntTotal(void *gDslVars);
extern ulong	SoftDslGetSymCntPhase(void *gDslVars);
...
extern void *  SoftDslBgMonUpdate(void *gDslVars, ulong cycCnt);
extern void	   SoftDslBgMonReset(void *gDslVars);
extern void	   SoftDslBgMonSetBgTcb(void *gDslVars, void *pTcb);

#if	(defined(BCM6368_SRC) && (!defined(VDSL_ONLY)))
extern ulong		SoftDslGetModulationCapability(void *gDslVars);
#ifdef	T1P413
extern void		SoftDslSelectStartupMode(void *gDslVars, int timeInterval, long currentStartupMode);
...
#define SoftDslGetTlsData(idx)				((ulong *)SoftDslGetTls())[idx]
#define SoftDslSetTlsData(idx,data)			((ulong *)SoftDslGetTls())[idx] = (data)
...
extern void SoftDslMipsSleep(ulong usec, int shift);
...
#if !defined(WINNT) && !defined(__KERNEL__) && !defined(_CFE_) && !defined(__ECOS)
...
#define DSL_STATUS_CODE(x)   ((x) & 0x0FFFFFFF)  /* bits 30,31 for line id,  bits 29,28 for client Type */
#if defined(DSL_BONDING) || defined(SUPPORT_DSL_BONDING)
#define	MAX_DSL_LINE		2
#define	DSL_COMMAND_CODE(x)	((x) & (~DSL_LINE_MASK))
#define	DSL_LINE_ID(x)		((uchar)(((ulong)(x) & DSL_LINE_MASK) >> DSL_LINE_SHIFT))
#else
