 ...
 *
 * $Id: wlc_bsscfg.h 409124 2013-06-22 04:35:15Z $
 */
...
typedef struct wlc_assoc {
	...
	struct wl_timer *disassoc_timer;	/* timer for auth/assoc request timeout */
...
typedef struct wlc_roam {
	...
	chanspec_t rcc_channels[MAX_ROAM_CHANNEL];
#endif
} wlc_roam_t;
...
#define TKIP_CM_KEYERR_NONSTA 0x40		/* applies to AP/tx - not client STA */
...
#endif
#ifdef WL_WMF_IGMP_QUERY_FILTER
	bool		wmf_igmp_query_filter;	/* 1 to enable, 0 by default */
#endif /* WL_WMF_IGMP_QUERY_FILTER */
#ifdef MCAST_REGEN
	...
	bool wpa2_auth_ft;
#endif
	/* 4335c0 ROM restriction for this structure */
#ifdef WLFBT
	uint8 current_akm_type; 	/* AKM suite used for current association */
#endif

	int32    rateset; 		/* bss rateset */
...
/* NEW FIELD MUST BE ADDED BEFORE #ifdef BCMDBG */
/* LEAVE THESE AT THE END */
#ifdef BCMDBG
	/* Rapid PM transition */
...
#define WLC_BSSCFG_PROXD	0x20000000	/* The BSS is for proximity detection */
#define WLC_BSSCFG_AWDL		0x40000000
#define WLC_BSSCFG_MCHAN_DISABLE	0x80000000 /* Do not particiate in mchan scheduler */
...
extern int wlc_bsscfg_start_ibss(wlc_bsscfg_t *cfg, const uint8 ssid[], int ssid_len,
	chanspec_t in_chanspec, bool radar_allowed, int type);
#endif
...
#ifdef WLAWDL
#define WLC_AWDL_BCMCSCB_GET(bsscfg) \
	(!BSSCFG_AWDL(bsscfg) ? NULL : \
	 (bsscfg)->bcmc_scb[CHSPEC_WLCBANDUNIT(bsscfg->current_bss->chanspec)])
#else
#define WLC_AWDL_BCMCSCB_GET(wlc, bsscfg) NULL
#endif
...
#define BSS_PROXD_ENAB(wlc, cfg) \
	(PROXD_ENAB((wlc)->pub) && (cfg) && ((cfg)->flags & WLC_BSSCFG_PROXD))
#else
...
#define BSSCFG_AWDL(cfg)    ((cfg) ? (cfg)->is_awdl : FALSE)
#else
#define BSS_AWDL_ENAB(wlc, cfg) 	FALSE
#define BSSCFG_AWDL(cfg)    (0)
#endif

#ifdef WLMCHAN
#define BSS_MCHAN_ENAB(wlc, cfg) \
	(MCHAN_ENAB((wlc)->pub) && !((cfg)->flags & WLC_BSSCFG_MCHAN_DISABLE))
#else
