...
#define BCM_VLAN_LOCK() BcmVlanLock( &bcmVlan_lock_g )
#define BCM_VLAN_UNLOCK() BcmVlanUnLock( &bcmVlan_lock_g )
#else
...
static inline void dumpHexData(void *head, int len)
{
    ...
    printk("addr : 0x%08X, length : %d bytes", (unsigned int)curPtr, len);
