...
ifneq ($(ROOTFS_DEVICE_TABLES),)
	...
	cd $(DEVICETABLE_DIR); find -name "*.static" | xargs cat >> $(FULL_DEVICE_TABLE)
	echo "$(HOST_DIR)/usr/bin/makedevs -d $(FULL_DEVICE_TABLE) $(TARGET_DIR)" >> $(FAKEROOT_SCRIPT)
endif
	echo "" > $(TARGET_DIR)/etc/devicetable
	cd $(DEVICETABLE_DIR); find -name "*.dynamic" | xargs cat >> $(TARGET_DIR)/etc/devicetable
...
####for bcm4708########---temp,BR2_LINUX_KERNEL_VERSION="2.6.36"
ifeq ($(BR2_LINUX_KERNEL_VERSION),"2.6.36")
	$(TOPDIR)/fs/bcm_router_tools/trx -o $(TOPDIR)/output/images/linux.trx $(TOPDIR)/output/images/vmlinuz $(TOPDIR)/output/images/rootfs.$(1)
endif
####end of bcm4708##########
