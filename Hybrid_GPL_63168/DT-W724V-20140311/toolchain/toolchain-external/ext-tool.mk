...
#声明工具链中常用工具
TOOLCHAIN := $(TOOLCHAIN_EXTERNAL_DIR)
CROSS_COMPILE = $(TOOLCHAIN_EXTERNAL_BIN)/$(TOOLCHAIN_EXTERNAL_PREFIX)-
...
TOOLCHAIN_EXTERNAL_DEPENDENCIES = 
else
...
$(STAMP_DIR)/ext-toolchain-installed: $(TOOLCHAIN_EXTERNAL_DEPENDENCIES)
	...
	ln -sf $(STAGING_DIR) $(HOST_DIR)/usr/sysroot; \
	touch $@
...
ifeq ($(BR2_ARCH),"i386")
export C_INCLUDE_PATH     := $(STAGING_DIR)/usr/include:$(HOST_DIR)/usr/include:$(C_INCLUDE_PATH)
export CPLUS_INCLUDE_PATH := $(STAGING_DIR)/usr/include:$(HOST_DIR)/usr/include:$(CPLUS_INCLUDE_PATH)
export LIBRARY_PATH       := $(STAGING_DIR)/lib:$(STAGING_DIR)/usr/lib:$(LIBRARY_PATH)
uclibc: dependencies
	@rm -f $(HOST_DIR)/usr/sysroot
else
uclibc: dependencies $(HOST_DIR)/usr/bin/ext-toolchain-wrapper
ifeq ($(BR2_ENABLE_DEBUG),y)
	if test -e $(CROSS_COMPILE)gdb; then\
		cp $(CROSS_COMPILE)gdb gdb;\
	fi
endif
uclibc-clean: 
	@rm -f $(HOST_DIR)/usr/bin/ext-toolchain-wrapper
	@rm -f $(STAMP_DIR)/ext-toolchain-installed
	@rm -f $(STAMP_DIR)/ext-toolchain-checked
endif
